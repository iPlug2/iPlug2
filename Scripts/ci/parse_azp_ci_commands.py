#!/usr/bin/env python3
"""
Parse /ci-azp commands from PR description for iPlug2 Azure Pipelines CI builds.

This script parses special CI commands from a GitHub PR description and outputs
Azure Pipeline variable commands to control what gets built (Xcode/VS projects).

Note: CMake builds use /ci-cmake commands and are handled by cmake-ci.yml.

Usage:
    python parse_azp_ci_commands.py <pr_body_text>
    python parse_azp_ci_commands.py --file <path_to_pr_body_file>

Command Syntax (in PR description):
    /ci-azp projects=Project1,Project2   # Build specific projects
    /ci-azp platforms=mac,win,ios,web    # Build specific platforms
    /ci-azp formats=vst3,app,clap,auv2   # Build specific formats
    /ci-azp graphics=nanovg,skia         # Graphics backends (or 'both')
    /ci-azp test                         # Enable plugin testing
    /ci-azp skip                         # Skip CI entirely

    # Shortcuts
    /ci-azp full                         # All projects, all enabled platforms
    /ci-azp quick                        # IPlugEffect only (default)
    /ci-azp web                          # Web platform only
    /ci-azp mac                          # macOS only
    /ci-azp win                          # Windows only
...
def parse_ci_commands(text):
    """Parse /ci-azp commands from PR body text."""
    result = {
        'projects': [],
        'platforms': [],
        'formats': [],
    ...
    # Find all /ci-azp command lines
    ci_pattern = re.compile(r'^/ci-azp\s+(.*)$', re.MULTILINE | re.IGNORECASE)
...
def output_github_actions(config, should_build=True):
    ...
    # Output graphics as JSON array for GitHub Actions matrix
    set_output('graphics_matrix', json.dumps(config['graphics']))
...
def main():
    parser = argparse.ArgumentParser(description='Parse /ci-azp commands from PR description')
    ...
    if args.event_type == 'dispatch':
        ...
        # Comment: build if /ci-azp found and not skip
        should_build = '/ci-azp' in text.lower() and not config['skip']
    else:  # pr
        # PR: build only if explicit /ci-azp command present
        should_build = bool(re.search(r'^/ci-azp\s', text, re.MULTILINE | re.IGNORECASE)) and not config['skip']
