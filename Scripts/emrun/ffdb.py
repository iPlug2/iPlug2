...
def adb_devices():
  try:
    ...
    devices = [x for x in devices if 'device' in x]
...
def b2g_screenshot(filename):
  ...
  if not isinstance(data, str): # The device is sending the screenshot in multiple fragments since it's too long to fit in one message?
