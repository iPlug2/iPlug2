...
static uint64_t GetAPIBusTypeForChannelIOConfig(int configIdx, ERoute dir, int busIdx, const IOConfig* pConfig,
                                                WDL_TypedBuf<uint64_t>* APIBusTypes = nullptr)
{
  ...
  switch (numChans)
  {
    ...
    default:
      DBGMSG(
        "No stem format found for this channel count, you need to implement GetAPIBusTypeForChannelIOConfig() and "
        "#define CUSTOM_BUSTYPE_FUNC\n");
      assert(0);
...
AAX_Result GetEffectDescriptions(AAX_ICollection* pC)
{
  ...
  if (PLUG_TYPE == 1)
    category = AAX_ePlugInCategory_SWGenerators;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "None") == (0))
    category = AAX_ePlugInCategory_None;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "EQ") == (0))
    category = AAX_ePlugInCategory_EQ;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "Dynamics") == (0))
    category = AAX_ePlugInCategory_Dynamics;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "PitchShift") == (0))
    category = AAX_ePlugInCategory_PitchShift;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "Reverb") == (0))
    category = AAX_ePlugInCategory_Reverb;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "Delay") == (0))
    category = AAX_ePlugInCategory_Delay;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "Modulation") == (0))
    category = AAX_ePlugInCategory_Modulation;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "Harmonic") == (0))
    category = AAX_ePlugInCategory_Harmonic;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "NoiseReduction") == (0))
    category = AAX_ePlugInCategory_NoiseReduction;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "Dither") == (0))
    category = AAX_ePlugInCategory_Dither;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "SoundField") == (0))
    category = AAX_ePlugInCategory_SoundField;
  else if (strcmp(AAX_PLUG_CATEGORY_STR, "Effect") == (0))
    category = AAX_ePlugInCategory_None;
  ...
  const int NIOConfigs = IPlugProcessor::ParseChannelIOStr(
    PLUG_CHANNEL_IO, channelIO, totalNInChans, totalNOutChans, totalNInBuses, totalNOutBuses);

  auto PopulateSetupInfo = [](int configIdx, const IOConfig* pConfig, int typeId, int audioSuiteId,
                              AAX_SIPlugSetupInfo& setupInfo) {
    if (PLUG_TYPE == 1 && pConfig->GetTotalNChannels(kInput) == 0)
    {
      // For some reason in protools instruments need to have input buses if not defined set input chan count the same
      // as output
      setupInfo.mInputStemFormat =
        (AAX_EStemFormat)GetAPIBusTypeForChannelIOConfig(configIdx, ERoute::kOutput, 0 /* first bus */, pConfig);
    }
    else
      setupInfo.mInputStemFormat =
        (AAX_EStemFormat)GetAPIBusTypeForChannelIOConfig(configIdx, ERoute::kInput, 0 /* first bus */, pConfig);

    setupInfo.mOutputStemFormat =
      (AAX_EStemFormat)GetAPIBusTypeForChannelIOConfig(configIdx, ERoute::kOutput, 0 /* first bus */, pConfig);
    ...
    {
      setupInfo.mAuxOutputStemFormats[i] =
        (AAX_EStemFormat)GetAPIBusTypeForChannelIOConfig(configIdx, ERoute::kOutput, i, pConfig);
