...
AAX_Result AAX_CIPlugParameters::UpdateParameterNormalizedValue(AAX_CParamID iParamID, double aValue,
...
AAX_Result AAX_CIPlugParameters::GenerateCoefficients()
{
  ...
  {
    TNumberedParamStateList* const numberedParamState =
      new TNumberedParamStateList(std::make_pair(stateNum, paramStateList));
    {
  ...
  result =
    Controller()->PostPacket(AAX_FIELD_INDEX(AAX_SIPlugRenderInfo, mCurrentStateNum), &stateNum, sizeof(int64_t));
...
AAX_Result AAX_CIPlugParameters::StaticDescribe(AAX_IEffectDescriptor* ioDescriptor,
                                                const AAX_SIPlugSetupInfo& setupInfo)
{
  ...
  if (setupInfo.mNeedsGlobalMIDI)
    err = compDesc->AddMIDINode(
      globalNodeID, AAX_eMIDINodeType_Global, setupInfo.mGlobalMIDINodeName, setupInfo.mGlobalMIDIEventMask);
  else
    err = compDesc->AddPrivateData(globalNodeID, sizeof(float), AAX_ePrivateDataOptions_DefaultOptions);

  if (setupInfo.mNeedsInputMIDI)
    err = compDesc->AddMIDINode(
      localInputNodeID, AAX_eMIDINodeType_LocalInput, setupInfo.mInputMIDINodeName, setupInfo.mInputMIDIChannelMask);
  else
  ...
  if (setupInfo.mNeedsOutputMIDI)
    err = compDesc->AddMIDINode(localOutputNodeID, AAX_eMIDINodeType_LocalOutput, setupInfo.mOutputMIDINodeName,
                                setupInfo.mOutputMIDIChannelMask);
  else
  ...
  {
    AAX_CFieldIndex nodeID =
      index + ((AAX_CFieldIndex)(offsetof(AAX_SIPlugRenderInfo, mAdditionalInputMIDINodes) / sizeof(void*)));
    AAX_CString nodeName(setupInfo.mInputMIDINodeName);

    if (index < setupInfo.mNumAdditionalInputMIDINodes)
      err = compDesc->AddMIDINode(
        nodeID, AAX_eMIDINodeType_LocalInput, nodeName.CString(), setupInfo.mInputMIDIChannelMask);
    else
  ...
  if (setupInfo.mNumMeters > 0)
    err = compDesc->AddMeters(
      AAX_FIELD_INDEX(AAX_SIPlugRenderInfo, mMeters), setupInfo.mMeterIDs, static_cast<uint32_t>(setupInfo.mNumMeters));
  else
    err = compDesc->AddPrivateData(
      AAX_FIELD_INDEX(AAX_SIPlugRenderInfo, mMeters), sizeof(float), AAX_ePrivateDataOptions_DefaultOptions);
  ...
  {
    err = compDesc->AddAuxOutputStem(0 /*not used*/, static_cast<int32_t>(setupInfo.mAuxOutputStemFormats[index]),
                                     setupInfo.mAuxOutputStemNames[index]);
  }

  // Add pointer to the data model instance and other interesting information.
  err = compDesc->AddPrivateData(AAX_FIELD_INDEX(AAX_SIPlugRenderInfo, mPrivateData), sizeof(AAX_SIPlugPrivateData),
  ...
  // AAX Hybrid
  AAX_ASSERT((AAX_eStemFormat_None != setupInfo.mHybridInputStemFormat)
             == (AAX_eStemFormat_None != setupInfo.mHybridOutputStemFormat));
  if ((AAX_eStemFormat_None != setupInfo.mHybridInputStemFormat)
      && (AAX_eStemFormat_None != setupInfo.mHybridOutputStemFormat))
  {
    err = properties->AddProperty(
      AAX_eProperty_HybridInputStemFormat, static_cast<int32_t>(setupInfo.mHybridInputStemFormat));
    err = properties->AddProperty(
      AAX_eProperty_HybridOutputStemFormat, static_cast<int32_t>(setupInfo.mHybridOutputStemFormat));
  }
...
void AAX_CALLBACK AAX_CIPlugParameters::StaticRenderAudio(AAX_SIPlugRenderInfo* const inInstancesBegin[],
                                                          const void* inInstancesEnd)
{
  for (AAX_SIPlugRenderInfo* const* instanceRenderInfoPtr = inInstancesBegin; instanceRenderInfoPtr != inInstancesEnd;
       ++instanceRenderInfoPtr)
  {
...
AAX_CIPlugParameters::SParamValList AAX_CIPlugParameters::GetUpdatesForState(int64_t inTargetStateNum)
{
  ...
  for (TNumberedParamStateList* numberedStateList = stateLists.Pop(); NULL != numberedStateList;
       numberedStateList = stateLists.Pop())
  {
...
{
  for (TNumberedParamStateList* numberedStateList = mFinishedParameterChanges.Pop(); NULL != numberedStateList;
       numberedStateList = mFinishedParameterChanges.Pop())
  {
  ...
  for (const TParamValPair* paramVal = mFinishedParameterValues.Pop(); NULL != paramVal;
       paramVal = mFinishedParameterValues.Pop())
  {
    if (paramVal)
    {
      delete paramVal;
