                                       ...
                                       error:(NSError**)ppOutError
{
  ...
  self = [super initWithComponentDescription:componentDescription options:options error:ppOutError];
...
#pragma mark Audio I/O
  ...
  if (nOutputBuses == 0) // MIDI FX
  {
    ...
    AVAudioChannelLayout* pChannelLayout =
      [[AVAudioChannelLayout alloc] initWithLayoutTag:kAudioChannelLayoutTag_Stereo];
    pOutputBusFormat = [[AVAudioFormat alloc] initStandardFormatWithSampleRate:DEFAULT_SAMPLE_RATE
    ...
    _mOutputBusArray = [[AUAudioUnitBusArray alloc] initWithAudioUnit:self
                                                              busType:AUAudioUnitBusTypeOutput
                                                               busses:pOutputBusses];
  }
  ...
  if (nInputBuses)
  {
    ...
    for (int busIdx = 0; busIdx < nInputBuses; busIdx++)
    {
      ...
      AVAudioChannelLayout* pChannelLayout =
        [[AVAudioChannelLayout alloc] initWithLayoutTag:pTags[nTags - 1]]; // assume last tag has most channels
      pInputBusFormat = [[AVAudioFormat alloc] initStandardFormatWithSampleRate:DEFAULT_SAMPLE_RATE
                                                                  channelLayout:pChannelLayout];
      if (pInputBusFormat)
    ...
    _mInputBusArray = [[AUAudioUnitBusArray alloc] initWithAudioUnit:self
...
#pragma mark parameters
  ...
  for (int paramIdx = 0; paramIdx < mPlug->NParams(); paramIdx++)
  {
    ...
    if (!pParam->GetCanAutomate())
      options |= kAudioUnitParameterFlag_NonRealTime;
    if (pParam->GetMeta())
      options |= kAudioUnitParameterFlag_IsElementMeta;

    switch (pParam->Type())
    {
      case IParam::kTypeBool: unit = kAudioUnitParameterUnit_Boolean; break;
      case IParam::kTypeEnum: unit = kAudioUnitParameterUnit_Indexed; break;
    ...
    AUParameter* pAUParam = [AUParameterTree
      createParameterWithIdentifier:[NSString stringWithFormat:@"%d", paramIdx]
  ...
  for (auto p = 0; p < [treeArray count]; p++)
  {
    ...
    {
      AUParameterGroup* pGroup = [AUParameterTree
        createGroupWithIdentifier:[NSString stringWithString:[NSString stringWithFormat:@"Group %d", p - 1]]
                             name:[NSString stringWithCString:mPlug->GetParamGroupName(p - 1)
...
#pragma mark presets
  ...
  else
  {
    ...
    {
      [pPresets addObject:NewAUPreset(i, [NSString stringWithCString:mPlug->GetPresetName(i)
                                                            encoding:NSUTF8StringEncoding])];
    }
  }
...
#pragma mark blocks
  ...
  dispatch_async(dispatch_get_main_queue(), ^{ pPlug->CreateTimer(); });
  ...
  mParameterTree.implementorValueProvider = ^(AUParameter* param) { return pPlug->GetParameter(param.address); };
  ...
  if (mPlug->HasUI())
  {
    mUIUpdateParamObserverToken =
      [mParameterTree tokenByAddingParameterObserver:^(AUParameterAddress address, AUValue value) {
        dispatch_async(dispatch_get_main_queue(), ^{ pPlug->SendParameterValueFromObserver(address, value); });
      }];
  ...
  return ^AUAudioUnitStatus(AudioUnitRenderActionFlags* actionFlags, const AudioTimeStamp* timestamp,
    ...
    if (outputBusNumber == lastOutputBusConnected)
    {
      ...
      if (_musicalContextCapture)
      {
        ...
        _musicalContextCapture(
          &tempo, &numerator, &denominator, &ppqPos, &sampleOffsetToNextBeat, &currentMeasureDownbeatPosition);
      ...
      if (_transportStateCapture)
      {
        ...
        timeInfo.mTransportIsRunning =
          transportStateFlags == AUHostTransportStateMoving || transportStateFlags == AUHostTransportStateRecording;
    ...
    if (currentPreset.number >= 0)
    {
      ...
      for (AUAudioUnitPreset* pFactoryPreset in self->mPresets)
      {
        if (currentPreset.number == pFactoryPreset.number)
        {
          ...
          {
            AUParameter* parameterToChange =
              [self->mParameterTree parameterWithAddress:self->mPlug->GetParamAddress(paramIdx)];
...
#else
  [pDict setValue:[NSNumber numberWithInt:mPlug->GetPluginVersion(false)]
           forKey:[NSString stringWithUTF8String:kAUPresetVersionKey]];
  [pDict setValue:[NSNumber numberWithInt:mPlug->GetAUPluginType()]
           forKey:[NSString stringWithUTF8String:kAUPresetTypeKey]];
  [pDict setValue:[NSNumber numberWithInt:mPlug->GetUniqueID()]
           forKey:[NSString stringWithUTF8String:kAUPresetSubtypeKey]];
  [pDict setValue:[NSNumber numberWithInt:mPlug->GetMfrID()]
           forKey:[NSString stringWithUTF8String:kAUPresetManufacturerKey]];
  [pDict setValue:[NSString stringWithUTF8String:mPlug->GetPresetName(mPlug->GetCurrentPresetIdx())]
           forKey:[NSString stringWithUTF8String:kAUPresetNameKey]];
...
- (void)beginInformHostOfParamChange:(uint64_t)address;
{
  ...
  [parameterToChange setValue:exitingValue
                   originator:mUIUpdateParamObserverToken
                   atHostTime:0
                    eventType:AUParameterAutomationEventTypeTouch]; // TODO: atHostTime:0 ?
}
...
- (void)endInformHostOfParamChange:(uint64_t)address;
{
  ...
  [parameterToChange setValue:exitingValue
                   originator:mUIUpdateParamObserverToken
                   atHostTime:0
                    eventType:AUParameterAutomationEventTypeRelease]; // TODO: atHostTime:0 ?
}
