...
void IPlugAPPHost::PopulateDriverSpecificControls(HWND hwndDlg)
{
	...
	{
		SendDlgItemMessage(
			hwndDlg, IDC_COMBO_AUDIO_IN_DEV, CB_ADDSTRING, 0, (LPARAM) GetAudioDeviceName(mAudioInputDevs[i]).c_str());
	...
	for (int i = 0; i < mAudioOutputDevs.size(); i++)
	{
		SendDlgItemMessage(hwndDlg,
						   ...
						   CB_ADDSTRING,
						   0,
						   (LPARAM) GetAudioDeviceName(mAudioOutputDevs[i]).c_str());
...
bool IPlugAPPHost::PopulateMidiDialogs(HWND hwndDlg)
{
	...
	else
	{
		...
		LRESULT indevidx = SendDlgItemMessage(
			hwndDlg, IDC_COMBO_MIDI_IN_DEV, CB_FINDSTRINGEXACT, -1, (LPARAM) mState.mMidiInDev.Get());
		...
		{
			SendDlgItemMessage(
				hwndDlg, IDC_COMBO_MIDI_OUT_DEV, CB_ADDSTRING, 0, (LPARAM) mMidiOutputDevNames[i].c_str());
		}

		LRESULT outdevidx = SendDlgItemMessage(
			hwndDlg, IDC_COMBO_MIDI_OUT_DEV, CB_FINDSTRINGEXACT, -1, (LPARAM) mState.mMidiOutDev.Get());
		...
		case WM_COMMAND:
			switch (LOWORD(wParam))
			{
				...
				case IDC_COMBO_AUDIO_IN_R:
					if (HIWORD(wParam) == CBN_SELCHANGE)
						SendDlgItemMessage(
							hwndDlg, IDC_COMBO_AUDIO_IN_R, CB_SETCURSEL, mState.mAudioInChanR - 1, 0);  // TODO: TEMP
				...
				case IDC_COMBO_AUDIO_OUT_R:
					if (HIWORD(wParam) == CBN_SELCHANGE)
						SendDlgItemMessage(
							hwndDlg, IDC_COMBO_AUDIO_OUT_R, CB_SETCURSEL, mState.mAudioOutChanR - 1, 0);  // TODO: TEMP
					mState.mAudioOutChanR =
						(int) SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_OUT_R, CB_GETCURSEL, 0, 0);
					break;
				...
				case IDC_COMBO_AUDIO_SR:
					if (HIWORD(wParam) == CBN_SELCHANGE)
					{
						...
						mState.mAudioSR =
							(uint32_t) SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_SR, CB_GETITEMDATA, idx, 0);
					}
					break;
...
#if PLATFORM_WINDOWS
						if ((_this->mState.mAudioDriverType == kDeviceASIO) &&
							(_this->mDAC->isStreamRunning() == true))  // TODO: still not right
							ASIOControlPanel();
#elif PLATFORM_MAC
