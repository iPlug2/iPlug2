...
const std::string kBufferSizeOptions[kNumBufferSizeOptions] = {
...
public:
  ...
  struct AppState
  {
    ...
    {
      return (rhs.mAudioDriverType == mAudioDriverType && rhs.mBufferSize == mBufferSize && rhs.mAudioSR == mAudioSR
              && rhs.mMidiInChan == mMidiInChan && rhs.mMidiOutChan == mMidiOutChan
...
private:
  ...
  AppState mTempState;
  /** When the audio driver is started the current state is copied here so that if OK is pressed after APPLY nothing is
   * changed */
