...
#pragma mark - Methods you can implement/override in your plug-in class - you do not call these methods
  ...
  /** Override this method to implement a custom comparison of incoming state data with your plug-ins state data, in
   * order to support the ProTools compare light when using custom state chunks. The default implementation will compare
  ...
  /* Implement this and return true to trigger your custom about box, when someone clicks about in the menu of a
   * standalone app or VST3 plugin */
  virtual bool OnHostRequestingAboutBox() { return false; }

  /* Implement this and return true to trigger your custom help info, when someone clicks help in the menu of a
   * standalone app or VST3 plugin */
  virtual bool OnHostRequestingProductHelp() { return false; }

  /** Implement this to do something specific when IPlug becomes aware of the particular host that is hosting the
   * plug-in. The method may get called multiple times. */
   ...
   * @param results You should populate this typed buf with the indexes of the overview parameters if the host wants to
   * show count number of controls */
  ...
  /** You need to implement this method if you are not using IGraphics and you want to support AAX's view interface
   * functionality (special shortcuts to add automation for a parameter etc.)
...
#pragma mark - Methods you can call - some of which have custom implementations in the API classes, some implemented in IPlugAPIBase.cpp
  /** SetParameterValue is called from the UI in the middle of a parameter change gesture (possibly via delegate) in
   * order to update a parameter's value. It will update mParams[paramIdx], call InformHostOfParamChange and
   * IPlugAPIBase::OnParamChange();
  ...
  /** In a distributed VST3 or WAM plugin, if you modify the parameters on the UI side (e.g. recall preset in custom
   * preset browser), you can call this to update the parameters on the DSP side */
...
#pragma mark - Methods called by the API class - you do not call these methods in your plug-in class
  ...
  /** This is called from the plug-in API class in order to update UI controls linked to plug-in parameters, prior to
   * calling OnParamChange() NOTE: It may be called on the high priority audio thread. Its purpose is to place parameter
   * changes in a queue to defer to main thread for the UI
   * @param paramIdx The index of the parameter that changed
  ...
  /** This method is implemented in some API classes, in order to do specific initialisation for particular problematic
   * hosts. This is not the same as OnHostIdentified(), which you may implement in your plug-in class to do your own
   * specific initialisation after a host has been identified */
...
private:
  ...
  /** Implemented by the API class, called by the UI via SetParameterValue() with the value of a parameter change
   * gesture
   * @param paramIdx The parameter that is being changed
...
private:
  ...
  IPlugQueue<IMidiMsg> mMidiMsgsFromEditor{
    MIDI_TRANSFER_SIZE}; // a queue of midi messages generated in the editor by clicking keyboard UI etc
  IPlugQueue<IMidiMsg> mMidiMsgsFromProcessor{
    MIDI_TRANSFER_SIZE}; // a queue of MIDI messages received (potentially on the high priority thread), by the
                         // processor to send to the editor
