...
void IPluginBase::GetBuildInfoStr(WDL_String& str, const char* date, const char* time) const
{
  ...
  str.SetFormatted(
    MAX_BUILD_INFO_STR_LEN, "%s %s (%s), built on %s at %.5s ", version.Get(), GetAPIStr(), GetArchStr(), date, time);
}
...
void IPluginBase::InitParamRange(int startIdx, int endIdx, int countStart, const char* nameFmtStr, double defaultVal,
                                 double minVal, double maxVal, double step, const char* label, int flags,
                                 const char* group, const IParam::Shape& shape, IParam::EParamUnit unit,
                                 IParam::DisplayFunc displayFunc)
{
  ...
  for (auto p = startIdx; p <= endIdx; p++)
  {
    ...
    GetParam(p)->InitDouble(
      nameStr.Get(), defaultVal, minVal, maxVal, step, label, flags, group, shape, unit, displayFunc);
  }
}

void IPluginBase::CloneParamRange(int cloneStartIdx, int cloneEndIdx, int startIdx, const char* searchStr,
...
{
  ForParamInRange(startIdx, endIdx, [](int paramIdx, IParam& param) { param.SetToDefault(); });
}

void IPluginBase::DefaultParamValues(const char* paramGroup)
{
  ForParamInGroup(paramGroup, [](int paramIdx, IParam& param) { param.SetToDefault(); });
}
...
void IPluginBase::DumpMakePresetSrc(const char* filename) const
{
  ...
  if (!sDumped)
  {
    ...
    for (i = 0; i < n; ++i)
    {
      ...
      switch (pParam->Type())
      {
        case IParam::kTypeBool: snprintf(paramVal, maxLen, "%s", (pParam->Bool() ? "true" : "false")); break;
        case IParam::kTypeInt: snprintf(paramVal, maxLen, "%d", pParam->Int()); break;
        case IParam::kTypeEnum: snprintf(paramVal, maxLen, "%d", pParam->Int()); break;
        case IParam::kTypeDouble:
        default: snprintf(paramVal, maxLen, "%.6f", pParam->Value()); break;
      }
...
bool IPluginBase::LoadPresetFromFXP(const char* file)
{
  if (CStringHasContents(file))
  {
    ...
    if (fp)
    {
      ...
      if (chunkMagic != 'CcnK')
        return false;
      if (fxpVersion != kFXPVersionNum)
        return false; // TODO: what if a host saves as a different version?
      if (pluginID != GetUniqueID())
        return false;
      // if (pluginVersion != GetPluginVersion(true)) return false; // TODO: provide mechanism for loading earlier
      // versions if (numParams != NParams()) return false; // TODO: provide mechanism for loading earlier versions with
      // less params
  ...
  if (CStringHasContents(file))
  {
    ...
    if (fp)
    {
      ...
      if (pluginID != GetUniqueID())
        return false;
      // if (pluginVersion != GetPluginVersion(true)) return false; // TODO: provide mechanism for loading earlier
      // versions if (numPgms != NPresets()) return false; // TODO: provide mechanism for loading earlier versions with
      // less params
