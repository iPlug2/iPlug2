...
IPluginBase::IPluginBase(int nParams, int nPresets) : EDITOR_DELEGATE_CLASS(nParams)
{
...
void IPluginBase::GetBuildInfoStr(WDL_String& str) const
{
	...
	str.SetFormatted(MAX_BUILD_INFO_STR_LEN,
					 "%s version %s %s (%s), built on %s at %.5s ",
					 GetPluginName(),
					 version.Get(),
					 GetAPIStr(),
					 GetArchStr(),
...
void IPluginBase::DumpMakePresetSrc(const char* filename) const
{
	...
	if (!sDumped)
	{
		...
		for (i = 0; i < n; ++i)
		{
			...
			switch (pParam->Type())
			{
				case IParam::kTypeBool: sprintf(paramVal, "%s", (pParam->Bool() ? "true" : "false")); break;
				case IParam::kTypeInt: sprintf(paramVal, "%d", pParam->Int()); break;
				case IParam::kTypeEnum: sprintf(paramVal, "%d", pParam->Int()); break;
				case IParam::kTypeDouble:
				default: sprintf(paramVal, "%.6f", pParam->Value()); break;
			}
		...
		if (fp)
		{
			...
			if (DoesStateChunks() && fxbMagic == 'FBCh')
			{
				...
				return true;
					 ...
					 'FxBk')  // Due to the big Endian-ness of FXP/FXB format we cannot call SerializeParams()
			{
				int32_t chunkMagic;
				int32_t byteSize;
				int32_t fxpMagic;
