...
public:
  ...
  {
    int errorCount = REAPERAPI_LoadAPI(
      [this](const char* str) { return (void*)mHostCallback(NULL, 0xdeadbeef, 0xdeadf00d, 0, (void*)str, 0.0); });
    if (errorCount > 0)
   ...
   * @param xpos x position on the screen to popup the menu. NOTE: screen coordinates! Can convert mouse position in
   * IControls with IGraphics::ClientToScreen()
   * @param ypos y position on the screen to popup the menu. NOTE: screen coordinates! Can convert mouse position in
   * IControls with IGraphics::ClientToScreen() */
  void CreateParameterPopup(void* pView, int xpos, int ypos)
  {
    ...
    *(VstIntPtr*)&DoFxLastTweakParmCtxMenu2 =
      mHostCallback(NULL, 0xdeadbeef, 0xdeadf00d, 0, (void*)"DoFxLastTweakParmCtxMenu2", 0.0);
  ...
  MediaTrack* GetReaperTrack() { return (MediaTrack*)mHostCallback(&mAEffect, 0xdeadbeef, 0xdeadf00e, 1, 0, 0.0); }
  ...
  void InitializeVideo(void* staticProcessVideoFrame, void* staticGetVideoParam)
  {
    ...
    if (pCtx)
    {
      ...
      *(void**)&video_CreateVideoProcessor =
        (void*)mHostCallback(&mAEffect, 0xdeadbeef, 0xdeadf00d, 0, (void*)"video_CreateVideoProcessor", 0.0f);
      if (video_CreateVideoProcessor)
      {
        ...
        if (mVideoProc)
        {
          ...
          mVideoProc->process_frame =
            (IVideoFrame * (*)(IREAPERVideoProcessor*, const double*, int, double, double, int))
              staticProcessVideoFrame;
