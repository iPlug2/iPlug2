...
#pragma mark - Plug-in properties
   ...
   * @return Effect version in VVVVRRMM (if \p decimal is \c true) or Hexadecimal 0xVVVVRRMM (if \p decimal is \c false)
   * format */
  ...
  /** Get the product name as a CString. A shipping product may contain multiple plug-ins, hence this. Not used in all
   * APIs */
...
#pragma mark - Parameters
  ...
  /** Implemented by the API class, call this if you update parameter labels and hopefully the host should update it's
   * displays (not applicable to all APIs) */
...
#pragma mark - State Serialization
  ...
  /** Serializes the current double precision floating point, non-normalised values (IParam::mValue) of all parameters,
   * into a binary byte chunk.
  ...
  {
    assert(idx > -1 && idx < NPresets());
    mCurrentPresetIdx = idx;
  }

  /** Implemented by the API class, called by the UI (etc) when the plug-in initiates a program/preset change (not
...
#pragma mark - Preset Manipulation
  ...
  /** Gets the number of factory presets. NOTE: some hosts don't like 0 presets, so even if you don't support factory
   * presets, this method should return 1
  ...
  /** This method can be used to initialize baked-in factory presets with the default parameter values. It finds the
   * first uninitialized preset and initializes nPresets that follow sequentially. Typically you would use it if you
   * have configured you plugin bank to e.g. 32 presets, you provide e.g. 8 factory presets via MakePresetXXX, but you
   * want to fill the rest of the 32 presets with default values
   ...
   * usage: MakePresetFromNamedParams(name, nParamsNamed, paramEnum1, paramVal1, paramEnum2, paramVal2, ..., paramEnumN,
   * paramVal2) See DumpMakePresetFromNamedParamsSrc() which is a utility that can be used to create the code for
   * MakePreset() calls nParamsNamed may be less than the total number of params.
   * @param name The preset name
  ...
  /** Creates a preset from an IByteChunk containging serialized data. This can be used when your plugin state includes
   * arbitary data, other than just parameters.
   * @param name The preset name
   ...
   * @param file The full path of the file to write or overwrite
   * @param paramEnumNames A list of all parameter names. e.g. const char* pParamNames[] = {"kParam1", "kParam2",
   * "kParam3"}; */
...
#pragma mark - Parameter manipulation
  ...
  /** Initialise a range of parameters simultaneously. This mirrors the arguments available in IParam::InitDouble, for
   * maximum flexibility
   * @param startIdx The index of the first parameter to initialise
   * @param endIdx The index of the last parameter to initialise
   * @param countStart An integer representing the start of the count in the format string. If the first parameter
   * should have "0" in its name, set this to 0
   * @param nameFmtStr A limited format string where %i can be used to get the index + countStart, in the range of
  ...
  void InitParamRange(int startIdx, int endIdx, int countStart, const char* nameFmtStr, double defaultVal,
                      double minVal, double maxVal, double step, const char* label = "", int flags = 0,
                      const char* group = "", const IParam::Shape& shape = IParam::ShapeLinear(),
                      IParam::EParamUnit unit = IParam::kUnitCustom, IParam::DisplayFunc displayFunc = nullptr);
  ...
  void CloneParamRange(int cloneStartIdx, int cloneEndIdx, int startIdx, const char* searchStr = "",
                       const char* replaceStr = "", const char* newGroup = "");
   ...
   * @param func A lambda function to modify the parameter. Ideas: you could randomise the parameter value or reset to
   * default, modify certain params based on their group */
...
private:
  ...
  /** A list of unique cstrings found specified as "parameter groups" when defining IParams. These are used in various
   * APIs to group parameters together in automation dialogues. */
