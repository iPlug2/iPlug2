...
void IPlugEmAudioWorklet::OnProcessorCreated(EMSCRIPTEN_WEBAUDIO_T audioContext, bool success, void* userData)
{
  ...
  int sr = EM_ASM_INT({ return Module.audioContext.sampleRate; });
                                       ...
                                       void* userData)
{
  ...
  // Attach input buffers
  if (numInputs > 0 && inputs)
  {
    for (int i = 0; i < inputs[0].numberOfChannels && i < pPlugin->MaxNChannels(ERoute::kInput); i++)
    {
      pPlugin->AttachBuffers(ERoute::kInput, i, 1, &inputs[0].data[i * blockSize], blockSize);
    }
  }

  // Attach output buffers
  if (numOutputs > 0 && outputs)
  {
    for (int i = 0; i < outputs[0].numberOfChannels && i < pPlugin->MaxNChannels(ERoute::kOutput); i++)
    {
      pPlugin->AttachBuffers(ERoute::kOutput, i, 1, &outputs[0].data[i * blockSize], blockSize);
    }
  }
...
// IEditorDelegate methods - send messages to UI thread
...
{
  // For binary data, we need to copy to heap and pass pointer
  EM_ASM({
    if (Module.SCMFD) {
