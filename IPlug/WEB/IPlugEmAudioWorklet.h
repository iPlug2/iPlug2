...
public:
  ...
  // Parameter gesture tracking (for DAW automation recording)
  void BeginParamChangeFromUI(int paramIdx);
  void EndParamChangeFromUI(int paramIdx);
...
private:
  ...
  // Worklet thread stack (allocated once, must be 16-byte aligned for AudioWorklet)
  // 32KB is recommended for complex DSP operations
  static constexpr int kWorkletStackSize = 32768;
  alignas(16) uint8_t mWorkletStack[kWorkletStackSize];
