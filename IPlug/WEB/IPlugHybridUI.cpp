...
IPlugHybridUI::IPlugHybridUI(const InstanceInfo& info, const Config& config)
: IPlugAPIBase(config, kAPIWEB)
{
  mControllerName.SetFormatted(32, "%s_Hybrid", GetPluginName());
}

void IPlugHybridUI::SendParameterValueFromUI(int paramIdx, double value)
{
  ...
  EM_ASM({
    if (typeof window.iPlugController !== 'undefined') {
      ...
      console.warn('IPlugHybridUI: controller not ready');
    }
  }, paramIdx, value);
...
void IPlugHybridUI::SendMidiMsgFromUI(const IMidiMsg& msg)
{
  EM_ASM({
    if (typeof window.iPlugController !== 'undefined') {
...
void IPlugHybridUI::SendSysexMsgFromUI(const ISysEx& msg)
{
  EM_ASM({
    if (typeof window.iPlugController !== 'undefined') {
...
void IPlugHybridUI::SendArbitraryMsgFromUI(int msgTag, int ctrlTag, int dataSize, const void* pData)
{
  if (dataSize > 0 && pData)
  {
    EM_ASM({
...
void IPlugHybridUI::SendDSPIdleTick()
{
  EM_ASM({
    if (typeof window.iPlugController !== 'undefined') {
...
extern std::unique_ptr<iplug::IPlugHybridUI> gPlug;
