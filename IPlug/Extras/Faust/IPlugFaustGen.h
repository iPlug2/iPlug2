...
#pragma once
  ...
  #define FAUST_BLOCK(class, member, file, nvoices, rate)                                                              \
    FaustGen member                                                                                                    \
    {                                                                                                                  \
      #class, file, nvoices, rate                                                                                      \
    }
#else
  #define FAUST_BLOCK(class, member, file, nvoices, rate)                                                              \
    Faust_##class member                                                                                               \
  ...
  // if this file is not found, you need to run the code without FAUST_COMPILED defined and make sure to call
  // CompileCPP();
...
static inline StatTime GetModifiedTime(StatType& s)
{
  return s.st_mtimespec;
}
static inline bool Equal(StatTime a, StatTime b)
{
...
static inline StatTime TimeZero()
{
  ...
  #define FAUST_RECOMPILE_INTERVAL 5000 // ms
    ...
    #else
      #define FAUST_EXE                                                                                                \
        "C:\\\"Program Files\"\\Faust\\bin\\faust.exe" // Double quotes around "Program Files" because of whitespace
    #endif
  #endif
...
class FaustGen : public IPlugFaust
{
  ...
  #else
    static std::string GetLLVMArchStr() { return ""; }
  ...
  private:
    struct FMeta : public Meta
    {
      ...
      const std::string get(const char* key, const char* value) { return items.Get(key, value); }
...
public:
  FaustGen(const char* name, const char* inputDSPFile = 0, int nVoices = 1, int rate = 1, const char* outputCPPFile = 0,
           const char* drawPath = 0, const char* libraryPath = FAUST_SHARE_PATH);
   ...
   * @param maxNInputs Specify a number here to tell FaustGen the maximum number of inputs the hosting code can
   * accommodate
   * @param maxNOutputs Specify a number here to tell FaustGen the maximum number of outputs the hosting code can
   * accommodate */
  void SetMaxChannelCount(int maxNInputs, int maxNOutputs) override
  {
    mMaxNInputs = maxNInputs;
    mMaxNOutputs = maxNOutputs;
  }
  ...
  {
    mFactory->FreeDSPFactory();
    mFactory->LoadFile(path);
  }

  /** This method allows SVG files generated by a specific instance of FaustGen can be located. The path to the SVG file
   * for process.svg will be returned, if drawPath has been specified in the constructor. This method will trigger an
   * assertion if drawPath has not been specified
   * @param path The absolute path to process.svg for this instance. */
  void GetDrawPath(WDL_String& path) override;

  /** This is a static method that can be called to compile all .dsp files used to a single .hpp file, using the
   * commandline FAUST compiler
