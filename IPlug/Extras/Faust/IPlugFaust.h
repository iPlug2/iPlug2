...
/** This abstract interface is used by the IPlug FAUST architecture file and the IPlug libfaust JIT compiling class
 * FaustGen In order to provide a consistent interface to FAUST DSP whether using the JIT compiler or a compiled C++
 * class */
...
public:
  ...
  virtual ~IPlugFaust() { mParams.Empty(true); }
  ...
  /** In FaustGen this is implemented, so that the SVG files generated by a specific instance can be located. The path
   * to the SVG file for process.svg will be returned. There is a NO-OP implementation here so that when not using the
   * JIT compiler, the same class can be used interchangeably
   * @param path The absolute path to process.svg for this instance. */
  virtual void GetDrawPath(WDL_String& path) {}

  /** Call this method from FaustGen in order to execute a shell command and compile the C++ code against the
   * IPlugFaust_arch architecture file There is a NO-OP implementation here so that when not using the JIT compiler, the
   * same class can be used interchangeably
  ...
  void ProcessMidiMsg(const IMidiMsg& msg) { mMidiHandler->decodeMessage(msg); }
  ...
  void addButton(const char* label, FAUSTFLOAT* zone) override { AddOrUpdateParam(IParam::kTypeBool, label, zone); }
...
protected:
  void AddOrUpdateParam(IParam::EParamType type, const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init = 0.,
                        FAUSTFLOAT min = 0., FAUSTFLOAT max = 0., FAUSTFLOAT step = 1.);

  void BuildParameterMap();

  int FindExistingParameterWithName(const char* name);

  void OnUITimer(Timer& timer) { GUI::updateAllGuis(); }
  ...
  WDL_StringKeyedArray<FAUSTFLOAT*>
    mMap; // map is used for setting FAUST parameters by name, also used to reconnect existing parameters
