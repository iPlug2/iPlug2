...
public:
  ...
  void SetStageTime(int stage, T timeMS)
  {
    switch (stage)
    {
      case kAttack:
        ...
        break;
      case kDecay: mDecayIncr = CalcIncrFromTimeExp(Clip(timeMS, MIN_ENV_TIME_MS, MAX_ENV_TIME_MS), mSampleRate); break;
      case kRelease:
  ...
  T GetPrevOutput() const { return mPrevOutput; }
  ...
  /** Kill the envelope
   * @param hard If true, the envelope will get reset automatically, probably causing an audible glitch. If false, it's
   * a "soft kill", which will fade out in EARLY_RELEASE_TIME */
  ...
  inline T Process(T sustainLevel = 0.)
  {
    ...
    switch (mStage)
    {
      case kIdle: result = mEnvValue; break;
      case kAttack:
      ...
      case kDecay:
        ...
        break;
      case kSustain: result = sustainLevel; break;
      case kRelease:
      ...
      case kReleasedToEndEarly:
        ...
        result = mEnvValue * mReleaseLevel;
        break;
      default: result = mEnvValue; break;
    }
