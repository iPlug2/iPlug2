...
{
  const char* options[] = {
    "document_root", DOCUMENT_ROOT, "listening_ports", PORT, "extra_mime_types", ".wasm=application/wasm", 0};
  ...
  if (!sServer)
  {
    try
    {
      sServer = std::make_unique<CivetServer>(cpp_options);
    }
...
bool IWebsocketServer::handleData(CivetServer* pServer, struct mg_connection* pConn, int bits, char* pData,
                                  size_t dataSize)
{
  WDL_MutexLock lock(&mMutex);
