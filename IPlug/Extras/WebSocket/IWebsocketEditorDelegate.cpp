...
bool IWebsocketEditorDelegate::OnWebsocketData(int connIdx, void* pData, size_t dataSize)
{
  ...
  {
    int paramIdx = *((int*)(pByteData + pos));
    pos += 4;
    double value = *((double*)(pByteData + pos));
    pos += 8;
  ...
  {
    IMidiMsg msg;
    msg.mStatus = *((uint8_t*)(pByteData + pos));
    pos++;
    msg.mData1 = *((uint8_t*)(pByteData + pos));
    pos++;
    msg.mData2 = *((uint8_t*)(pByteData + pos));
    pos++;
...
void IWebsocketEditorDelegate::SendControlValueFromDelegate(int ctrlTag, double normalizedValue)
{
  IByteChunk data;
  data.PutStr("SCVFD");
  data.Put(&ctrlTag);
...
void IWebsocketEditorDelegate::ProcessWebsocketQueue()
{
  while (mParamChangeFromClients.ElementsAvailable())
  {
    ...
    SendParameterValueFromDelegate(
      p.idx, p.value, true); // TODO:  if the parameter hasn't changed maybe we shouldn't do anything?
  }
  ...
  while (mMIDIFromClients.ElementsAvailable())
  {
    ...
    IGEditorDelegate::SendMidiMsgFromDelegate(
      msg); // Call the superclass, since we don't want to send another MIDI message to the websocket
