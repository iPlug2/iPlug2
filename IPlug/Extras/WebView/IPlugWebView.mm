...
namespace iplug
{
extern bool GetResourcePathFromBundle(const char* fileName, const char* searchExt, WDL_String& fullPath,
                                      const char* bundleID);
}
...
void* IWebView::OpenWebView(void* pParent, float x, float y, float w, float h, float scale)
{
  ...
  WKUserScript* script1 =
    [[WKUserScript alloc] initWithSource:@"function IPlugSendMsg(m) { webkit.messageHandlers.callback.postMessage(m); }"
                           injectionTime:WKUserScriptInjectionTimeAtDocumentStart
                        forMainFrameOnly:YES];
  [controller addUserScript:script1];

  // this script prevents view scaling on iOS
  WKUserScript* script2 =
    [[WKUserScript alloc] initWithSource:
                            @"var meta = document.createElement('meta'); meta.name = 'viewport'; meta.content = "
                            @"'width=device-width, initial-scale=1.0, maximum-scale=1.0, "
                            @"user-scalable=no, shrink-to-fit=YES'; var head = "
                            @"document.getElementsByTagName('head')[0]; head.appendChild(meta);"
  ...
  // #else
  //   [webView setAutoresizingMask: UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleLeftMargin |
...
void IWebView::LoadFile(const char* fileName, const char* bundleID)
{
  ...
  NSString* str = @"file:";
  NSString* webroot =
    [str stringByAppendingString:[pPath stringByReplacingOccurrencesOfString:[NSString stringWithUTF8String:fileName]
                                                                  withString:@""]];
  NSURL* pageUrl = [NSURL URLWithString:[webroot stringByAppendingString:[NSString stringWithUTF8String:fileName]]
                          relativeToURL:nil];
...
void IWebView::EvaluateJavaScript(const char* scriptStr, completionHandlerFunc func)
{
  ...
  {
    [webView evaluateJavaScript:[NSString stringWithUTF8String:scriptStr]
