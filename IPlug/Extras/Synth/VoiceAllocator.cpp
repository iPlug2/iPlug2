...
void VoiceAllocator::clearVoiceInputs(SynthVoice* pv)
{
...
void VoiceAllocator::AddVoice(SynthVoice* pv, uint8_t zone)
{
	if(mVoicePtrs.size() + 1 < UCHAR_MAX)
	{
		mVoicePtrs.push_back(pv);
		clearVoiceInputs(pv);
		pv->mKey = -1;
		pv->mZone = zone;
		...
		VoiceControlRamps* pg = mVoiceGlides.back().get();
		for(int i=0; i<kNumVoiceControlRamps; ++i)
		{
			pg->at(i).mpOutput = &(pv->mInputs[i]);
		}
	}
	else
	{
