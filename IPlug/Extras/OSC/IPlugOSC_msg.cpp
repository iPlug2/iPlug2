...
const void* OscMessageRead::GetIndexedArg(int idx, char* typeOut) const
{
  ...
  if (ptr >= m_type_end || valptr >= m_arg_end)
    return 0;
...
const int* OscMessageRead::PopIntArg(bool peek)
{
  ...
  if (*m_type_ptr != 'i')
    return 0;
  if (m_arg_ptr + sizeof(int) > m_arg_end)
    return 0;
...
const float* OscMessageRead::PopFloatArg(bool peek)
{
  ...
  if (*m_type_ptr != 'f')
    return 0;
  if (m_arg_ptr + sizeof(float) > m_arg_end)
    return 0;
...
const char* OscMessageRead::PopStringArg(bool peek)
{
  ...
  if (*m_type_ptr != 's')
    return 0;
  if (m_arg_ptr + strlen(m_arg_ptr) > m_arg_end)
    return 0;
...
bool OscMessageWrite::PushWord(const char* word)
{
  ...
  if (m_msg_ptr + len + 1 >= m_msg + sizeof(m_msg))
    return false;
...
{
  if (m_type_ptr + 1 > m_types + sizeof(m_types))
    return false;
  if (m_arg_ptr + sizeof(int) > m_args + sizeof(m_args))
    return false;
...
bool OscMessageWrite::PushFloatArg(float val)
{
  if (m_type_ptr + 1 > m_types + sizeof(m_types))
    return false;
  if (m_arg_ptr + sizeof(float) > m_args + sizeof(m_args))
    return false;
...
bool OscMessageWrite::PushStringArg(const char* val)
{
  ...
  if (m_type_ptr + 1 > m_types + sizeof(m_types))
    return false;
  if (m_arg_ptr + padlen > m_args + sizeof(m_args))
    return false;
