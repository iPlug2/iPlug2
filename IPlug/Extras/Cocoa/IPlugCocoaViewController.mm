...
{
  ((CocoaEditorDelegate*)editorDelegate)
    ->SendArbitraryMsgFromUI(
      static_cast<int>(msgTag), static_cast<int>(ctrlTag), static_cast<int>([msg length]), [msg bytes]);
}
...
- (NSString*)getParameterName:(NSInteger)paramIdx
{
  NSString* name = [NSString
    stringWithUTF8String:((CocoaEditorDelegate*)editorDelegate)->GetParam(static_cast<int>(paramIdx))->GetName()];
  return name;
}
...
- (NSString*)getParameterLabel:(NSInteger)paramIdx
{
  NSString* label = [NSString
    stringWithUTF8String:((CocoaEditorDelegate*)editorDelegate)->GetParam(static_cast<int>(paramIdx))->GetLabel()];
  return label;
}
...
- (NSString*)getParameterGroup:(NSInteger)paramIdx
{
  NSString* group = [NSString
    stringWithUTF8String:((CocoaEditorDelegate*)editorDelegate)->GetParam(static_cast<int>(paramIdx))->GetGroup()];
  return group;
}
