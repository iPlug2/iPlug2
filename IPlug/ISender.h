...
struct ISenderData
{
  ...
  ISenderData() { memset(vals.data(), 0, sizeof(vals)); }
...
public:
  ...
  void PushData(const ISenderData<MAXNC, T>& d) { mQueue.Push(d); }
  ...
  /** This variation can be used if you need to supply multiple controls with the same ISenderData, overrideing the tags
   in the data packet
...
public:
  ...
  /** Queue peaks from sample buffers into the sender This can be called on the realtime audio thread.
   ...
   @param ctrlTag a control tag to indicate which control to send the buffers to. Note: if you don't supply the control
   tag here, you must use TransmitDataToControlsWithTags() and specify one or more tags there
...
/** IPeakAvgSender is a utility class which can be used to defer peak & avg/RMS data from sample buffers for sending to
 * the GUI It also features an envelope follower to control meter ballistics
 */
template <int MAXNC = 1, int QUEUE_SIZE = 64>
...
public:
  ...
  IPeakAvgSender(double minThresholdDb = -90.0, bool rmsMode = true, float windowSizeMs = 5.0f,
...
public:
  ...
  /** Queue sample buffers into the sender, checking the data is over the required threshold. This can be called on the
