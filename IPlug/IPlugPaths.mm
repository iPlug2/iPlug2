...
void HostPath(WDL_String& path, const char* bundleID)
{
  @autoreleasepool
  {
    NSBundle* pBundle = [NSBundle bundleWithIdentifier:[NSString stringWithCString:bundleID
...
void VST3PresetsPath(WDL_String& path, const char* mfrName, const char* pluginName, bool isSystem)
{
  ...
  path.SetFormatted(
    MAX_MACOS_PATH_LEN, "%s/Audio/Presets/%s/%s/", [pApplicationSupportDirectory UTF8String], mfrName, pluginName);
}
...
bool GetResourcePathFromBundle(const char* fileName, const char* searchExt, WDL_String& fullPath, const char* bundleID)
{
  @autoreleasepool
  {
    ...
    NSString* pFile = [[NSString stringWithCString:fileName
                                          encoding:NSUTF8StringEncoding] stringByDeletingPathExtension];
    ...
    if (isCorrectType && pBundle && pFile)
    {
      ...
      if (isAppExtension)
        pBasePath = [[[[pBundle bundlePath] stringByDeletingLastPathComponent] stringByDeletingLastPathComponent]
          stringByAppendingString:@"/Resources/"];
      else
        pBasePath = [[pBundle resourcePath] stringByAppendingString:@"/"];

      NSString* pPath =
        [[[pBasePath stringByAppendingString:pFile] stringByAppendingString:@"."] stringByAppendingString:pExt];
...
bool GetResourcePathFromSharedLocation(const char* fileName, const char* searchExt, WDL_String& fullPath,
                                       const char* subfolder)
{
  @autoreleasepool
  {
    ...
    NSString* pExt = [NSString stringWithCString:searchExt encoding:NSUTF8StringEncoding];
    NSString* pFile = [[[NSString stringWithCString:fileName encoding:NSUTF8StringEncoding] lastPathComponent]
    ...
    if (isCorrectType && pFile)
    {
      ...
      if (subfolder)
      {
        ...
        NSString* pPath =
          [[[[pMusicLocation stringByAppendingPathComponent:pPluginName] stringByAppendingPathComponent:@"Resources"]
            stringByAppendingPathComponent:pFile] stringByAppendingPathExtension:pExt];
                                 ...
                                 const char* sharedResourcesSubPath)
{
  if (CStringHasContents(name))
  {
    ...
    // finally check name, which might be a full path - if the plug-in is trying to load a resource at runtime (e.g.
    // skin-able UI)
...
void BundleResourcePath(WDL_String& path, PluginIDType bundleID)
{
  ...
  if (IsOOPAuv3AppExtension())
    pBundle = [NSBundle
      bundleWithPath:[[[pBundle bundlePath] stringByDeletingLastPathComponent] stringByDeletingLastPathComponent]];
...
bool GetResourcePathFromBundle(const char* fileName, const char* searchExt, WDL_String& fullPath, const char* bundleID)
{
  @autoreleasepool
  {
    ...
    if (isCorrectType && pBundle && pFile)
    {
      ...
      NSString* pPath = [[[[pRootPath stringByAppendingString:@"/"] stringByAppendingString:pFile]
        stringByAppendingString:@"."] stringByAppendingString:pExt];
