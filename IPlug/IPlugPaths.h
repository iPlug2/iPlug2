...
/** Get the path to the plug-in bundle resource path. On macOS and iOS if this is called in an AUv3 app extension it
 * will return the bundle of the parent app iOS bundles are flat, so the path is just to the .app where as macOS bundles
 * contain a resources subfolder On Windows this is only useful for VST3 plug-ins which have a "bundle" with a resource
 * path since v3.6
 * @param path WDL_String reference where the path will be put on success or empty string on failure
...
/** @param path WDL_String reference where the path will be put on success or empty string on failure
 * @param isSystem Set \c true if you want to obtain the system-wide path, otherwise the path will be in the user's home
 * folder */
extern void AppSupportPath(WDL_String& path, bool isSystem = false);

/** @param path WDL_String reference where the path will be put on success or empty string on failure
 * @param mfrName CString to specify the manufacturer name, which will be the top level folder for .vstpreset files for
 * this manufacturer's product
 * @param pluginName CString to specify the plug-in name, which will be the sub folder (beneath mfrName) in which the
 * .vstpreset files are located
 * @param isSystem Set \c true if you want to obtain the system-wide path, otherwise the path will be in the user's home
 * folder */
...
/** Find the absolute path of a resource based on it's file name (e.g. “background.png”) and type (e.g. “png”), or in
 * the case of windows, confirm the existence of a particular resource in the binary. If it fails to find the resource
 * with the binary it will test the fileNameOrResID argument as an absolute path, to see if the file exists in that
 * place. On macOS resources are usually included inside the bundle resources folder. On Windows resources are usually
 * baked into the binary via the resource compiler. In this case the fileName argument is the resource id to look for.
 ...
 * here (for example if you want to reduce the disk footprint of multiple bundles, such as when you have multiple
 * plug-in formats installed).
 *
 * @param fileNameOrResID The filename or resourceID including extension. If no resource is found this argument is
 * tested as an absolute path.
 * @param type The resource type (file extension) in lower or upper case, e.g. ttf or TTF for a truetype font
 * @param result WDL_String which will either contain the full path to the resource on disk, or the full Windows
 * resourceID on success
 * @return \c true on success */
extern EResourceLocation LocateResource(const char* fileNameOrResID, const char* type, WDL_String& result,
 ...
 * @return const void pointer to the data if successfull on windows. Returns nullptr if unsuccessfull or on platforms
 * other than windows */
