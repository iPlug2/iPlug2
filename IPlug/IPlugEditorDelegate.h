...
/** This pure virtual interface delegates communication in both directions between a UI editor and something else (which
...
public:
  ...
  virtual ~IEditorDelegate() { mParams.Empty(true); }
...
#pragma mark - Methods you may want to override...
   ...
   * Like OnParamChange, OnParamChangeUI will be called when a parameter changes. However, whereas OnParamChange may be
   * called on the audio thread and should be used to update DSP state, OnParamChangeUI is always called on the
   ...
   * updating parameters has an effect or if you need to avoid multiple settings of linked parameters). This must update
   * both DSP and UI. The default implementation calls OnParamChange() and OnParamChangeUI() for each parameter.
  ...
  /** This is called by API classes after restoring state and by IPluginBase::RestorePreset(). Typically used to update
   * user interface, where multiple parameter values have changed. If you need to do something when state is restored
   * you can override it If you override this method you should call this parent, or implement the same functionality in
   * order to get controls to update, when state is restored. */
  virtual void OnRestoreState() { SendCurrentParamValuesFromDelegate(); };

  /** KeyDown handler, in order to get keystrokes from certain hosts/plugin formats that send key press messages through
   * the plug-in API, rather than the view
...
#pragma mark - Methods for sending values TO the user interface
   ...
   * WARNING: should not be called on the realtime audio thread.
   * This method can be used to send opaque data from a class implementing IEditorDelegate to a specific control in the
   * user interface. The message can be handled in the destination control via IControl::OnMsgFromDelegate
   ...
   * @param dataSize The size in bytes of the data payload pointed to by pData. Note: if this is nonzero, pData must be
   * valid.
   ...
   * OnParamChangeUI gets triggered In IGraphics plug-ins, this will update any IControls that have their mParamIdx set
   * > -1
...
#pragma mark - Methods for sending values FROM the user interface
  // The following methods are called from the user interface in order to set or query values of parameters in the class
  // implementing IEditorDelegate
  ...
  /** If the editor changes UI dimensions, e.g. from clicking a button to choose a size or dragging a corner resizer, it
   * needs to call into the plug-in API to resize the window in the plugin returns a bool to indicate whether the DAW or
   * plugin class has resized the host window */
   ...
   * keyboard. Eventually the MIDI message can be handled in IPlugProcessor::ProcessMidiMsg(), from where it can be used
   * to trigger sound and or forwarded to the API's MIDI output.
   ...
   * valid.
   * @param pData Ptr to the opaque data payload for the message */
  virtual void SendArbitraryMsgFromUI(int msgTag, int ctrlTag = kNoTag, int dataSize = 0,
...
private:
  /** A list of IParam objects. This list is populated in the delegate constructor depending on the number of parameters
   * passed as an argument to MakeConfig() in the plug-in class implementation constructor */
