...
/** This virtual interface delegates communication in both directions between a UI editor and something else (which
 * is usually a plug-in) It is also the class that owns parameter objects (for historical reasons) - although it's not
 * necessary to allocate them
 *
 *  This is the lowest level base class in iPlug 2 that facilitates distributing editor and DSP parts for plug-in
 * formats that need that, but also allowing non-distributed plug-ins to use the same API. In distributed plug-in
 * architectures certain methods will be overridden in order to pipe messages to various places, using whatever
 * mechanism that plug-in format requires. In this case, there are actually two classes that implement the
 * IEditorDelegate interface, but only one which is directly connected to the user interface (IGraphics etc.), the other
 * being connected to a class inheriting IPlugAPIBase that deals with processing audio, see for example
 * IPlugVST3Processor
 *
 *  Note on method names:
 *  - "FromUI" in a method name, means that that method is called by something in the UI i.e. when the user interacts
 * with a control.
 *  - "FromDelegate" in a method name means that method is called from a class that implements the IEditorDelegate
 * interface, which is usually your plug-in base class, but may not be in the case of an isolated editor class, or if
 * you are using IGraphics without IPlug, and your IEditorDelegate is not a plug-in
 *
 *  NOTES:
 ...
 *  A parameter OBJECT is also referred to as a "param", in method names such as IEditorDelegate::GetParam(int paramIdx)
 * and IControl::GetParam(). */
class IEditorDelegate
{
 public:
	...
	/** If you are not using IGraphics, you can implement this method to attach to the native parent view e.g. NSView,
	 * UIView, HWND. Defer calling OnUIOpen() if necessary. */
	...
	/** If you are not using IGraphics you can if you need to free resources etc when the window closes. Call base
	 * implementation. */
...
#pragma mark - Methods you may want to override...
	/** Override this method to do something before the UI is opened. You must call the base implementation to make sure
	 * controls linked to parameters get updated correctly. */
	 ...
	 * Like OnParamChange, OnParamChangeUI will be called when a parameter changes. However, whereas OnParamChange may
	 * be called on the audio thread and should be used to update DSP state, OnParamChangeUI is always called on the
	 ...
	 * updating parameters has an effect or if you need to avoid multiple settings of linked parameters). This must
	 * update both DSP and UI. The default implementation calls OnParamChange() and OnParamChangeUI() for each
	 * parameter.
	...
	/** This is called by API classes after restoring state and by IPluginBase::RestorePreset(). Typically used to
	 * update user interface, where multiple parameter values have changed. If you need to do something when state is
	 * restored you can override it If you override this method you should call this parent, or implement the same
	 * functionality in order to get controls to update, when state is restored. */
	virtual void OnRestoreState()
	{
		SendCurrentParamValuesFromDelegate();
	};

	/** KeyDown handler, in order to get keystrokes from certain hosts/plugin formats that send key press messages
	 * through the plug-in API, rather than the view
...
#pragma mark - Methods for sending values TO the user interface
	 ...
	 *  This is important when modifying groups of parameters, restoring state and opening the UI, in order to update it
	 ...
	 * In IGraphics plug-ins, this method is used to update controls in the user interface from a class implementing
	 * IEditorDelegate, when the control is not linked to a parameter. A typical use case would be a meter control. In
	 * OnIdle() your plug-in would call this method to update the IControl's value.
	 ...
	 * WARNING: should not be called on the realtime audio thread.
	 * This method can be used to send opaque data from a class implementing IEditorDelegate to a specific control in
	 * the user interface. The message can be handled in the destination control via IControl::OnMsgFromDelegate
	 ...
	 * @param dataSize The size in bytes of the data payload pointed to by pData. Note: if this is nonzero, pData must
	 * be valid.
	 ...
	 * WARNING: should not be called on the realtime audio thread.
	 * This method can be used to send opaque data from a class implementing IEditorDelegate to the IEditorDelegate
	 * connected to the user interface The message can be handled at the destination via IEditorDelegate::OnMessage()
	 * @param msgTag A unique tag to identify the message
	 * @param dataSize The size in bytes of the data payload pointed to by pData. Note: if this is nonzero, pData must
	 * be valid.
	 * @param pData Ptr to the opaque data payload for the message */
	...
	{
		OnMessage(msgTag, kNoTag, dataSize, pData);
	}

	/** SendMidiMsgFromDelegate (Abbreviation: SMMFD)
	 * WARNING: should not be called on the realtime audio thread.
	 * This method can be used to send regular MIDI data from the class implementing IEditorDelegate to the user
	 * interface The message can be handled at the destination via IEditorDelegate::OnMidiMsgUI()
	 ...
	 * WARNING: should not be called on the realtime audio thread.
	 * This method is called by the class implementing the delegate interface (not the plug-in API class) in order to
	 * update the user interface with the new parameter values, typically after automation. The similarly named
	 * IPlugAPIBase::SendParameterValueFromAPI() should take care of queueing and deferring, if there is no main thread
	 * notification from the API If you override this method you should call the base class implementation to make sure
	 * OnParamChangeUI gets triggered In IGraphics plug-ins, this will update any IControls that have their mParamIdx
	 * set > -1
	...
	{
		OnParamChangeUI(paramIdx, EParamSource::kDelegate);
	}  // TODO: normalised?

#pragma mark - Methods for sending values FROM the user interface
	// The following methods are called from the user interface in order to set or query values of parameters in the
	// class implementing IEditorDelegate

	/** When modifying a range of parameters in the editor, it can be necessary to broadcast that fact via the host, for
	 * instance in a distributed plug-in. You can use it if you restore a preset using a custom preset mechanism. */
	...
	/** SPVFUI Called by the UI during a parameter change gesture, in order to notify the host of the new value (via a
	 * call in the API class) If you override this method you should call the base class implementation to make sure
	 * OnParamChangeUI gets triggered
	 * @param paramIdx The index of the parameter that is changing value
	...
	/** If the editor changes UI dimensions, e.g. from clicking a button to choose a size or dragging a corner resizer,
	 * it needs to call into the plug-in API to resize the window in the plugin returns a bool to indicate whether the
	 * DAW or plugin class has resized the host window */
	 ...
	 * This method should be used  when  sending a MIDI message from the UI. For example clicking on a key in a virtual
	 * keyboard. Eventually the MIDI message can be handled in IPlugProcessor::ProcessMidiMsg(), from where it can be
	 * used to trigger sound and or forwarded to the API's MIDI output.
	 ...
	 * Since it is extremely unlikely that you would want to use Sysex to communicate between editor and processor \todo
	 * is this correct?
	 ...
	 * be valid.
	 * @param pData Ptr to the opaque data payload for the message */
	virtual void SendArbitraryMsgFromUI(int msgTag,
										int ctrlTag       = kNoTag,
										int dataSize      = 0,
...
#pragma mark - Editor resizing
	...
	{
		if (math::ClampEval(w, mMinWidth, mMaxWidth) && math::ClampEval(h, mMinHeight, mMaxHeight))
		{
			return true;
		}
		else
		{
			w = math::Clamp(w, mMinWidth, mMaxWidth);
			h = math::Clamp(h, mMinHeight, mMaxHeight);
			return false;
		}
	}
 ...
 protected:
	/** A list of IParam objects. This list is populated in the delegate constructor depending on the number of
	 * parameters passed as an argument to MakeConfig() in the plug-in class implementation constructor */
	WDL_PtrList<IParam> mParams;

 private:
	/** The width of the plug-in editor in pixels. Can be updated by resizing, exists here for persistance, even if UI
	 * doesn't exist. */
	int mEditorWidth = 0;
	/** The height of the plug-in editor in pixels. Can be updated by resizing, exists here for persistance, even if UI
	 * doesn't exist */
	int mEditorHeight = 0;
