...
, mRec(pRec)
{
  mTimer = std::unique_ptr<Timer>(
    Timer::Create(std::bind(&ReaperExtBase::OnTimer, this, std::placeholders::_1), IDLE_TIMER_RATE));
}
...
void ReaperExtBase::RegisterAction(const char* actionName, std::function<void()> func, bool addMenuItem,
                                   int* pToggle /*, IKeyPress keyCmd*/)
{
  ReaperAction action;
...
{
  std::vector<ReaperAction>::iterator it =
    std::find_if(gActions.begin(), gActions.end(), [&](const auto& e) { return e.accel.accel.cmd == command; });
...
WDL_DLGRET ReaperExtBase::MainDlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (uMsg)
  {
    ...
    case WM_DESTROY: gHWND = NULL; return 0;
    case WM_CLOSE:
      gPlug->CloseWindow();
      DestroyWindow(hwnd);
      return 0;
