...
void Trace(const char* funcName, int line, const char* format, ...)
{
	...
	if (sTrace++ < MAX_LOG_LINES)
	{
		...
		if (strstr(funcName, "rocess") ||
			strstr(
				funcName,
				"ender"))  // These are not typos! by excluding the first character, we can use TRACE in methods called ProcessXXX or process etc.
		{
			...
			else if (sProcessCount == MAX_PROCESS_TRACE_COUNT)
			{
				fprintf(sLogFile.mFP,
						"**************** DISABLING PROCESS TRACING AFTER %d HITS ****************\n\n",
						sProcessCount);
				fflush(sLogFile.mFP);
				return;
			}
		...
		#endif
		{
			...
			{
				fprintf(sLogFile.mFP,
						"**************** DISABLING IDLE/MOUSEOVER TRACING AFTER %d HITS ****************\n",
						sIdleCount);
				fflush(sLogFile.mFP);
				return;
			}
...
static const char* AUSelectStr(int select)
{
	switch (select)
	{
		...
		case kAudioUnitComplexRenderSelect: return "kAudioUnitComplexRenderSelect";
		case kAudioUnitProcessSelect: return "kAudioUnitProcessSelect";
		case kAudioUnitProcessMultipleSelect: return "kAudioUnitProcessMultipleSelect";
		case kAudioUnitRange: return "kAudioUnitRange";
...
static const char* AUPropertyStr(int propID)
{
	switch (propID)
	{
		...
		case kAudioUnitProperty_ParameterHistoryInfo: return "kAudioUnitProperty_FrequencyResponse";
		...
		case kAudioUnitProperty_SupportsMPE: return "kAudioUnitProperty_SupportsMPE";
		#endif
		default: return "unknown";
	}
}
