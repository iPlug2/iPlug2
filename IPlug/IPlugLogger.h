...
void Trace(const char* funcName, int line, const char* format, ...)
{
  ...
  if (sTrace++ < MAX_LOG_LINES)
  {
    ...
    if (strstr(funcName, "rocess")
        || strstr(funcName, "ender")) // These are not typos! by excluding the first character, we can use TRACE in
                                      // methods called ProcessXXX or process etc.
    {
      ...
      {
        fprintf(
          sLogFile.mFP, "**************** DISABLING PROCESS TRACING AFTER %d HITS ****************\n\n", sProcessCount);
        fflush(sLogFile.mFP);
        return;
      }
    }
