...
static void GetModulePath(HMODULE hModule, WDL_String& path)
{
	...
	if (GetModuleFileName(hModule, pathCStr, MAX_WIN32_PATH_LEN))
	{
		...
		if (s >= 0 && s + 1 < static_cast<int>(strlen(pathCStr)))
		{
...
static BOOL EnumResNameProc(HANDLE module, LPCTSTR type, LPTSTR name, LONG_PTR param)
{
	...
	else
	{
		...
		if (search != 0 && name != 0)
		{
			...
			if (strcmp(strlwr(search->Get()), strippedName.Get()) ==
				0)  // if we are looking for a resource with this name
			{
...
EResourceLocation LocateResource(
	const char* name, const char* type, WDL_String& result, const char*, void* pHInstance, const char*)
{
...
EResourceLocation LocateResource(
	const char* name, const char* type, WDL_String& result, const char*, void*, const char*)
{
	if (CStringHasContents(name))
	{
