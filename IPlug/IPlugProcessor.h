...
#pragma mark - Methods you implement in your plug-in class - you do not call these methods
   ...
   * THIS METHOD IS CALLED BY THE HIGH PRIORITY AUDIO THREAD - You should be careful not to do any unbounded, blocking
   * operations such as file I/O which could cause audio dropouts
   ...
   * You can use IMidiQueue in combination with this method in order to queue the message and process at the appropriate
   * time in ProcessBlock() THIS METHOD IS CALLED BY THE HIGH PRIORITY AUDIO THREAD - You should be careful not to do
   * any unbounded, blocking operations such as file I/O which could cause audio dropouts
   * @param msg The incoming midi message (includes a timestamp to indicate the offset in the forthcoming block of audio
   * to be processed in ProcessBlock()) */
  virtual void ProcessMidiMsg(const IMidiMsg& msg);

  /** Override this method to handle incoming MIDI System Exclusive (SysEx) messages. The method is called prior to
   * ProcessBlock(). THIS METHOD IS CALLED BY THE HIGH PRIORITY AUDIO THREAD - You should be careful not to do any
   * unbounded, blocking operations such as file I/O which could cause audio dropouts */
  virtual void ProcessSysEx(ISysEx& msg) {}

  /** Override this method in your plug-in class to do something prior to playback etc. (e.g.clear buffers, update
   * internal DSP with the latest sample rate) */
  virtual void OnReset() { TRACE }

  /** Override OnActivate() which should be called by the API class when a plug-in is "switched on" by the host on a
   * track when the channel count is known. This may not work reliably because different hosts have different
   * interpretations of "activate". Unlike OnReset() which called when the transport is reset or the sample rate changes
   * OnActivate() is a good place to handle change of I/O connections.
...
#pragma mark - Methods you can call - some of which have custom implementations in the API classes, some implemented in IPlugProcessor.cpp
  ...
  /** Send a single MIDI System Exclusive (SysEx) message // TODO: info about what thread should this be called on or
   * not called on!
  ...
  /** @return The tail size in samples (useful for reverberation plug-ins, that may need to decay after the transport
   * stops or an audio item ends) */
...
#pragma mark -
  ...
  /** Used to determine the maximum number of input or output buses based on what was specified in the channel I/O
   * config string
   * @param direction Return input or output bus count
   * @param pConfigIdxWithTheMostBuses Optional ptr to report the index of the config with the max bus count, if
   * multiple configs have the same bus count, this should report the one with the higher channel count
  ...
  {
    return mIOConfigs.Get(0)->ContainsWildcard(direction);
  } // \todo only supports a single I/O config
  ...
  /** @return \c true if this plug-in has a side-chain input, which may not necessarily be active in the current I/O
   * config */
  bool HasSidechainInput() const { return MaxNBuses(ERoute::kInput) > 1; }

  /** This is called by IPlugVST in order to limit a plug-in to stereo I/O for certain picky hosts \todo may no longer
   * be relevant*/
   ...
   * * For example a 4 channel plug-in that deals with FuMa BFormat first order ambisonic material, might label these
   channels "W", "X", "Y", "Z", rather than the default "input 1", "input 2", "input 3", "input 4"
  ...
  /** Call this method if you need to update the tail size at runtime, for example if the decay time of your reverb
   * effect changes Some apis have special interpretations of certain numbers. For VST3 set to 0xffffffff for infinite
   * tail, or 0 for none (default) For VST2 setting to 1 means no tail
   ...
   * A hypen character \c(-) deliminates input-output. Supports multiple buses, which are indicated using a period \c(.)
   * character. For instance plug-in that supports mono input and mono output with a mono side-chain input could have a
   * channel io string of "1.1-1". A drum synthesiser with four stereo output busses could be configured with a io
   * string of "0-2.2.2.2";
  ...
  static int ParseChannelIOStr(const char* IOStr, WDL_PtrList<IOConfig>& channelIOList, int& totalNInChans,
...
protected
: // these members are protected because they need to be access by the API classes, and don't want a setter/getter
