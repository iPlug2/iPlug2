...
extern uint64_t GetAPIBusTypeForChannelIOConfig(int configIdx,
												iplug::ERoute dir,
												int busIdx,
 ...
 public:
	...
	void Initialize(T* pPlug)
	{
		...
		// Add the maximum number of input buses in any ioconfig. channel count/API Bus type will be changed later if nessecary
		{
			...
			{
				uint64_t busType =
					GetAPIBusTypeForChannelIOConfig(configWithMostInputBuses, ERoute::kInput, busIdx, pConfig);

				int flags = busIdx == 0 ? flags = Steinberg::Vst::BusInfo::BusFlags::kDefaultActive : flags = 0;
				WDL_String busName;
				GetBusName(ERoute::kInput, busIdx, nIn, busName);
				Steinberg::UString(tmpStringBuf, 128).fromAscii(busName.Get(), 128);

				pPlug->addAudioInput(tmpStringBuf,
									 busType,
									 busIdx > 0 ? Steinberg::Vst::BusTypes::kAux : Steinberg::Vst::BusTypes::kMain,
									 flags);
			}
		}
		...
		// Add the maximum number of output buses in any ioconfig. channel count/API Bus type will be changed later if nessecary
		{
			...
			{
				uint64_t busType =
					GetAPIBusTypeForChannelIOConfig(configWithMostOutputBuses, ERoute::kOutput, busIdx, pConfig);
				int flags = busIdx == 0 ? flags = Steinberg::Vst::BusInfo::BusFlags::kDefaultActive : flags = 0;
				WDL_String busName;
				GetBusName(ERoute::kOutput, busIdx, nOut, busName);
				Steinberg::UString(tmpStringBuf, 128).fromAscii(busName.Get(), 128);

				pPlug->addAudioOutput(tmpStringBuf,
									  busType,
									  busIdx > 0 ? Steinberg::Vst::BusTypes::kAux : Steinberg::Vst::BusTypes::kMain,
									  flags);
			}
		...
		if (IsMidiEffect() && maxNOutBuses == 0)
		{
			...
			pPlug->addAudioOutput(
				tmpStringBuf, Steinberg::Vst::SpeakerArr::kEmpty, Steinberg::Vst::BusTypes::kMain, flags);
		}
	...
	// MIDI Processing
	void ProcessMidiIn(Steinberg::Vst::IEventList* pEventList,
					   IPlugQueue<IMidiMsg>& editorQueue,
					   IPlugQueue<IMidiMsg>& processorQueue);

	void ProcessMidiOut(IPlugQueue<SysExData>& sysExQueue,
						SysExData& sysExBuf,
	...
	template <class T>
	bool SetBusArrangements(T* pPlug,
							...
							Steinberg::int32 numOutBuses)
	{
		...
		{
			inputBuses.push_back(Steinberg::Vst::SpeakerArr::getChannelCount(pInputBusArrangements[i]));
		}

		for (int i = 0; i < numOutBuses; i++)
		{
			outputBuses.push_back(Steinberg::Vst::SpeakerArr::getChannelCount(pOutputBusArrangements[i]));
		}
		...
		if (matchingIdx > -1)
		{
			...
			for (auto inBusIdx = 0; inBusIdx < nIn; inBusIdx++)
			{
				...
				if (pBus && Steinberg::Vst::SpeakerArr::getChannelCount(pBus->getArrangement()) != nInputsRequired)
				{
					pPlug->removeAudioInputBus(pBus);
					const int flags = inBusIdx == 0 ? Steinberg::Vst::BusInfo::BusFlags::kDefaultActive : 0;
					Steinberg::Vst::SpeakerArrangement arr =
						GetAPIBusTypeForChannelIOConfig(matchingIdx, ERoute::kInput, inBusIdx, pConfig);

					WDL_String busName;
					GetBusName(ERoute::kInput, inBusIdx, nIn, busName);
					Steinberg::UString(tmpStringBuf, 128).fromAscii(busName.Get(), 128);
					pPlug->addAudioInput(tmpStringBuf, arr, (Steinberg::Vst::BusTypes) inBusIdx > 0, flags);
				}
			}
			...
			for (auto outBusIdx = 0; outBusIdx < nOut; outBusIdx++)
			{
				...
				// TODO: also check that existing SpeakerArr is correct
				if (pBus && Steinberg::Vst::SpeakerArr::getChannelCount(pBus->getArrangement()) != nOutputsRequired)
				{
					pPlug->removeAudioOutputBus(pBus);
					int flags = outBusIdx == 0 ? Steinberg::Vst::BusInfo::BusFlags::kDefaultActive : 0;
					Steinberg::Vst::SpeakerArrangement arr =
						GetAPIBusTypeForChannelIOConfig(matchingIdx, ERoute::kOutput, outBusIdx, pConfig);

					WDL_String busName;
					GetBusName(ERoute::kOutput, outBusIdx, nOut, busName);
					Steinberg::UString(tmpStringBuf, 128).fromAscii(busName.Get(), 128);
					pPlug->addAudioOutput(tmpStringBuf, arr, (Steinberg::Vst::BusTypes) outBusIdx > 0, flags);
				}
			}
	...
	void AttachBuffers(ERoute direction,
					   int idx,
					   int n,
