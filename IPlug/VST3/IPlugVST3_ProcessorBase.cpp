...
void IPlugVST3ProcessorBase::ProcessMidiIn(IEventList* pEventList, IPlugQueue<IMidiMsg>& editorQueue,
    ...
    for (int32 i = 0; i < numEvent; i++)
    {
      ...
      if (pEventList->getEvent(i, event) == kResultOk)
      {
        switch (event.type)
        {
          ...
          {
            msg.MakeNoteOnMsg(
              event.noteOn.pitch, event.noteOn.velocity * 127, event.sampleOffset, event.noteOn.channel);
            ProcessMidiMsg(msg);
          ...
          {
            msg.MakePolyATMsg(event.polyPressure.pitch, event.polyPressure.pressure * 127., event.sampleOffset,
...
void IPlugVST3ProcessorBase::ProcessMidiOut(IPlugQueue<SysExData>& sysExQueue, SysExData& sysExBuf,
                                            IEventList* pOutputEvents, int32 numSamples)
{
...
void IPlugVST3ProcessorBase::AttachBuffers(ERoute direction, int idx, int n, AudioBusBuffers& pBus, int nFrames,
                                           int32 sampleSize)
{
...
bool IPlugVST3ProcessorBase::SetupProcessing(const ProcessSetup& setup, ProcessSetup& storedSetup)
{
  ...
  IPlugProcessor::SetBlockSize(
    setup.maxSamplesPerBlock); // TODO: should IPlugVST3Processor call SetBlockSize in construct unlike other APIs?
...
void IPlugVST3ProcessorBase::ProcessAudio(ProcessData& data, ProcessSetup& setup, const BusList& ins,
                                          const BusList& outs)
{
  ...
  if (sampleSize == kSample32 || sampleSize == kSample64)
  {
    if (data.numInputs)
    {
      ...
      if (ins.size() > 1)
      {
        ...
        if (mSidechainActive)
          AttachBuffers(ERoute::kInput, mMaxNChansForMainInputBus, data.inputs[1].numChannels, data.inputs[1],
                        data.numSamples, sampleSize);
      }
      else
      {
        SetChannelConnections(ERoute::kInput, 0, MaxNChannels(ERoute::kInput), false);
    ...
    for (int outBus = 0, chanOffset = 0; outBus < data.numOutputs; outBus++)
    {
      ...
      SetChannelConnections(
        ERoute::kOutput, chanOffset + busChannels, MaxNChannels(ERoute::kOutput) - (chanOffset + busChannels), false);
...
void IPlugVST3ProcessorBase::Process(ProcessData& data, ProcessSetup& setup, const BusList& ins, const BusList& outs,
                                     IPlugQueue<IMidiMsg>& fromEditor, IPlugQueue<IMidiMsg>& fromProcessor,
                                     IPlugQueue<SysExData>& sysExFromEditor, SysExData& sysExBuf)
{
