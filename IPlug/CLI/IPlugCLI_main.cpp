...
bool saveParams(IPlugCLI* pPlugin, const char* filename)
{
  FILE* f = fopen(filename, "w");
  if (!f)
  {
...
bool loadParams(IPlugCLI* pPlugin, const char* filename)
{
  ...
  while ((pos = content.find("\"index\"", pos)) != std::string::npos)
  {
    ...
    if (colonPos == std::string::npos || colonPos > objEnd) break;

    int idx = atoi(content.c_str() + colonPos + 1);

    size_t valuePos = content.find("\"value\"", colonPos);
    if (valuePos == std::string::npos || valuePos > objEnd) { pos = objEnd; continue; }

    size_t valueColonPos = content.find(':', valuePos);
    if (valueColonPos == std::string::npos || valueColonPos > objEnd) { pos = objEnd; continue; }
...
int main(int argc, char* argv[])
{
  ...
  while (args.hasNext())
  {
    ...
    else if (args.match("--process"))
    {
      ...
      for (int c = 0; c < nOutCh; c++)
      {
        outputChannels[c].resize(nFrames, 0.0);
        outputs[c] = outputChannels[c].data();
      }
    ...
    else if (args.match("--impulse"))
    {
      ...
      plugin->SetupProcessing(sampleRate, blockSize);
      plugin->ProcessImpulse(length, outputChannels);
    ...
    else if (args.match("--sine"))
    {
      ...
      plugin->ProcessSine(freq, nFrames, outputChannels);
    ...
    else if (args.match("--noise"))
    {
      ...
      plugin->SetupProcessing(sampleRate, blockSize);
      int nFrames = static_cast<int>((ms / 1000.0) * sampleRate);
      plugin->ProcessNoise(nFrames, outputChannels);
    ...
    else if (args.match("--step"))
    {
      ...
      plugin->SetupProcessing(sampleRate, blockSize);
      int nFrames = static_cast<int>((ms / 1000.0) * sampleRate);
      plugin->ProcessStep(nFrames, outputChannels);
    ...
    else if (args.match("--chirp"))
    {
      ...
      plugin->SetupProcessing(sampleRate, blockSize);
      int nFrames = static_cast<int>((ms / 1000.0) * sampleRate);
      plugin->ProcessChirp(startFreq, endFreq, nFrames, outputChannels);
    ...
    else if (args.match("--midi-cc"))
    {
      ...
      int timeMs = args.peekIsValue() ? args.getInt(0) : 0;
  ...
  if (!outputTxtFile.empty())
  {
    if (!outputChannels.empty())
      outputText(outputChannels[0], outputTxtFile.c_str());
    else
      fprintf(stderr, "Warning: --output-txt specified but no audio was processed\n");
  }

  if (!outputWavFile.empty())
  {
    if (outputChannels.empty())
    {
      fprintf(stderr, "Warning: --output specified but no audio was processed\n");
    }
    else
    {
