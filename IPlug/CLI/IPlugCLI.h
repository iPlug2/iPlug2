...
public:
   ...
   * @param output Per-channel output vectors */
  void ProcessImpulse(int responseLength, std::vector<std::vector<double>>& output);

  /** Process a sine wave input and capture output
   * @param frequency Sine wave frequency in Hz
   * @param nFrames Total frames to process
   * @param output Per-channel output vectors */
  void ProcessSine(double frequency, int nFrames, std::vector<std::vector<double>>& output);

  /** Process white noise input and capture output
   * @param nFrames Total frames to process
   * @param output Per-channel output vectors */
  void ProcessNoise(int nFrames, std::vector<std::vector<double>>& output);

  /** Process a unit step input and capture output
   * @param nFrames Total frames to process
   * @param output Per-channel output vectors */
  void ProcessStep(int nFrames, std::vector<std::vector<double>>& output);
   ...
   * @param nFrames Total frames to process
   * @param output Per-channel output vectors */
  void ProcessChirp(double startFreq, double endFreq, int nFrames, std::vector<std::vector<double>>& output);
