...
struct IChannelData
{
	...
	TOUT** mData =
		nullptr;  // If this is for an input channel, points into IPlugProcessor::mInData, if it's for an output channel points into IPlugProcessor::mOutData
...
struct IPreset
{
	...
	IPreset() { sprintf(mName, "%s", UNUSED_PRESET_NAME); }
};
