...
namespace iplug
{
	...
	class IByteChunk : private IByteGetter
	{
	  ...
	  public:
		...
		static int GetIPlugVerFromChunk(const IByteChunk& chunk, int& position)
		{
			...
			if (magicpos > position && magic == mVersionMagic)
	...
	struct IChannelData
	{
		...
		// If this is for an input channel, points into IPlugProcessor::mInData,
		// if it's for an output channel points into IPlugProcessor::mOutData
		TOUT** mData       = nullptr;
	...
	struct IOConfig
	{
		WDL_PtrList<IBusInfo>
			mBusInfo[2];  // A particular valid io config may have multiple input buses or output busses
	...
	struct IKeyPress
	{
		 ...
		 * @param c /todo
		 * @param a /todo */
		IKeyPress(const char* _utf8, int vk, bool s = false, bool c = false, bool a = false) : VK(vk), S(s), C(c), A(a)
		{
			strcpy(utf8, _utf8);
		}
