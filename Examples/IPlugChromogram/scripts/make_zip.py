...
def  main():
  ...
  installer = "\installer\IPlugChromogram Installer.exe"
   
  if demo:
    installer = "\installer\IPlugChromogram Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugChromogram manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugChromogram-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugChromogram-v" + FULLVERSIONSTR + "-win-demo.zip"
