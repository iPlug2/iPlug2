...
def  main():
  ...
  installer = "\installer\IPlugMetalCppUI Installer.exe"
   
  if demo:
    installer = "\installer\IPlugMetalCppUI Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugMetalCppUI manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugMetalCppUI-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugMetalCppUI-v" + FULLVERSIONSTR + "-win-demo.zip"
