class KnobControl extends HTMLElement {
  constructor() {
    ...
    const circleFillColor = this.getAttribute('circle-fill-color') || '#333';
    const pointerColor = this.getAttribute('pointer-color') || '#fff';
    const pointerWidth = parseFloat(this.getAttribute('pointer-width')) || 4;
    const valueArcColor = this.getAttribute('value-arc-color') || '#fff';
    const valueArcWidth = parseFloat(this.getAttribute('value-arc-width')) || 2;
