...
def  main():
  ...
  installer = "\installer\IPlugWebUI Installer.exe"
   
  if demo:
    installer = "\installer\IPlugWebUI Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugWebUI manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugWebUI-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugWebUI-v" + FULLVERSIONSTR + "-win-demo.zip"
