...
#pragma mark - Voice
  ...
  {
  public:
		IPlugInstrumentVoice() : mOsc1(0.75)
		{
			//std::cout << "new Voice: " << mInputs.size() << " control inputs.\n";
		}
		...
		void ProcessSamplesAccumulating(sample** inputs, sample** outputs, int nInputs, int nOutputs, int startIdx, int nFrames) override
		{
			...
			{
				float noise = timbreBuffer[i]*rand();
		...
		{
			 std::cout << "setting control " << controlNumber << " to value " << value << "\n";
		}
  ...
  private:
		...
		float timbreBuffer[kMaxBlockSize];
