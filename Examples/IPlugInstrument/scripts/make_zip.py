...
def  main():
  ...
  installer = "\installer\IPlugInstrument Installer.exe"
   
  if demo:
    installer = "\installer\IPlugInstrument Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugInstrument manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugInstrument-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugInstrument-v" + FULLVERSIONSTR + "-win-demo.zip"
