...
def  main():
  ...
  installer = "\installer\IPlugMidiEffect Installer.exe"
   
  if demo:
    installer = "\installer\IPlugMidiEffect Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugMidiEffect manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugMidiEffect-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugMidiEffect-v" + FULLVERSIONSTR + "-win-demo.zip"
