...
def  main():
  ...
  installer = "\installer\IPlugShaderUI Installer.exe"
   
  if demo:
    installer = "\installer\IPlugShaderUI Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugShaderUI manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugShaderUI-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugShaderUI-v" + FULLVERSIONSTR + "-win-demo.zip"
