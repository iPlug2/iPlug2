...
def  main():
  ...
  installer = "\installer\IPlugSOUL Installer.exe"
   
  if demo:
    installer = "\installer\IPlugSOUL Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugSOUL manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugSOUL-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugSOUL-v" + FULLVERSIONSTR + "-win-demo.zip"
