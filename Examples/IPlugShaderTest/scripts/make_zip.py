...
def  main():
  ...
  installer = "\installer\IPlugShaderTest Installer.exe"
   
  if demo:
    installer = "\installer\IPlugShaderTest Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugShaderTest manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugShaderTest-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugShaderTest-v" + FULLVERSIONSTR + "-win-demo.zip"
