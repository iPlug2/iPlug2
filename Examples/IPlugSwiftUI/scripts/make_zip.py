...
def  main():
  ...
  installer = "\installer\IPlugSwiftUI Installer.exe"
   
  if demo:
    installer = "\installer\IPlugSwiftUI Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugSwiftUI manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugSwiftUI-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugSwiftUI-v" + FULLVERSIONSTR + "-win-demo.zip"
