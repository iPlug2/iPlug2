...
def  main():
  ...
  installer = "\installer\IPlugFaustExample Installer.exe"
   
  if demo:
    installer = "\installer\IPlugFaustExample Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugFaustExample manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugFaustExample-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugFaustExample-v" + FULLVERSIONSTR + "-win-demo.zip"
