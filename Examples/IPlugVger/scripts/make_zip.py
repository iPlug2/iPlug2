...
def  main():
  ...
  installer = "\installer\IPlugVger Installer.exe"
   
  if demo:
    installer = "\installer\IPlugVger Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugVger manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugVger-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugVger-v" + FULLVERSIONSTR + "-win-demo.zip"
