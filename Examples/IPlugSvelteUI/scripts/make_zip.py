...
def  main():
  ...
  installer = "\installer\IPlugSvelteUI Installer.exe"
   
  if demo:
    installer = "\installer\IPlugSvelteUI Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugSvelteUI manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugSvelteUI-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugSvelteUI-v" + FULLVERSIONSTR + "-win-demo.zip"
