...
def  main():
  ...
  installer = "\installer\IPlugMinimal Installer.exe"
   
  if demo:
    installer = "\installer\IPlugMinimal Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugMinimal manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugMinimal-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugMinimal-v" + FULLVERSIONSTR + "-win-demo.zip"
