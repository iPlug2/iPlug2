...
if %1 == 1 (echo Making IPlugMinimal Windows DEMO VERSION distribution ...) else (echo Making IPlugMinimal Windows FULL VERSION distribution ...)
...
REM - msbuild IPlugMinimal-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild IPlugMinimal.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild IPlugMinimal.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugMinimal.aaxplugin\Contents\Win32\IPlugMinimal.aaxplugin --out .\build-win\aax\bin\IPlugMinimal.aaxplugin\Contents\Win32\IPlugMinimal.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugMinimal.aaxplugin\Contents\x64\IPlugMinimal.aaxplugin --out .\build-win\aax\bin\IPlugMinimal.aaxplugin\Contents\x64\IPlugMinimal.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugMinimal.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugMinimal.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "IPlugMinimal Installer" ".\installer\IPlugMinimal Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\IPlugMinimal Installer.exe" ".\installer\IPlugMinimal Demo Installer.exe"
REM -del ".\installer\IPlugMinimal Installer.exe"
REM -)
