...
def  main():
  ...
  installer = "\installer\IPlugScopeControl Installer.exe"
   
  if demo:
    installer = "\installer\IPlugScopeControl Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugScopeControl manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugScopeControl-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugScopeControl-v" + FULLVERSIONSTR + "-win-demo.zip"
