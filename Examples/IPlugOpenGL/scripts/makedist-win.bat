...
if %1 == 1 (echo Making IPlugOpenGL Windows DEMO VERSION distribution ...) else (echo Making IPlugOpenGL Windows FULL VERSION distribution ...)
...
msbuild IPlugOpenGL.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild IPlugOpenGL.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugOpenGL.aaxplugin\Contents\Win32\IPlugOpenGL.aaxplugin --out .\build-win\aax\bin\IPlugOpenGL.aaxplugin\Contents\Win32\IPlugOpenGL.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugOpenGL.aaxplugin\Contents\x64\IPlugOpenGL.aaxplugin --out .\build-win\aax\bin\IPlugOpenGL.aaxplugin\Contents\x64\IPlugOpenGL.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugOpenGL.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugOpenGL.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "IPlugOpenGL Installer" ".\installer\IPlugOpenGL Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\IPlugOpenGL Installer.exe" ".\installer\IPlugOpenGL Demo Installer.exe"
REM -del ".\installer\IPlugOpenGL Installer.exe"
REM -)
