...
def  main():
  ...
  installer = "\installer\IPlugOpenGL Installer.exe"
   
  if demo:
    installer = "\installer\IPlugOpenGL Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugOpenGL manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugOpenGL-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugOpenGL-v" + FULLVERSIONSTR + "-win-demo.zip"
