...
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mMakeGraphicsFunc = [&]() { return MakeGraphics(*this, PLUG_WIDTH, PLUG_HEIGHT, PLUG_FPS, 1.); };
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(mInputMeter = new IVPeakAvgMeterControl<4>(
                               b.FracRectVertical(0.5, true), "Inputs", meterStyle, EDirection::Horizontal,
                               {"Main L", "Main R", "SideChain L", "SideChain R"}),
                             kCtrlTagInputMeter);
    pGraphics->AttachControl(
      mOutputMeter = new IVPeakAvgMeterControl<2>(
        b.FracRectVertical(0.5, false), "Outputs", meterStyle, EDirection::Vertical, {"Main L", "Main R"}),
      kCtrlTagOutputMeter);
...
void IPlugSideChain::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
  ...
  /*
     ...
     Logic/Garageband have an long-standing bug where if no sidechain is selected, the same buffers that are sent to the
     first bus, are sent to the sidechain bus
     ...
     Imperfect hack around it here. Probably a better solution is to have an enable sidechain button in the plug-in UI,
     in addition to the host sidechain routing.
  */
