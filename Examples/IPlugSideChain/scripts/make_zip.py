...
def  main():
  ...
  installer = "\installer\IPlugSideChain Installer.exe"
   
  if demo:
    installer = "\installer\IPlugSideChain Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugSideChain manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugSideChain-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugSideChain-v" + FULLVERSIONSTR + "-win-demo.zip"
