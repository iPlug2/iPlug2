...
def  main():
  ...
  installer = "\installer\IPlugFaustDSP Installer.exe"
   
  if demo:
    installer = "\installer\IPlugFaustDSP Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugFaustDSP manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugFaustDSP-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugFaustDSP-v" + FULLVERSIONSTR + "-win-demo.zip"
