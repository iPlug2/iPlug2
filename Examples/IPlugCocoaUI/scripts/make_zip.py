...
def  main():
  ...
  installer = "\installer\IPlugCocoaUI Installer.exe"
   
  if demo:
    installer = "\installer\IPlugCocoaUI Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugCocoaUI manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugCocoaUI-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugCocoaUI-v" + FULLVERSIONSTR + "-win-demo.zip"
