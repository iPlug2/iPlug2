...
def  main():
  ...
  installer = "\installer\Testing Installer.exe"
   
  if demo:
    installer = "\installer\Testing Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Testing manual.pdf" 
  ]
  ...
  ZIPNAME = "Testing-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Testing-v" + FULLVERSIONSTR + "-win-demo.zip"
