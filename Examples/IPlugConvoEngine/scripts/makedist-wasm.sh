...
if [ "$DEBUG_BUILD" -eq 1 ]; then
  echo "BUILDING DSP WASM MODULE (AudioWorklet) - DEBUG"
else
...
if [ "$DEBUG_BUILD" -eq 1 ]; then
  emmake make --makefile $PROJECT_NAME-wasm-dsp.mk DEBUG=1
else
...
if [ "$HAS_UI" -eq 1 ]; then
  ...
  echo "============================================================"
  if [ "$DEBUG_BUILD" -eq 1 ]; then
    echo "BUILDING UI WASM MODULE (IGraphics) - DEBUG"
  else
  ...
  if [ "$DEBUG_BUILD" -eq 1 ]; then
    emmake make --makefile $PROJECT_NAME-wasm-ui.mk DEBUG=1
  else
