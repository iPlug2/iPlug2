...
if %1 == 1 (echo Making IPlugConvoEngine Windows DEMO VERSION distribution ...) else (echo Making IPlugConvoEngine Windows FULL VERSION distribution ...)
...
msbuild IPlugConvoEngine.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild IPlugConvoEngine.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugConvoEngine.aaxplugin\Contents\Win32\IPlugConvoEngine.aaxplugin --out .\build-win\aax\bin\IPlugConvoEngine.aaxplugin\Contents\Win32\IPlugConvoEngine.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugConvoEngine.aaxplugin\Contents\x64\IPlugConvoEngine.aaxplugin --out .\build-win\aax\bin\IPlugConvoEngine.aaxplugin\Contents\x64\IPlugConvoEngine.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugConvoEngine.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugConvoEngine.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "IPlugConvoEngine Installer" ".\installer\IPlugConvoEngine Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\IPlugConvoEngine Installer.exe" ".\installer\IPlugConvoEngine Demo Installer.exe"
REM -del ".\installer\IPlugConvoEngine Installer.exe"
REM -)
