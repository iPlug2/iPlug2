...
def  main():
  ...
  installer = "\installer\IPlugConvoEngine Installer.exe"
   
  if demo:
    installer = "\installer\IPlugConvoEngine Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugConvoEngine manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugConvoEngine-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugConvoEngine-v" + FULLVERSIONSTR + "-win-demo.zip"
