...
def  main():
  ...
  installer = "\installer\IPlugNanogui Installer.exe"
   
  if demo:
    installer = "\installer\IPlugNanogui Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugNanogui manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugNanogui-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugNanogui-v" + FULLVERSIONSTR + "-win-demo.zip"
