...
def  main():
  ...
  installer = "\installer\IPlugChunks Installer.exe"
   
  if demo:
    installer = "\installer\IPlugChunks Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugChunks manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugChunks-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugChunks-v" + FULLVERSIONSTR + "-win-demo.zip"
