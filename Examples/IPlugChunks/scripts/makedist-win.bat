...
if %1 == 1 (echo Making IPlugChunks Windows DEMO VERSION distribution ...) else (echo Making IPlugChunks Windows FULL VERSION distribution ...)
...
msbuild IPlugChunks.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild IPlugChunks.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugChunks.aaxplugin\Contents\Win32\IPlugChunks.aaxplugin --out .\build-win\aax\bin\IPlugChunks.aaxplugin\Contents\Win32\IPlugChunks.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugChunks.aaxplugin\Contents\x64\IPlugChunks.aaxplugin --out .\build-win\aax\bin\IPlugChunks.aaxplugin\Contents\x64\IPlugChunks.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugChunks.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugChunks.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "IPlugChunks Installer" ".\installer\IPlugChunks Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\IPlugChunks Installer.exe" ".\installer\IPlugChunks Demo Installer.exe"
REM -del ".\installer\IPlugChunks Installer.exe"
REM -)
