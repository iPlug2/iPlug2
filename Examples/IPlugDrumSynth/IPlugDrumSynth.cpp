...
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(
      new IVMeterControl<8>(buttons.GetGridCell(1, 1, 4, EDirection::Horizontal, 3), ""), kCtrlTagMeter);
...
void IPlugDrumSynth::OnParamChange(int paramIdx)
{
  switch (paramIdx)
  {
    case kParamMultiOuts: mDSP.SetMultiOut(GetParam(paramIdx)->Bool()); break;
    default: break;
  }
}
