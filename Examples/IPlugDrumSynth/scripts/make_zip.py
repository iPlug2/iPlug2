...
def  main():
  ...
  installer = "\installer\IPlugDrumSynth Installer.exe"
   
  if demo:
    installer = "\installer\IPlugDrumSynth Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugDrumSynth manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugDrumSynth-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugDrumSynth-v" + FULLVERSIONSTR + "-win-demo.zip"
