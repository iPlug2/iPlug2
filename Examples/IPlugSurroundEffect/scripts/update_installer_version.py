...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=IPlugSurroundEffect Demo Installer\n"
      else:
        line="OutputBaseFilename=IPlugSurroundEffect Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the IPlugSurroundEffect Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the IPlugSurroundEffect installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=IPlugSurroundEffect Demo installer\n"
     else:
       line="SetupWindowTitle=IPlugSurroundEffect installer\n"
