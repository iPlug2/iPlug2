...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=IPlugReaperPlugin Demo Installer\n"
      else:
        line="OutputBaseFilename=IPlugReaperPlugin Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the IPlugReaperPlugin Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the IPlugReaperPlugin installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=IPlugReaperPlugin Demo installer\n"
     else:
       line="SetupWindowTitle=IPlugReaperPlugin installer\n"
