...
def  main():
  ...
  installer = "\installer\IPlugUIKit Installer.exe"
   
  if demo:
    installer = "\installer\IPlugUIKit Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugUIKit manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugUIKit-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugUIKit-v" + FULLVERSIONSTR + "-win-demo.zip"
