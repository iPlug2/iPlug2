...
def  main():
  ...
  installer = "\installer\IPlugOSCEditor Installer.exe"
   
  if demo:
    installer = "\installer\IPlugOSCEditor Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugOSCEditor manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugOSCEditor-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugOSCEditor-v" + FULLVERSIONSTR + "-win-demo.zip"
