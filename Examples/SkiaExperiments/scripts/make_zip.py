...
def  main():
  ...
  installer = "\installer\SkiaExperiments Installer.exe"
   
  if demo:
    installer = "\installer\SkiaExperiments Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SkiaExperiments manual.pdf" 
  ]
  ...
  ZIPNAME = "SkiaExperiments-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SkiaExperiments-v" + FULLVERSIONSTR + "-win-demo.zip"
