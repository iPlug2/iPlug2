...
def  main():
  ...
  installer = "\installer\IPlugWebView Installer.exe"
   
  if demo:
    installer = "\installer\IPlugWebView Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugWebView manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugWebView-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugWebView-v" + FULLVERSIONSTR + "-win-demo.zip"
