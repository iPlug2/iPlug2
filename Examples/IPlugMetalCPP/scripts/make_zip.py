...
def  main():
  ...
  installer = "\installer\IPlugMetalCPP Installer.exe"
   
  if demo:
    installer = "\installer\IPlugMetalCPP Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugMetalCPP manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugMetalCPP-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugMetalCPP-v" + FULLVERSIONSTR + "-win-demo.zip"
