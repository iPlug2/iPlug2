...
def  main():
  ...
  installer = "\installer\IPlugVisualizer Installer.exe"
   
  if demo:
    installer = "\installer\IPlugVisualizer Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugVisualizer manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugVisualizer-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugVisualizer-v" + FULLVERSIONSTR + "-win-demo.zip"
