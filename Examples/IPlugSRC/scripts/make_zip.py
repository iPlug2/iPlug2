...
def  main():
  ...
  installer = "\installer\IPlugSRC Installer.exe"
   
  if demo:
    installer = "\installer\IPlugSRC Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugSRC manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugSRC-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugSRC-v" + FULLVERSIONSTR + "-win-demo.zip"
