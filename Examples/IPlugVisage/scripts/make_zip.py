...
def  main():
  ...
  installer = "\installer\IPlugVisage Installer.exe"
   
  if demo:
    installer = "\installer\IPlugVisage Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugVisage manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugVisage-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugVisage-v" + FULLVERSIONSTR + "-win-demo.zip"
