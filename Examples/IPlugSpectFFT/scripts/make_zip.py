...
def  main():
  ...
  installer = "\installer\IPlugSpectFFT Installer.exe"
   
  if demo:
    installer = "\installer\IPlugSpectFFT Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugSpectFFT manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugSpectFFT-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugSpectFFT-v" + FULLVERSIONSTR + "-win-demo.zip"
