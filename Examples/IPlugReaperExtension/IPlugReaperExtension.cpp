...
: ReaperExtBase(pRec)
{
  ...
  auto action2 = []() { InsertTrackAtIndex(GetNumTracks(), false); };
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(new IVButtonControl(
      bounds.GetGridCell(0, 3, 1).GetPadded(-20.).SubRectVertical(2, 0).GetMidVPadded(20),
      [&](IControl* pCaller) {
    ...
    //    pGraphics->AttachControl(new IVButtonControl(bounds.GetGridCell(0, 2, 2).GetPadded(-20.).SubRectVertical(2,
    //    1).GetMidVPadded(20),
    //                                                 [&](IControl* pCaller) {
    //                                                   SplashClickActionFunc(pCaller);
    //                                                   ToggleDocking();
    ...
    pGraphics->AttachControl(
      new ITextControl(bounds.GetGridCell(1, 3, 1), str.Get(), IText(24, EAlign::Center)), kCtrlTagText);

    pGraphics->AttachControl(new IVSliderControl(
      bounds.GetGridCell(2, 3, 1).GetPadded(-20),
      [](IControl* pCaller) {
        WDL_String valStr;
