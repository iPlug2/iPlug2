...
def  main():
  ...
  installer = "\installer\IPlugP5js Installer.exe"
   
  if demo:
    installer = "\installer\IPlugP5js Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugP5js manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugP5js-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugP5js-v" + FULLVERSIONSTR + "-win-demo.zip"
