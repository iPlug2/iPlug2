...
def  main():
  ...
  installer = "\installer\IPlugElements Installer.exe"
   
  if demo:
    installer = "\installer\IPlugElements Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugElements manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugElements-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugElements-v" + FULLVERSIONSTR + "-win-demo.zip"
