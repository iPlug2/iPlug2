...
- Generate test signals (sine, noise, step, chirp) for DSP analysis
...
These commands generate test signals for DSP analysis. Useful for measuring frequency response, distortion, and transient behavior.

#### `--sine <freq_hz> <duration_ms>`
Generate a sine wave at the specified frequency for the given duration.

```bash
# 1kHz sine wave for 100ms - useful for THD measurement
MyPlugin --set 0 100 --sine 1000 100 --output sine.wav
```

#### `--noise <duration_ms>`
Generate white noise for the specified duration.

```bash
# 500ms of noise - useful for frequency response measurement
MyPlugin --set 0 50 --noise 500 --output noise.wav
```

#### `--step <duration_ms>`
Generate a unit step (DC offset) for the specified duration.

```bash
# Step response - useful for transient analysis
MyPlugin --step 100 --output step.wav
```

#### `--chirp <start_hz> <end_hz> <duration_ms>`
Generate a logarithmic frequency sweep from start to end frequency.

```bash
# Sweep 20Hz to 20kHz over 2 seconds
MyPlugin --set 0 75 --chirp 20 20000 2000 --output chirp.wav
```

**Use Cases:**
- `--sine`: THD (Total Harmonic Distortion) measurement
- `--noise`: Full-spectrum frequency response
- `--step`: Attack/transient behavior, DC handling
- `--chirp`: Time-varying frequency response, aliasing detection
...
### DSP Analysis: THD Measurement

```bash
# Generate sine wave through effect and analyze for distortion
MyDistortion --set 0 75 --sine 1000 100 --output-txt thd.txt
# Then use external tools (Python/numpy) to compute THD from harmonics
```

### DSP Analysis: Frequency Sweep

```bash
# Chirp through a filter to visualize frequency response over time
MyFilter --set 0 1000 --chirp 20 20000 1000 --output chirp_response.wav
# View as spectrogram to see filter cutoff behavior
```

### DSP Analysis: Transient Response

```bash
# Step response to analyze attack characteristics
MyCompressor --set 0 50 --step 500 --output-txt step.txt
# Analyze rise time, overshoot, settling time
```

### DSP Analysis: Distortion Comparison

```bash
# Compare THD at different drive levels
for drive in 0 25 50 75 100; do
  MyDistortion --set 0 $drive --sine 1000 100 --output-txt "sine_drive${drive}.txt"
done
# Compute and compare THD for each
```
