...
struct FourValues : public IControl
{
  ...
  IRECT GetRect(int num) { return mRECT.GetGridCell(num / 2, num % 2, 2, 2).GetPadded(-120, -5, -120, -5); }
  ...
  auto updatePeersFunc =
    ...
    mLayoutFunc = [&](IGraphics* pGraphics) {
      ...
      pGraphics->AttachControl(
        new IVXYPadControl(b.GetGridCell(0, 2, 2), {kParamLeftX, kParamLeftY}), kCtrlLeftXYPad, "mux");
      pGraphics->AttachControl(
        new IVXYPadControl(b.GetGridCell(1, 2, 2), {kParamRightX, kParamRightY}), kCtrlRightXYPad, "mux");
      pGraphics->AttachControl(
        new IVKnobControl(b.GetGridCell(2, 2, 2).FracRectHorizontal(0.5), kParamLeftX), kCtrlLeftXKnob, "mux");
      pGraphics->AttachControl(
        new IVKnobControl(b.GetGridCell(2, 2, 2).FracRectHorizontal(0.5, true), kParamLeftY), kCtrlLeftYKnob, "mux");
      pGraphics->AttachControl(
        new IVKnobControl(b.GetGridCell(3, 2, 2).FracRectHorizontal(0.5), kParamRightX), kCtrlRightXKnob, "mux");
      pGraphics->AttachControl(
        new IVKnobControl(b.GetGridCell(3, 2, 2).FracRectHorizontal(0.5, true), kParamRightY), kCtrlRightYKnob, "mux");
      pGraphics->AttachControl(new IVSwitchControl(b.GetCentredInside(50), kParamLink));
      //    pGraphics->AttachControl(new FourValues(b.GetFromBottom(100).GetPadded(-20), kParamEnum1, kParamLeftX,
      //    kParamLeftY, kParamEnum2));

      pGraphics->ForControlInGroup("mux", [&](IControl* pControl) {
        pControl->SetActionFunction([&](IControl* pCaller) {
          if (GetParam(kParamLink)->Bool())
          {
            if (pCaller->GetTag() == kCtrlLeftXKnob || pCaller->GetTag() == kCtrlLeftYKnob
                || pCaller->GetTag() == kCtrlLeftXYPad)
            {
            ...
            else if (pCaller->GetTag() == kCtrlRightXKnob || pCaller->GetTag() == kCtrlRightYKnob
                     || pCaller->GetTag() == kCtrlRightXYPad)
            {
