...
def  main():
  ...
  installer = "\installer\MetaParamTest Installer.exe"
   
  if demo:
    installer = "\installer\MetaParamTest Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\MetaParamTest manual.pdf" 
  ]
  ...
  ZIPNAME = "MetaParamTest-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "MetaParamTest-v" + FULLVERSIONSTR + "-win-demo.zip"
