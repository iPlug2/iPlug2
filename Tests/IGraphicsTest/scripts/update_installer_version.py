...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=IGraphicsTest Demo Installer\n"
      else:
        line="OutputBaseFilename=IGraphicsTest Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the IGraphicsTest Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the IGraphicsTest installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=IGraphicsTest Demo installer\n"
     else:
       line="SetupWindowTitle=IGraphicsTest installer\n"
