...
python $EMSCRIPTEN/tools/file_packager.py imgs.data --use-preload-plugins --preload ../resources/img/ --use-preload-cache --indexedDB-name="/IGraphicsTest_pkg" --exclude *DS_Store --exclude  *@2x.png --exclude  *.svg >> imgs.js
# echo "\n}" >> imgs.js
...
echo "AudioWorkletGlobalScope.WAM = AudioWorkletGlobalScope.WAM || {}; AudioWorkletGlobalScope.WAM.IGraphicsTest = { ENVIRONMENT: 'WEB' };" > IGraphicsTest-wam.tmp.js;
cat IGraphicsTest-wam.js >> IGraphicsTest-wam.tmp.js
mv IGraphicsTest-wam.tmp.js IGraphicsTest-wam.js
...
sed -i.bak s,ORIGIN_PLACEHOLDER,$origin,g IGraphicsTest-awn.js
rm *.bak
