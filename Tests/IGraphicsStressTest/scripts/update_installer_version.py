...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=IGraphicsStressTest Demo Installer\n"
      else:
        line="OutputBaseFilename=IGraphicsStressTest Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the IGraphicsStressTest Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the IGraphicsStressTest installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=IGraphicsStressTest Demo installer\n"
     else:
       line="SetupWindowTitle=IGraphicsStressTest installer\n"
