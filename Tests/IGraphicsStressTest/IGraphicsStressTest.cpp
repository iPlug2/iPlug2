...
void IGraphicsStressTest::LayoutUI(IGraphics* pGraphics)
{
  ...
  if (pGraphics->NControls())
  {
    ...
    {
      pGraphics->GetControlWithTag(kCtrlTagButton1 + button)
        ->SetTargetAndDrawRECTs(buttonsArea.GetGridCell(button, 1, 6));
    }
    return;
  }
  ...
  auto DoFunc = [&](EFunc func, int thing = 0) {
    ...
    GetUI()
      ->GetControlWithTag(kCtrlTagNumThings)
      ->As<ITextControl>()
      ->SetStrFmt(64, "Number of things = %i", mNumberOfThings);
    GetUI()
      ->GetControlWithTag(kCtrlTagTestNum)
      ->As<ITextControl>()
      ->SetStrFmt(64, "Test %i/%i", this->mKindOfThing, 32);
      ...
      //      if (!g.CheckLayer(pCaller->mLayer))
      {
        ...
        for (int i = 0; i < this->mNumberOfThings; i++)
        {
          ...
          switch (this->mKindOfThing)
          {
            ...
            case 7:
              g.DrawArc(rc, rr.MW(), rr.MH(), rr.W() > rr.H() ? rr.H() : rr.W(), rrad1, rrad2, &rb, thickness);
              break;
            ...
            case 10:
              g.DrawDottedLine(rc, dir == 0 ? rr.L : rr.R, rr.B, dir == 0 ? rr.R : rr.L, rr.T, &rb, thickness);
              break;
  ...
  for (auto buttonLabel : {"Select test", "Next test", "Prev test", "Things++", "Things--"})
  {
    pGraphics
      ->AttachControl(
        new IVButtonControl(buttonsArea.GetGridCell(button, 1, 6), SplashClickActionFunc, buttonLabel,
                            DEFAULT_STYLE.WithLabelText(DEFAULT_TEXT.WithVAlign(EVAlign::Middle)).WithRoundness(0.2)),
        kCtrlTagButton1 + button)
        ...
        switch (button)
        {
          case 0:
          {
            static IPopupMenu menu{
              "Test",
              {"Start", "DrawRect", "FillRect", "DrawRoundRect", "FillRoundRect", "DrawEllipse", "FillEllipse",
               "DrawArc", "FillArc", "DrawLine", "DrawDottedLine", "DrawFittedBitmap", "DrawSVG"},
              [DoFunc](IPopupMenu* pMenu) { DoFunc(EFunc::Set, pMenu->GetChosenItemIdx()); }};
  ...
  pGraphics
    ->AttachControl(new IVToggleControl(buttonsArea.GetGridCell(button, 1, 6), SplashClickActionFunc, "",
                                        DEFAULT_STYLE.WithRoundness(0.2), "FPS OFF", "FPS ON"),
                    kCtrlTagButton6)
    ->SetAnimationEndActionFunction(
      [](IControl* pCaller) { pCaller->GetUI()->ShowFPSDisplay(pCaller->GetValue() > 0.5); });
}
#endif
