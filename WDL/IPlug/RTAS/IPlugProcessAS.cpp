...
{
  return new IPlugProcessAS;
}

IPlugProcessAS::IPlugProcessAS(void)
{ 
  // Reset metering values and multipliers for Template control
...
void IPlugProcessAS::GetMetersFromDSPorRTAS(long *allMeters, bool *clipIndicators)
{
  ...
  for (int i = 0; i < GetNumOutputs(); i++) 
  {
    ...
    // This adds some simple ballistics and keeps the meter from flickering.  The meter
    // value will never be less than 70% of the last output value
    if(currentVal < mMeterMin[i])
      currentVal = mMeterMin[i];
    mMeterMin[i] = currentVal * 0.7;
    
    if (fabs(currentVal) > 1.0)  // check for clipping
    { 
      currentVal = -1.0;
      ...
      currentVal *= k32BitPosMax;  // scale/convert it to an integer value.
      clipIndicators[i] = false;
    }
    
    allMeters[i] = currentVal;
    mMeterVal[i] = 0; // Clear the value out.
  }
}
...
void IPlugProcessAS::SetViewOrigin (Point anOrigin)
{
  ...
  CTemplateNoUIProcess::SetViewOrigin(anOrigin); // Call inherited
