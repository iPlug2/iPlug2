 ...
 * In most cases it does own the bitmap data, the exception being with NanoVG, where the image is loaded onto the GPU as
 * a texture, but still needs to be freed. Most of the time  end-users will deal with IBitmap rather than APIBitmap,
...
public:
   ...
   * @param scale An integer representing the scale of this bitmap in relation to a 1:1 pixel screen, e.g. 2 for an @2x
   * bitmap
...
private:
  ...
  WDL_String GetFontString(int nameID, EStringID stringID)
  {
    ...
    for (uint16_t i = 0; i < GetUInt16(mNameLocation + 2); ++i)
    {
      ...
      if (platformID == GetUInt16(loc + 0) && encodingID == GetUInt16(loc + 2) && languageID == GetUInt16(loc + 4)
          && nameID == GetUInt16(loc + 6))
      {
        ...
        switch (stringID)
        {
          ...
          case EStringID::Mac: return WDL_String((const char*)(mData + mNameLocation + stringLocation), length);
        }
      }
    }
...
public:
  IFontData()
  : IFontInfo(nullptr, 0, -1)
  , mFaceIdx(-1)
  {
  }

  IFontData(const void* data, int size, int faceIdx)
  : IFontInfo(data, size, faceIdx)
  , mFaceIdx(faceIdx)
  {
...
/** Used internally to store data statically, making sure memory is not wasted when there are multiple plug-in instances
 * loaded */
template <class T>
class StaticStorage
{
