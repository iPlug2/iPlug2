...
// for a paramater ID MyParam, define constants named MyParam_X, MyParam_Y, MyParam_W, MyParam_H to specify the
// Control's IRect then when instantiating a Control you can just call MakeIRect(MyParam) to specify the IRect
...
#define MakeIRectHVOffset(a, xoffs, yoffs)                                                                             \
  IRECT(a##_X + xoffs, a##_Y + yoffs, a##_X + a##_W + xoffs, a##_Y + a##_H + yoffs)
...
/** Calculate evenly distributed points on a radial line. NOTE: will crash if the nPoints and data array do not match
 * size.
...
static inline void RadialPoints(float angleDegrees, float cx, float cy, float rMin, float rMax, int nPoints,
                                float data[][2])
{
