...
#pragma mark -
  ...
  IPopupMenu(const char* rootTitle = "", int prefix = 0, bool multicheck = false,
  ...
  ~IPopupMenu() { mMenuItems.Empty(true); }

  static int Sortfunc(const Item** a, const Item** b) { return stricmp((*a)->GetText(), (*b)->GetText()); }
  ...
  {
    return AddItem(new Item(str, itemFlags), index);
  }
  ...
  Item* GetChosenItem() { return GetItem(mChosenItemIdx); }
  ...
  void SetFunction(IPopupFunction func) { mPopupFunc = func; }

  IPopupFunction GetFunction() { return mPopupFunc; }

  void ExecFunction() { mPopupFunc(this); }

  const char* GetRootTitle() const { return mRootTitle.Get(); }

  void SetRootTitle(const char* rootTitle) { return mRootTitle.Set(rootTitle); }
...
private:
  ...
  bool mCanMultiCheck; // multicheck = 0 doesn't actually prohibit multichecking, you should do that in your code, by
                       // calling CheckItemAlone instead of CheckItem
