...
public:
   ...
   * @param collapsedBounds If this control, when collapsed should occupy an area of the graphics context, specify this,
   * otherwise the collapsed area is empty
   * @param expandedBounds If you want to explicitly specify the size of the expanded pop-up, you can specify an area
   * here */
  IPopupMenuControl(int paramIdx = kNoParameter, IText text = IText(16), IRECT collapsedBounds = IRECT(),
  ...
  virtual void DrawCellBackground(IGraphics& g, const IRECT& bounds, const IPopupMenu::Item* pItem, bool sel,
                                  IBlend* pBlend);
  ...
  virtual void DrawSubMenuArrow(IGraphics& g, const IRECT& bounds, const IPopupMenu::Item* pItem, bool sel,
                                IBlend* pBlend);
  ...
  /** Sets the amount the main menu is shifted to make room for submenus. This helps on small GUI's where submenus will
   * intrude upon the main menu. This does not affect the positioning of menus that do not contain submenus.
   * @param distance The distance in pixels to shift the main menu. Use only positive numbers. It will shift left if the
   * center of the control that opens it is right of center of UI and to the right if otherwise. */
   ...
   * @param menu Reference to a menu from which to populate this user interface control. NOTE: this object should not be
   * a temporary, otherwise when the menu returns asynchronously, it may not exist.
...
private:
  ...
  public:
    ...
    {
      mScrollItemOffset--;
      mScrollItemOffset = Clip(mScrollItemOffset, 0, mCellBounds.GetSize() - 1);
    }

    void ScrollDown()
    {
      mScrollItemOffset++;
      mScrollItemOffset = Clip(mScrollItemOffset, 0, mMenu.NItems() - mCellBounds.GetSize());
    }

    /** Checks if any of the expanded cells for this panel contain a x, y coordinate, and if so returns an IRECT pointer
     * to the cell bounds
  ...
  public:
    ...
    WDL_PtrList<IRECT>
      mCellBounds; // The size of this array will always correspond to the number of items in the top level of the menu
  ...
  WDL_PtrList<MenuPanel> mMenuPanels; // An array of ptrs to MenuPanel objects for every panel, expands as sub menus are
                                      // revealed, contents deleted when the menu is dismissed
  ...
  IPopupMenu* mMenu =
    nullptr; // Pointer to the main IPopupMenu, that this control is visualising. This control does not own the menu.
  ...
  bool mForcedSouth = true; // if set true, a menu in the lower half of the GUI will appear below it's control if there
  ...
  IRECT mMaxBounds; // if view is only showing a part of the graphics context,
                    // we need to know because menus can't go there
