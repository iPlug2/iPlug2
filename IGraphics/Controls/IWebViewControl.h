...
/** A control that allows the embedding of HTML UI inside an IGraphics context using a platform-native webview and
 * bi-directional communication with that content NOTE: this control attaches a sub view on top of the IGraphics view,
 * so it will render all content on-top The platform native webviews run in a separate process and communicate via IPC.
 * On Windows the ICoreWebView2 is used, which requires Edge Chromium to be installed:
 * https://docs.microsoft.com/en-us/microsoft-edge/webview2 On macOS and iOS WkWebView is used
 * @ingroup IControls */
...
public:
   ...
   * @param readyFunc A function conforming to onReadyFunc, that will be called asyncronously when the webview has been
   * initialized
   * @param msgFunc A function conforming to onMessageFunc, that will be called when messages are posted from the
   * webview
   * @param dllPath (Windows only) an absolute path to the WebView2Loader.dll that is required to use the WebView2 on
   * windows
  ...
  IWebViewControl(const IRECT& bounds, bool opaque, OnReadyFunc readyFunc, OnMessageFunc msgFunc = nullptr,
                  const char* dllPath = "", const char* tmpPath = "")
  : IControl(bounds)
  , IWebView(opaque)
  ...
  void OnRescale() override { UpdateWebViewBounds(); }

  void OnResize() override { UpdateWebViewBounds(); }
