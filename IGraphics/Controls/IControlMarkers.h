...
//    }
    ...
    auto angle = a1r - float(iplug::PI / 2.0) + float(i * div);
...
static void DrawRadialLabels(IGraphics& g, const IRECT& track, const std::vector<std::string>& labels, float a1 = -135.0f, float a2 = 135.0f, const IText& text = DEFAULT_VALUE_TEXT)
{
  ...
  for (auto i = 0; i < numMajor; ++i)
  {
    auto angle = a1r - float(iplug::PI / 2.0f) + float(i * div);
    auto sinX = std::sin(angle);
    auto cosX = std::cos(angle);

    auto x = track.MW() + (radius * 1.75f) * cosX;
    auto y = track.MH() + (radius * 1.75f) * sinX;
...
public:
  ...
  void Draw(IGraphics& g) override
  {
    ...
    {
      g.StartLayer(this, mRECT);
      DrawRadialMarks(g, mTrackBounds, static_cast<int>(mLabels.size()), mAngle1, mAngle2, mMajorMarksWidth, mMinorMarksWidth, mMarkSize, mMarkRatio);
...
private:
  ...
  const float mAngle1, mAngle2;
  const float mMarkSize;
  const float mMarkRatio;
  const float mMajorMarksWidth = 2.0f;
  const float mMinorMarksWidth = 0.5f;
  ILayerPtr mLayer;
};
