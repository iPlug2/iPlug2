...
public:
  ...
  IBubbleControl(const IText& text = DEFAULT_TEXT.WithAlign(EAlign::Center), const IColor& fillColor = COLOR_WHITE,
                 const IColor& strokeColor = COLOR_BLACK, float roundness = 5.f)
  : IControl(IRECT())
  ...
  , mStrokeColor(strokeColor)
  {
    ...
    SetActionFunction([animationFunc, this](IControl* pCaller) { SetAnimation(animationFunc, 200); });
...
protected:
  virtual void DrawContent(IGraphics& g, const IRECT& r) { g.DrawText(mText, mStr.Get(), r, &mBlend); }
  ...
  virtual void MeasureText(const char* str, IRECT& contentBounds) { GetUI()->MeasureText(mText, str, contentBounds); }
  ...
  bool GetActive() const { return mState > EPopupState::kCollapsed; }

  ITouchID GetTouchID() const { return mTouchId; }
