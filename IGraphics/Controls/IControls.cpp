...
void IVLabelControl::Draw(IGraphics& g)
{
  ...
  if (mStr.GetLength())
  {
    if (mStyle.drawShadows && !IsDisabled())
      g.DrawText(mStyle.valueText.WithFGColor(GetColor(kSH)), mStr.Get(),
                 mRECT.GetTranslated(mStyle.shadowOffset, mStyle.shadowOffset), &mBlend);
...
IVButtonControl::IVButtonControl(const IRECT& bounds, IActionFunction aF, const char* label, const IVStyle& style,
                                 bool labelInButton, bool valueInButton, EVShape shape)
...
IVSwitchControl::IVSwitchControl(const IRECT& bounds, int paramIdx, const char* label, const IVStyle& style,
                                 bool valueInButton)
...
IVSwitchControl::IVSwitchControl(const IRECT& bounds, IActionFunction aF, const char* label, const IVStyle& style,
                                 int numStates, bool valueInButton)
...
IVToggleControl::IVToggleControl(const IRECT& bounds, int paramIdx, const char* label, const IVStyle& style,
...
IVToggleControl::IVToggleControl(const IRECT& bounds, IActionFunction aF, const char* label, const IVStyle& style,
                                 const char* offText, const char* onText, bool initialState)
...
IVSlideSwitchControl::IVSlideSwitchControl(const IRECT& bounds, int paramIdx, const char* label, const IVStyle& style,
...
IVSlideSwitchControl::IVSlideSwitchControl(const IRECT& bounds, IActionFunction aF, const char* label,
                                           const IVStyle& style, bool valueInButton, EDirection direction,
                                           int numStates, int initialState)
...
IVTabSwitchControl::IVTabSwitchControl(const IRECT& bounds, int paramIdx,
                                       const std::initializer_list<const char*>& options, const char* label,
...
IVTabSwitchControl::IVTabSwitchControl(const IRECT& bounds, IActionFunction aF,
                                       const std::initializer_list<const char*>& options, const char* label,
                                       const IVStyle& style, EVShape shape, EDirection direction)
                                    ...
                                    bool disabled)
{
  switch (mShape)
  {
    case EVShape::EndsRounded:
      if (mDirection == EDirection::Horizontal)
        DrawPressableRectangle(
          g, r, pressed, mouseOver, disabled, segment == ETabSegment::Start, segment == ETabSegment::End, false, false);
      else
        DrawPressableRectangle(
          g, r, pressed, mouseOver, false, disabled, segment == ETabSegment::Start, false, segment == ETabSegment::End);
      break;
    case EVShape::AllRounded:
      ...
      break;
    default: DrawPressableShape(g, mShape, r, pressed, mouseOver, disabled); break;
  }
}
...
IVRadioButtonControl::IVRadioButtonControl(const IRECT& bounds, int paramIdx,
                                           const std::initializer_list<const char*>& options, const char* label,
                                           const IVStyle& style, EVShape shape, EDirection direction, float buttonSize)
...
IVRadioButtonControl::IVRadioButtonControl(const IRECT& bounds, IActionFunction aF,
                                           const std::initializer_list<const char*>& options, const char* label,
                                           const IVStyle& style, EVShape shape, EDirection direction, float buttonSize)
...
void IVRadioButtonControl::DrawWidget(IGraphics& g)
{
  ...
  for (int i = 0; i < mNumStates; i++)
  {
    ...
    if (mTabLabels.Get(i))
    {
      ...
      g.DrawText(
        mStyle.valueText.WithFGColor(i == hit ? GetColor(kON) : GetColor(kX1)), mTabLabels.Get(i)->Get(), r, &mBlend);
    }
  }
}
...
IVKnobControl::IVKnobControl(const IRECT& bounds, int paramIdx, const char* label, const IVStyle& style,
                             bool valueIsEditable, bool valueInWidget, float a1, float a2, float aAnchor,
                             EDirection direction, double gearing, float trackSize)
...
IVKnobControl::IVKnobControl(const IRECT& bounds, IActionFunction aF, const char* label, const IVStyle& style,
                             bool valueIsEditable, bool valueInWidget, float a1, float a2, float aAnchor,
                             EDirection direction, double gearing, float trackSize)
...
IVSliderControl::IVSliderControl(const IRECT& bounds, int paramIdx, const char* label, const IVStyle& style,
                                 bool valueIsEditable, EDirection dir, double gearing, float handleSize,
                                 float trackSize, bool handleInsideTrack, float handleXOffset, float handleYOffset)
...
IVSliderControl::IVSliderControl(const IRECT& bounds, IActionFunction aF, const char* label, const IVStyle& style,
                                 bool valueIsEditable, EDirection dir, double gearing, float handleSize,
                                 float trackSize, bool handleInsideTrack, float handleXOffset, float handleYOffset)
...
void IVSliderControl::DrawTrack(IGraphics& g, const IRECT& filledArea)
{
  ...
  const IRECT adjustedTrackBounds =
    mDirection == EDirection::Vertical ? mTrackBounds.GetVPadded(extra) : mTrackBounds.GetHPadded(extra);
  const IRECT adjustedFillBounds =
    mDirection == EDirection::Vertical ? filledArea.GetVPadded(extra) : filledArea.GetHPadded(extra);
...
void IVSliderControl::DrawWidget(IGraphics& g)
{
  ...
  if (mHandleSize > 0.f)
  {
    DrawHandle(g, {cx + mHandleXOffset - mHandleSize, cy + mHandleYOffset - mHandleSize,
...
IVRangeSliderControl::IVRangeSliderControl(const IRECT& bounds, const std::initializer_list<int>& params,
                                           const char* label, const IVStyle& style, EDirection dir, bool onlyHandle,
                                           float handleSize, float trackSize)
...
void IVRangeSliderControl::DrawWidget(IGraphics& g)
{
  ...
  for (int i = 0; i < NVals() - 1; i++)
  {
    ...
    if (mDirection == EDirection::Vertical)
      g.FillRect(GetColor(kX1),
                 IRECT(filled1.L, filled1.T < filled2.T ? filled1.T : filled2.T, filled1.R,
                       filled1.T > filled2.T ? filled1.T : filled2.T),
                 &mBlend);
    else
      g.FillRect(GetColor(kX1),
                 IRECT(filled1.R < filled2.R ? filled1.R : filled2.R, filled1.T,
                       filled1.R > filled2.R ? filled1.R : filled2.R, filled1.B),
                 &mBlend);
  }
...
IVXYPadControl::IVXYPadControl(const IRECT& bounds, const std::initializer_list<int>& params, const char* label,
                               const IVStyle& style, float handleRadius, bool trackClipsHandle, bool drawCross)
...
void IVXYPadControl::DrawWidget(IGraphics& g)
{
  ...
  const IRECT handleBounds = IRECT(trackBounds.L + xpos - mHandleRadius, trackBounds.B - ypos - mHandleRadius,
                                   trackBounds.L + xpos + mHandleRadius, trackBounds.B - ypos + mHandleRadius);
...
IVPlotControl::IVPlotControl(const IRECT& bounds, const std::initializer_list<Plot>& plots, int numPoints,
                             const char* label, const IVStyle& style, float min, float max, bool useLayer)
: IControl(bounds)
, IVectorBase(style)
, mMin(min)
, mMax(max)
...
void IVPlotControl::Draw(IGraphics& g)
{
  ...
  auto drawFunc = [&]() {
    ...
    for (int p = 0; p < mPlots.size(); p++)
    {
      ...
      g.DrawData(
        mPlots[p].color, plotsRECT, mPoints.data(), static_cast<int>(mPoints.size()), nullptr, &mBlend, mTrackSize);
    }
...
IVGroupControl::IVGroupControl(const char* label, const char* groupName, float padL, float padT, float padR, float padB,
                               const IVStyle& style)
: IControl(IRECT())
, IVectorBase(style)
...
void IVGroupControl::SetBoundsBasedOnGroup(const char* groupName, float padL, float padT, float padR, float padB)
{
  ...
  GetUI()->ForControlInGroup(
    mGroupName.Get(), [&unionRect](IControl* pControl) { unionRect = unionRect.Union(pControl->GetRECT()); });
...
IVColorSwatchControl::IVColorSwatchControl(const IRECT& bounds, const char* label, ColorChosenFunc func,
...
{
  g.DrawRotatedSVG(
    mSVG, mRECT.MW(), mRECT.MH(), mRECT.W(), mRECT.H(), mStartAngle + GetValue() * (mEndAngle - mStartAngle), &mBlend);
}
...
ISVGSwitchControl::ISVGSwitchControl(const IRECT& bounds, const std::initializer_list<ISVG>& svgs, int paramIdx,
                                     IActionFunction aF)
...
ISVGSliderControl::ISVGSliderControl(const IRECT& bounds, const ISVG& handleSVG, const ISVG& trackSVG, int paramIdx,
                                     EDirection dir, double gearing)
...
void ISVGSliderControl::OnResize()
{
  ...
  if (mDirection == EDirection::Vertical)
  {
    ...
    handleBoundsAtMidPoint =
      mRECT.GetCentredInside(mRECT.H() * handleAspectRatio * handleOverTrackHeight, mRECT.H() * handleOverTrackHeight);
    mHandleBoundsAtMax = {handleBoundsAtMidPoint.L, mTrackSVGBounds.T, handleBoundsAtMidPoint.R,
                          mTrackSVGBounds.T + handleBoundsAtMidPoint.H()};
  ...
  else
  {
    ...
    mHandleBoundsAtMax = {mTrackSVGBounds.R - handleBoundsAtMidPoint.W(), mTrackSVGBounds.MH() - halfHeight,
                          mTrackSVGBounds.R, mTrackSVGBounds.MH() + halfHeight};
...
IBSliderControl::IBSliderControl(float x, float y, float trackLength, const IBitmap& handleBitmap,
                                 const IBitmap& trackBitmap, int paramIdx, EDirection dir, double gearing)
: ISliderControlBase(IRECT::MakeXYWH(x, y, dir == EDirection::Vertical ? handleBitmap.W() : trackLength,
                                     dir == EDirection::Vertical ? trackLength : handleBitmap.H()),
                     paramIdx, dir, gearing, float(dir == EDirection::Vertical ? handleBitmap.H() : handleBitmap.W()))
...
IBSliderControl::IBSliderControl(const IRECT& bounds, const IBitmap& handleBitmap, const IBitmap& trackBitmap,
                                 int paramIdx, EDirection dir, double gearing)
: ISliderControlBase(
  bounds, paramIdx, dir, gearing, float(dir == EDirection::Vertical ? handleBitmap.H() : handleBitmap.W()))
, IBitmapBase(handleBitmap)
, mTrackBitmap(trackBitmap)
{
  AttachIControl(this);
...
IBTextControl::IBTextControl(const IRECT& bounds, const IBitmap& bitmap, const IText& text, const char* str,
                             int charWidth, int charHeight, int charOffset, bool multiLine, bool vCenter, EBlend blend)
...
{
  g.DrawBitmapedText(
    mBitmap, mRECT, mText, &mBlend, mStr.Get(), mVCentre, mMultiLine, mCharWidth, mCharHeight, mCharOffset);
}
