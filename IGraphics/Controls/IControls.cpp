...
void IVLabelControl::Draw(IGraphics& g)
{
	...
	if (mStr.GetLength())
	{
		if (mStyle.drawShadows && !IsDisabled())
			g.DrawText(mStyle.valueText.WithFGColor(GetColor(kSH)),
					   mStr.Get(),
					   mRECT.GetTranslated(mStyle.shadowOffset, mStyle.shadowOffset),
					   &mBlend);
...
IVButtonControl::IVButtonControl(const IRECT& bounds,
								 IActionFunction aF,
								 const char* label,
								 const IVStyle& style,
								 bool labelInButton,
...
IVSwitchControl::IVSwitchControl(
	const IRECT& bounds, int paramIdx, const char* label, const IVStyle& style, bool valueInButton)
...
IVSwitchControl::IVSwitchControl(
	const IRECT& bounds, IActionFunction aF, const char* label, const IVStyle& style, int numStates, bool valueInButton)
...
IVToggleControl::IVToggleControl(
	const IRECT& bounds, int paramIdx, const char* label, const IVStyle& style, const char* offText, const char* onText)
...
IVTabSwitchControl::IVTabSwitchControl(const IRECT& bounds,
									   int paramIdx,
...
IVRadioButtonControl::IVRadioButtonControl(const IRECT& bounds,
										   int paramIdx,
										   const std::initializer_list<const char*>& options,
										   const char* label,
										   const IVStyle& style,
...
void IVRadioButtonControl::DrawWidget(IGraphics& g)
{
	...
	for (int i = 0; i < mNumStates; i++)
	{
		...
		if (mTabLabels.Get(i))
		{
			...
			g.DrawText(mStyle.valueText.WithFGColor(i == hit ? GetColor(kON) : GetColor(kX1)),
...
IVRangeSliderControl::IVRangeSliderControl(const IRECT& bounds,
...
void IVRangeSliderControl::DrawWidget(IGraphics& g)
{
	...
	for (int i = 0; i < NVals() - 1; i++)
	{
		...
		if (mDirection == EDirection::Vertical)
			...
			g.FillRect(GetColor(kX1),
					   IRECT(filled1.R < filled2.R ? filled1.R : filled2.R,
							 filled1.T,
							 filled1.R > filled2.R ? filled1.R : filled2.R,
							 filled1.B),
					   &mBlend);
	}
...
IVXYPadControl::IVXYPadControl(const IRECT& bounds,
							   const std::initializer_list<int>& params,
							   const char* label,
							   const IVStyle& style,
							   float handleRadius)
...
void IVXYPadControl::DrawWidget(IGraphics& g)
{
	...
	const IRECT handleBounds = IRECT(trackBounds.L + xpos - mHandleRadius,
									 trackBounds.B - ypos - mHandleRadius,
									 trackBounds.L + xpos + mHandleRadius,
									 trackBounds.B - ypos + mHandleRadius);
...
IVPlotControl::IVPlotControl(const IRECT& bounds,
							 const std::initializer_list<Plot>& plots,
							 int numPoints,
							 const char* label,
							 const IVStyle& style,
							 float min,
...
IVGroupControl::IVGroupControl(
	const char* label, const char* groupName, float padL, float padT, float padR, float padB, const IVStyle& style)
	: IControl(IRECT())
	, IVectorBase(style)
...
{
	g.DrawRotatedSVG(mSVG,
					 mRECT.MW(),
					 mRECT.MH(),
					 mRECT.W(),
					 mRECT.H(),
					 mStartAngle + GetValue() * (mEndAngle - mStartAngle),
					 &mBlend);
}
...
ISVGSwitchControl::ISVGSwitchControl(const IRECT& bounds,
...
ISVGSliderControl::ISVGSliderControl(
	const IRECT& bounds, const ISVG& handleSVG, const ISVG& trackSVG, int paramIdx, EDirection dir, double gearing)
...
void ISVGSliderControl::OnResize()
{
	...
	if (mDirection == EDirection::Vertical)
	{
		...
		handleBoundsAtMidPoint = mRECT.GetCentredInside(mRECT.H() * handleAspectRatio * handleOverTrackHeight,
														mRECT.H() * handleOverTrackHeight);
		mHandleBoundsAtMax     = {handleBoundsAtMidPoint.L,
                              mTrackSVGBounds.T,
                              handleBoundsAtMidPoint.R,
	...
	else
	{
		...
		mHandleBoundsAtMax     = {mTrackSVGBounds.R - handleBoundsAtMidPoint.W(),
                              mTrackSVGBounds.MH() - halfHeight,
                              mTrackSVGBounds.R,
                              mTrackSVGBounds.MH() + halfHeight};
...
IBSliderControl::IBSliderControl(float x,
								 float y,
								 float trackLength,
								 ...
								 EDirection dir,
								 double gearing)
	: ISliderControlBase(IRECT::MakeXYWH(x,
										 y,
...
IBSliderControl::IBSliderControl(const IRECT& bounds,
								 const IBitmap& handleBitmap,
								 const IBitmap& trackBitmap,
								 int paramIdx,
								 EDirection dir,
...
IBTextControl::IBTextControl(const IRECT& bounds,
