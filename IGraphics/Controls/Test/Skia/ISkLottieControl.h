...
public:
  ...
  void DoDraw(SkSize size, SkCanvas* canvas)
  {
    if (size.width() != mSize.width() || size.height() != mSize.height())
    {
      ...
      {
        float scale = std::min(size.width() / mAnimationSize.width(), size.height() / mAnimationSize.height());
        mMatrix.setScaleTranslate(scale, scale, (size.width() - mAnimationSize.width() * scale) * 0.5f,
