...
static const std::initializer_list<const char*> alignStrs = {
...
static const std::initializer_list<const char*> dirStrs = {
  "YGFlexDirectionColumn", "YGFlexDirectionColumnReverse", "YGFlexDirectionRow", "YGFlexDirectionRowReverse"};
static const std::initializer_list<const char*> justifyStrs = {"YGJustifyFlexStart",   "YGJustifyCenter",
...
public:
  ...
  void OnResize() override { DoLayout(); }
  ...
  void DoLayout()
  {
    ...
    mSettingsStr.SetFormatted(256, "%s, %s, %s, %s", alignStrs.begin()[mAlign], dirStrs.begin()[mDirection],
                              justifyStrs.begin()[mJustify], wrapStrs.begin()[mWrap]);
  }
  ...
  void OnPopupMenuSelection(IPopupMenu* pSelectedMenu, int) override
  {
    if (pSelectedMenu)
    {
      ...
      if (strcmp(pSelectedMenu->GetRootTitle(), "Align") == 0)
      {
        mAlign = idx;
      }
      else if (strcmp(pSelectedMenu->GetRootTitle(), "Direction") == 0)
      {
        mDirection = idx;
      }
      else if (strcmp(pSelectedMenu->GetRootTitle(), "Justify") == 0)
      {
        mJustify = idx;
      }
      else if (strcmp(pSelectedMenu->GetRootTitle(), "Wrap") == 0)
      {
        mWrap = idx;
      }
    }
