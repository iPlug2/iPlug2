...
class IVMultiSliderControl : public IVTrackControlBase
{
 public:
	...
	IVMultiSliderControl(const IRECT& bounds,
						 const char* label,
	...
	IVMultiSliderControl(
		const IRECT& bounds, const char* label, const IVStyle& style, int loParamIdx, int nSteps, EDirection dir)
					 ...
					 double maxClip = 1.) override
	{
		...
		if (sliderTest > -1)
		{
			SetValue(math::Clamp(value, 0., 1.), sliderTest);
...
{
 public:
	IVMultiToggleControl(const IRECT& bounds,
						 const char* label,
						 const IVStyle& style = DEFAULT_STYLE,
						 EDirection dir       = EDirection::Vertical)
