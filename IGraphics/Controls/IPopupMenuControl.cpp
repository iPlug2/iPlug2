				...
				else if (mState == kCollapsing)
				{
					for (auto i = 0; i < mMenuPanels.GetSize(); i++)
					{
						mMenuPanels.Get(i)->mBlend.mWeight = (float) (1. - progress) * mOpacity;
						 ...
						 kIdling)  // Idling is a special stage, to force the menu to redraw, before complete collapse
				{
					for (auto i = 0; i < mMenuPanels.GetSize(); i++)
					{
		...
		if (pMenuPanel->mShouldDraw)
		{
			...
			for (auto i = startCell; i <= endCell; i++)
			{
				...
				IPopupMenu::Item* pMenuItem =
					pMenuPanel->mMenu.GetItem(startCell + pMenuPanel->mScrollItemOffset + cellOffset++);
...
void IPopupMenuControl::DrawPanelShadow(IGraphics& g, MenuPanel* panel)
{
	...
	NVGpaint shadowPaint = nvgBoxGradient(vg,
										  ...
										  mRoundness * 2.f,
										  20.f,
										  NanoVGColor(COLOR_BLACK_DROP_SHADOW, &panel->mBlend),
...
void IPopupMenuControl::DrawCellBackground(
...
void IPopupMenuControl::DrawCellText(
	IGraphics& g, const IRECT& bounds, const IPopupMenu::Item* pItem, bool sel, IBlend* pBlend)
{
...
void IPopupMenuControl::DrawTick(
	IGraphics& g, const IRECT& bounds, const IPopupMenu::Item* pItem, bool sel, IBlend* pBlend)
{
	IRECT tickRect = IRECT(bounds.L, bounds.T, bounds.L + TICK_SIZE, bounds.B).GetCentredInside(TICK_SIZE);
...
void IPopupMenuControl::DrawSubMenuArrow(
	IGraphics& g, const IRECT& bounds, const IPopupMenu::Item* pItem, bool sel, IBlend* pBlend)
{
...
void IPopupMenuControl::GetPanelDimensions(IPopupMenu& menu, float& width, float& height) const
{
	...
	float numCells = static_cast<float>(numItems - numSeparators);
		...
		if (pCellRect == mMouseCellBounds)
		{
			if (pSubMenu != nullptr)
			{
				...
				if (pMenuPanelForThisMenu == nullptr)
				{
					...
					const float minT = mMaxBounds.T + mDropShadowSize;
					const float maxB = mMaxBounds.B - panelHeight - (mDropShadowSize * 2.f);
					const float maxR = mMaxBounds.R - panelWidth - (mDropShadowSize * 2.f);
					const float minL = mMaxBounds.L + mDropShadowSize;
					...
					if (mCalloutArrowDir == kSouth)
						yy = pCellRect->T - PAD;
					else if (mCalloutArrowDir == kNorth)
						yy = (pCellRect->T - (PAD / 2.f) - panelHeight) + (mCellGap * 2.f) + mDropShadowSize;

					if (mSubmenuOnRight)
						xx = pCellRect->R + PAD + calloutSpace;
					else
				...
				for (auto mr = 0; mr < mMenuPanels.GetSize(); mr++)
				{
					...
					if (pMenuPanel->mShouldDraw)
					{
						...
						while (pParentMenuPanel != nullptr)
						{
							...
							if (mSubmenuOnRight)
								mSubMenuCalloutArrowBounds = IRECT(pCellRect->R + PAD,
																   pCellRect->MH() - (calloutSpace / 2.f),
																   pCellRect->R + PAD + calloutSpace,
																   pCellRect->MH() + (calloutSpace / 2.f));
							else
								mSubMenuCalloutArrowBounds = IRECT(pCellRect->L - PAD - calloutSpace,
																   pCellRect->MH() - (calloutSpace / 2.f),
																   pCellRect->L - PAD,
																   pCellRect->MH() + (calloutSpace / 2.f));
						}
...
void IPopupMenuControl::Expand(const IRECT& anchorArea)
{
	...
	if (anchorArea.MH() <= mMaxBounds.MH())
	{
		...
		y = (anchorArea.MH() - panelHeight) + (calloutSpace + mText.mSize);

	if (anchorArea.MW() <= mMaxBounds.MW())
	{
		x = anchorArea.R + calloutSpace;
		mCalloutArrowBounds =
			IRECT(anchorArea.R, anchorArea.MH() - (calloutSpace / 2.f), x, anchorArea.MH() + (calloutSpace / 2.f));
		mCalloutArrowDir = kEast;
	}
	else
	{
		x                   = anchorArea.L - calloutSpace - panelWidth - mDropShadowSize;
		mCalloutArrowBounds = IRECT(anchorArea.L - calloutSpace,
									anchorArea.MH() - (calloutSpace / 2.f),
									anchorArea.L,
									anchorArea.MH() + (calloutSpace / 2.f));
	...
	if (y <= minT || y > maxB || x <= minL || x > maxR)  // if we're going off the top, right, left, or bottom
	{
		if ((y <= minT || x <= minL || x > maxR) && anchorArea.MH() <= mMaxBounds.MH())
		{
			...
			mCalloutArrowBounds = IRECT(anchorArea.MW() - (calloutSpace / 2.f),
										anchorArea.B,
										anchorArea.MW() + (calloutSpace / 2.f),
										anchorArea.B + calloutSpace);
			...
			if (y > maxB)
			{
				...
				{
					x                   = anchorArea.R + calloutSpace;
					mCalloutArrowBounds = IRECT(anchorArea.R,
												anchorArea.MH() - (calloutSpace / 2.f),
												x,
												anchorArea.MH() + (calloutSpace / 2.f));
					mCalloutArrowDir    = kEast;
	...
	for (auto i = 0; i < menu.NItems(); ++i)
	{
		...
		if ((top + toAddY + control.PAD) > control.mMaxBounds.B || newColumn)  // it's gonna go off the bottom
		{
			if (control.mScrollIfTooBig)
			{
				...
				mScrollMaxRows        = static_cast<int>(
                    maxH / (CellHeight() + control.mCellGap));  // maximum cell rows (full height, not with separators)
