...
void IPopupMenuControl::DrawCellBackground(IGraphics& g, const IRECT& bounds, const IPopupMenu::Item* pItem, bool sel,
                                           IBlend* pBlend)
{
  if (sel)
...
void IPopupMenuControl::DrawCellText(IGraphics& g, const IRECT& bounds, const IPopupMenu::Item* pItem, bool sel,
                                     IBlend* pBlend)
{
...
void IPopupMenuControl::DrawTick(IGraphics& g, const IRECT& bounds, const IPopupMenu::Item* pItem, bool sel,
                                 IBlend* pBlend)
{
  IRECT tickRect = IRECT(bounds.L, bounds.T, bounds.L + TICK_SIZE, bounds.B).GetCentredInside(TICK_SIZE);
...
void IPopupMenuControl::DrawSubMenuArrow(IGraphics& g, const IRECT& bounds, const IPopupMenu::Item* pItem, bool sel,
                                         IBlend* pBlend)
{
    ...
    if (pCellRect == mMouseCellBounds)
    {
      if (pSubMenu != nullptr)
      {
        ...
        if (pMenuPanelForThisMenu == nullptr)
        {
          ...
          if (mSubmenuOnRight)
            x = pCellRect->R + PAD + calloutSpace;
          else
            x = pCellRect->L - PAD - calloutSpace - panelWidth - mDropShadowSize;
          if (x <= minL)
            x = minL;
          if (x > maxR)
            x = maxR;
          ...
          pMenuPanelForThisMenu =
            mMenuPanels.Add(new MenuPanel(*this, *pSubMenu, x, y, mMenuPanels.Find(mActiveMenuPanel)));
        }
        ...
        for (auto mr = 0; mr < mMenuPanels.GetSize(); mr++)
        {
          ...
          if (pMenuPanel->mShouldDraw)
          {
            ...
            while (pParentMenuPanel != nullptr)
            {
              ...
              if (mSubmenuOnRight)
                mSubMenuCalloutArrowBounds =
                  IRECT(pCellRect->R + PAD, pCellRect->MH() - (calloutSpace / 2.f), pCellRect->R + PAD + calloutSpace,
                        pCellRect->MH() + (calloutSpace / 2.f));
              else
                mSubMenuCalloutArrowBounds =
                  IRECT(pCellRect->L - PAD - calloutSpace, pCellRect->MH() - (calloutSpace / 2.f), pCellRect->L - PAD,
                        pCellRect->MH() + (calloutSpace / 2.f));
            }
...
void IPopupMenuControl::Expand(const IRECT& anchorArea)
{
  ...
  else
  {
    ...
    mCalloutArrowBounds = IRECT(anchorArea.L - calloutSpace, anchorArea.MH() - (calloutSpace / 2.f), anchorArea.L,
  ...
  if (y <= minT || y > maxB || x <= minL || x > maxR) // if we're going off the top, right, left, or bottom
  {
    if ((y <= minT || x <= minL || x > maxR) && anchorArea.MH() <= mMaxBounds.MH())
    {
      ...
      mCalloutArrowBounds = IRECT(anchorArea.MW() - (calloutSpace / 2.f), anchorArea.B,
                                  anchorArea.MW() + (calloutSpace / 2.f), anchorArea.B + calloutSpace);
    ...
    if ((y > maxB || x <= minL || x > maxR) && anchorArea.MH() > mMaxBounds.MH())
    {
      ...
      mCalloutArrowBounds = IRECT(anchorArea.MW() - (calloutSpace / 2.f), anchorArea.T - calloutSpace,
                                  anchorArea.MW() + (calloutSpace / 2.f), anchorArea.T);
