 ...
 * - Works with NanoVG and Skia backends
 *
...
private:
  /** ITransport manages elapsed time vs. theoretical time based on incoming data */
  class ITransport
  {
  public:
    ...
    /** Compute time difference in milliseconds between real elapsed
        time and time computed from incoming data */
  ...
  void DrawSpectrogram(IGraphics& g)
  {
    ...
    // Create or update NanoVG image from pixel buffer
    if (mImageNeedsCreate)
    {
      if (mNVGImage != 0)
        nvgDeleteImage(vg, mNVGImage);
      mNVGImage = nvgCreateImageRGBA(vg, numBins, numRows, 0, mPixelBuffer.data());
      mImageNeedsCreate = false;
      mImageNeedsUpdate = false;
    }
    else if (mImageNeedsUpdate)
    {
      nvgUpdateImage(vg, mNVGImage, mPixelBuffer.data());
      mImageNeedsUpdate = false;
    }
    ...
    if (mDirection == EDirection::Horizontal)
    {
      if (oldPartFrac > 0.001f)
      {
        ...
        DrawImagePortionNanoVG(vg, dstRect, writePos, oldPartFrac, true);
      }
      if (newPartFrac > 0.001f)
      {
        IRECT dstRect = mRECT.GetFromRight(mRECT.W() * newPartFrac);
        DrawImagePortionNanoVG(vg, dstRect, 0.0f, newPartFrac, true);
      }
    }
    else
    {
      if (oldPartFrac > 0.001f)
      {
        ...
        DrawImagePortionNanoVG(vg, dstRect, writePos, oldPartFrac, false);
      }
      if (newPartFrac > 0.001f)
      {
        IRECT dstRect = mRECT.GetFromTop(mRECT.H() * newPartFrac);
        DrawImagePortionNanoVG(vg, dstRect, 0.0f, newPartFrac, false);
      }
    }
    ...
    SkImageInfo info = SkImageInfo::Make(numBins, numRows, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    SkPixmap pixmap(info, mPixelBuffer.data(), numBins * 4);
    ...
    if (mDirection == EDirection::Horizontal)
    {
      ...
      {
        IRECT dstRect = mRECT.GetFromLeft(mRECT.W() * oldPartFrac);
        DrawImagePortionSkia(canvas, image.get(), dstRect, writePos, oldPartFrac, true);
      }
      if (newPartFrac > 0.001f)
      {
        IRECT dstRect = mRECT.GetFromRight(mRECT.W() * newPartFrac);
        DrawImagePortionSkia(canvas, image.get(), dstRect, 0.0f, newPartFrac, true);
      }
    }
    else
    {
      ...
      {
        IRECT dstRect = mRECT.GetFromBottom(mRECT.H() * oldPartFrac);
        DrawImagePortionSkia(canvas, image.get(), dstRect, writePos, oldPartFrac, false);
      }
      if (newPartFrac > 0.001f)
      {
        IRECT dstRect = mRECT.GetFromTop(mRECT.H() * newPartFrac);
        DrawImagePortionSkia(canvas, image.get(), dstRect, 0.0f, newPartFrac, false);
      }
    }
#endif
  }
...
#if defined(IGRAPHICS_NANOVG)
  void DrawImagePortionNanoVG(NVGcontext* vg, const IRECT& dst, float srcYStart, float srcYFrac, bool rotated)
  {
    if (!rotated)
    {
      // Vertical mode: flip vertically so newest data (end of src range) appears at top
      nvgSave(vg);

      float cy = dst.T + dst.H() * 0.5f;
      nvgTranslate(vg, 0, cy);
      nvgScale(vg, 1, -1);
      nvgTranslate(vg, 0, -cy);

      float patternH = dst.H() / srcYFrac;
      float patternY = dst.T - srcYStart * patternH;
      float patternW = dst.W();
      float patternX = dst.L;

      NVGpaint paint = nvgImagePattern(vg, patternX, patternY, patternW, patternH, 0.0f, mNVGImage, 1.0f);
      nvgBeginPath(vg);
      nvgRect(vg, dst.L, dst.T, dst.W(), dst.H());
    ...
    else
    {
      ...
      float patternW = rotW;
      float patternH = rotH / srcYFrac;
      float patternX = -rotW * 0.5f;
      float patternY = -rotH * 0.5f - srcYStart * patternH;

      NVGpaint paint = nvgImagePattern(vg, patternX, patternY, patternW, patternH, 0.0f, mNVGImage, 1.0f);
      nvgBeginPath(vg);
      nvgRect(vg, -rotW * 0.5f, -rotH * 0.5f, rotW, rotH);
      nvgFillPaint(vg, paint);
      nvgFill(vg);
...
#elif defined(IGRAPHICS_SKIA)
  void DrawImagePortionSkia(SkCanvas* canvas, SkImage* image, const IRECT& dst, float srcYStart, float srcYFrac, bool rotated)
  {
    float imgW = static_cast<float>(mImageWidth);
    float imgH = static_cast<float>(mImageHeight);

    // Source rect in image coordinates (rows)
    float srcTop = srcYStart * imgH;
    float srcHeight = srcYFrac * imgH;

    SkRect srcRect = SkRect::MakeXYWH(0, srcTop, imgW, srcHeight);
    SkRect dstRect = SkRect::MakeLTRB(dst.L, dst.T, dst.R, dst.B);
    ...
    if (!rotated)
    {
      ...
      canvas->drawImageRect(image, srcRect, dstRect, sampling, nullptr, SkCanvas::kStrict_SrcRectConstraint);
    ...
    else
    {
      ...
      float rotW = dst.H();
      float rotH = dst.W();
      SkRect rotDstRect = SkRect::MakeXYWH(-rotW * 0.5f, -rotH * 0.5f, rotW, rotH);

      canvas->drawImageRect(image, srcRect, rotDstRect, sampling, nullptr, SkCanvas::kStrict_SrcRectConstraint);
  ...
  void ScaleSpectrogramData(int ch, TDataPacket& powerSpectrum)
  {
    ...
    // Start at i=1 to skip DC bin (0 Hz) which doesn't need frequency scaling
