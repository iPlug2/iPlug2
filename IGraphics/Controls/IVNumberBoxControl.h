...
{
 public:
	IVNumberBoxControl(const IRECT& bounds,
	...
	void OnAttached() override
	{
		...
		sections.Pad(-1.f, 1.f, 0.f, 1.f);
		GetUI()
			->AttachControl(mIncButton = new IVButtonControl(
								sections.FracRectVertical(0.5f, true), SplashClickActionFunc, "+", mStyle))
			->SetAnimationEndActionFunction(mIncrementFunc);
		GetUI()
			->AttachControl(mDecButton = new IVButtonControl(
								sections.FracRectVertical(0.5f, false), SplashClickActionFunc, "-", mStyle))
	...
	{
		mRealValue = math::Clamp(mRealValue, mMinValue, mMaxValue);
