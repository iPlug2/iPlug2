...
#endif
{
 public:
	...
	// TODO: OnMouseDown and OnMouseUp should be OnMouseButtonDown and OnMouseButtonUp to show intent. OnMouseDown and
	// OnMouseUp can be actual movement.
	...
	/** Called after the control has been attached, and its delegate and graphics member variable set. Use this method
	 * for controls that might need to attach sub controls that should be above their parent in the stack */
	...
	/** Called by default when the user right clicks a control. If IGRAPHICS_NO_CONTEXT_MENU is enabled as a
	 * preprocessor macro right clicking control will mean IControl::CreateContextMenu() and
	 * IControl::OnContextSelection() do not function on right clicking control. VST3 provides contextual menu support
	 * which is hard wired to right click controls by default. You can add custom items to the menu by implementing
	 * IControl::CreateContextMenu() and handle them in IControl::OnContextSelection(). In non-VST 3 hosts right
	 * clicking will still create the menu, but it will not feature entries added by the host. */
	...
	/** Implement this to customise how a colored highlight is drawn on the control in ProTools (AAX format only), when
	 * a control is linked to a parameter that is automated.
	 * @param g The graphics context to which this control belongs. */
	 ...
	 * @param valIdx An index to choose which of the controls linked parameters to retrieve. NOTE: since controls
	 * usually have only 1 parameter you can omit this argument and use the default index of 0 */
	 ...
	 * @param valIdx An index to choose which of the controls linked parameters to retrieve.
	 * NOTE: since controls usually have only 1 parameter you can omit this argument and use the default index of 0 */
	...
	inline void SetTooltip(const char* str)
	{
		mTooltip.Set(str);
	}
	 ...
	 * Normaly controls are either linked to a single parameter or no parameter
	 * but some may be linked to multiple parameters.
	 * @param valIdx An index to choose which of the controls linked parameters to retrieve. NOTE: since controls
	 * usually have only 1 parameter you can omit this argument and use the default index of 0
	...
	int NVals() const
	{
		return (int) mVals.size();
	}

	/** Check to see which of the control's values relates to this x and y coordinate
	 * @param x x coordinate to check
	 * @param y x coordinate to check
	 * @return An integer specifying which value matches the x, y coordinates,
	 *         or kNoValIdx if the position is not linked to a value. */
	 ...
	 * This method is called from the class implementing the IEditorDelegate interface in order
	 * to update a control's value members and set it to be marked dirty for redraw.
	 ...
	 * @param valIdx either an integer > -1 (kNoValIdx) in order to set an individual value to
	 * the default value of the associated parameter, or kNoValIdx to default all values.
	 * This method will call through to SetDirty(true, valIdx), which will mean that the new
	 * value gets sent back to the delegate. */
	 ...
	 * @return const IText& The control's mText object, typically used to determine
	 * font/layout/size etc of the main text in a control. */
	...
	/** Set the size of the control, preserving the current position. This may need to be overriden if you maintain
	 * custom positioning data in your control, or if your TargetRECT is not the same as the main RECT.
	...
	{
		mPromptShowsParamLabel = enable;
	}

	/** Hit test the control. Override this method if you want the control to be hit only if a visible part of it is
	 * hit, or whatever.
	...
	/** Called at each display refresh by the IGraphics draw loop, after IControl::Animate(), to determine if the
	 ...
	 * @return The class implementing the IEditorDelegate interface that handles communication to/from from this
	 * IGraphics instance.*/
	 ...
	 * @param valIdx /todo */
	virtual void SnapToMouse(float x,
							 float y,
...
#pragma mark - IControl Member variables
 protected:
	...
	/** if mGraphics::mHandleMouseOver = true, this will be true when the mouse is over control. If you need finer
	 * grained control of mouseovers, you can override OnMouseOver() and OnMouseOut() */
...
{
 public:
	IBitmapBase(const IBitmap& bitmap) : mBitmap(bitmap) {}
...
class IVectorBase
{
 public:
	...
	virtual void DrawPressableShape(
		IGraphics& g, EVShape shape, const IRECT& bounds, bool pressed, bool mouseOver, bool disabled)
	{
		switch (shape)
		{
	...
	/** /todo
	 ...
	 @return /todo */
	IRECT DrawPressableRectangle(IGraphics& g,
								 const IRECT& bounds,
								 bool pressed,
								 bool mouseOver,
								 bool disabled,
	...
	IRECT DrawPressableTriangle(
		IGraphics& g, const IRECT& bounds, bool pressed, bool mouseOver, float angle, bool disabled)
	{
		...
		else
		{
			// outer shadow
			if (mStyle.drawShadows)
				g.FillTriangle(GetColor(kSH),
							   x1 + mStyle.shadowOffset,
							   y1 + mStyle.shadowOffset,
							   x2 + mStyle.shadowOffset,
							   y2 + mStyle.shadowOffset,
							   x3 + mStyle.shadowOffset,
							   y3 + mStyle.shadowOffset,
							   &blend);
		...
		if (mStyle.showValue && !mValueInWidget)
		{
			...
			switch (mStyle.valueText.mVAlign)
			{
				...
				case EVAlign::Bottom:
				{
					...
					mWidgetBounds =
						clickableArea.GetReducedFromBottom(textRect.H()).GetScaledAboutCentre(mStyle.widgetFrac);
					break;
				}
				case EVAlign::Top:
					mValueBounds = clickableArea.GetFromTop(textRect.H()).GetMidHPadded(valueDisplayWidth);
					mWidgetBounds =
						clickableArea.GetReducedFromTop(textRect.H()).GetScaledAboutCentre(mStyle.widgetFrac);
					break;
				default:
					break;
			}
 ...
 protected:
	...
	float mSplashRadius =
		0.f;  // Modified during the default SplashClickAnimationFunc to specify the radius of the splash
 ...
 public:
	...
	virtual void AddTouch(ITouchID touchID, float x, float y, float radius)
	{
		...
		if (NTrackedTouches() < MAX_TOUCHES)
			mTrackedTouches.insert(std::make_pair(
				touchID, TrackedTouch(touchIndex, x, y, radius, std::chrono::high_resolution_clock::now())));
	}
	...
	{
		auto itr = std::find_if(mTrackedTouches.begin(), mTrackedTouches.end(), [index](auto element) {
...
{
 public:
	IKnobControlBase(const IRECT& bounds,
 ...
 protected:
	 ...
	 * This is usually the area that contains the knob handle, can override if your control contains extra elements such
	 * as labels.
	 * @return IRECT The bounds over which mouse deltas will be used to calculate the amount dragging changes the
	 * control value */
...
{
 public:
	ISliderControlBase(const IRECT& bounds,
					   int paramIdx     = kNoParameter,
...
class IVTrackControlBase : public IControl, public IVectorBase
{
 public:
		...
		, mDirection(dir)
	{
		...
		if (trackNames.size())
		{
			assert(trackNames.size() ==
				   maxNTracks);  // check that the trackNames list size matches the number of tracks
...
{
 public:
	ISwitchControlBase(const IRECT& bounds,
					   int paramIdx       = kNoParameter,
					   IActionFunction aF = nullptr,
					   int numStates      = 2);
	...
	{
		return static_cast<int>(0.5 + GetValue() * (static_cast<double>(mNumStates) - 1.0));
	}
...
{
 public:
	IDirBrowseControlBase(const IRECT& bounds, const char* extension /* e.g. ".txt"*/) : IControl(bounds)
	{
...
class IBitmapControl : public IControl, public IBitmapBase
{
 public:
	...
	IBitmapControl(const IRECT& bounds,
...
{
 public:
	ITextControl(const IRECT& bounds,
				 const char* str          = "",
...
{
 public:
	ITextToggleControl(const IRECT& bounds,
					   int paramIdx          = kNoParameter,
...
class ICaptionControl : public ITextControl
{
 public:
	...
	ICaptionControl(const IRECT& bounds,
					int paramIdx,
					const IText& text     = DEFAULT_TEXT,
					const IColor& BGColor = DEFAULT_BGCOLOR,
