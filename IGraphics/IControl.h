...
/**
 * @file
 * @brief This file contains the base IControl implementation, along with some base classes for specific types of
 * control.
 */
...
public:
   ...
   * @param actionFunc pass in a lambda function to provide custom functionality when the control "action" happens
   * (usually mouse down). */
  ...
  /** Implement this method to respond to a mouseover event on this control. Implementations should call base class, if
   * you wish to use mMouseIsOver.
   * @param x The X coordinate of the mouse event
   * @param y The Y coordinate of the mouse event
   * @param mod A struct indicating which modifier keys are held for the event */
  virtual void OnMouseOver(float x, float y, const IMouseMod& mod);

  /** Implement this method to respond to a mouseout event on this control. Implementations should call base class, if
   * you wish to use mMouseIsOver. */
  ...
  /** Called when IControl is constructed or resized using SetRect(). NOTE: if you call SetDirty() in this method, you
   * should call SetDirty(false) to avoid triggering parameter changes */
  virtual void OnResize() {}

  /** Called just prior to when the control is attached, after its delegate and graphics member variable set */
  virtual void OnInit() {}

  /** Called after the control has been attached, and its delegate and graphics member variable set. Use this method for
   * controls that might need to attach sub controls that should be above their parent in the stack */
  virtual void OnAttached() {}

  /** Implement to receive messages sent to the control, see IEditorDelegate:SendControlMsgFromDelegate() */
  virtual void OnMsgFromDelegate(int msgTag, int dataSize, const void* pData){};

  /** Implement to receive MIDI messages sent to the control if mWantsMidi == true, see
   * IEditorDelegate:SendMidiMsgFromDelegate() */
   ...
   * macro right clicking control will mean IControl::CreateContextMenu() and IControl::OnContextSelection() do not
   * function on right clicking control. VST3 provides contextual menu support which is hard wired to right click
   * controls by default. You can add custom items to the menu by implementing IControl::CreateContextMenu() and handle
   * them in IControl::OnContextSelection(). In non-VST 3 hosts right clicking will still create the menu, but it will
   * not feature entries added by the host. */
  ...
  /** Implement this to customise how a colored highlight is drawn on the control in ProTools (AAX format only), when a
   * control is linked to a parameter that is automated.
   * @param g The graphics context to which this control belongs. */
  virtual void DrawPTHighlight(IGraphics& g);

  /** Call this method in response to a mouse event to create an edit box so the user can enter a value, or pop up a
   * pop-up menu, if the control is linked to a parameter (mParamIdx > kNoParameter)
   * @param valIdx An index to choose which of the control's linked parameters to retrieve. NOTE: since controls usually
   * have only 1 parameter you can omit this argument and use the default index of 0
   */
  void PromptUserInput(int valIdx = 0);

  /** Create a text entry box so the user can enter a value, or pop up a pop-up menu, if the control is linked to a
   * parameter (mParamIdx > kNoParameter) specifying the bounds
   ...
   * Normaly controls are either linked to a single parameter or no parameter but some may be linked to multiple
   * parameters
   * @param valIdx An index to choose which of the control's linked parameters to retrieve. NOTE: since controls usually
   * have only 1 parameter you can omit this argument and use the default index of 0
   ...
   * If you are calling this "manually" to reuse a control for multiple parameters, you probably want to call
   * IEditorDelegate::SendCurrentParamValuesFromDelegate() afterward, to update the control values
   ...
   * @return An integer specifying which value matches the x, y coordinates, or kNoValIdx if the position is not linked
   * to a value. */
   ...
   * This method is called from the class implementing the IEditorDelegate interface in order to update a control's
   * value members and set it to be marked dirty for redraw.
   ...
   * This method is called after a text entry or popup menu prompt triggered by PromptUserInput(), calling
   * SetDirty(true), which will mean that the new value gets sent back to the delegate
   ...
   * @param valIdx either an integer > -1 (kNoValIdx) in order to set an individual value to the default value of the
   * associated parameter, or kNoValIdx to default all values This method will call through to SetDirty(true, valIdx),
   * which will mean that the new value gets sent back to the delegate */
   ...
   * @return const IText& The control's mText object, typically used to determine font/layout/size etc of the main text
   * in a control. */
  ...
  /** Set the position of the control, preserving the width and height. This may need to be overriden if you maintain
   * custom positioning data in your control
  ...
  /** Set the size of the control, preserving the current position. This may need to be overriden if you maintain custom
   * positioning data in your control, or if your TargetRECT is not the same as the main RECT.
   ...
   * which maps double click to single click for this control (and also causes the mouse to be captured by the control
   * on double click).
  ...
  /** Hit test the control. Override this method if you want the control to be hit only if a visible part of it is hit,
   * or whatever.
   ...
   * notify the class implementing the IEditorDelegate interface that the parameter changed. If this control has an
   * ActionFunction, that can also be triggered. NOTE: it is easy to forget that this method always sets the control
   * dirty, the argument refers to whether a consecutive action should be performed */
  ...
  /** Called at each display refresh by the IGraphics draw loop, after IControl::Animate(), to determine if the control
   * is marked as dirty.
  ...
  /** Gets a pointer to the class implementing the IEditorDelegate interface that handles parameter changes from this
   * IGraphics instance. If you need to call other methods on that class, you can use
   * static_cast<PLUG_CLASS_NAME>(GetDelegate();
   * @return The class implementing the IEditorDelegate interface that handles communication to/from from this IGraphics
   * instance.*/
  ...
  /** This can be used in IControl::Draw() to check if the mouse is over the control, without implementing mouse over
   * methods. Note that this method is only enabled if IGraphics::EnableMouseOver() is set to \c true
  ...
  virtual void SnapToMouse(float x, float y, EDirection direction, const IRECT& bounds, int valIdx = -1,
                           double minClip = 0., double maxClip = 1.);
...
protected:
   ...
   * @param valIdx If this is > kNoValIdx execute the function for an individual value. If equal to kNoValIdx call the
  ...
  /** if mGraphics::mHandleMouseOver = true, this will be true when the mouse is over control. If you need finer grained
   * control of mouseovers, you can override OnMouseOver() and OnMouseOut() */
...
/** A base interface to be combined with IControl for vectorial controls "IVControls", in order for them to share a
 * common style If you need more flexibility, you're on your own! */
class IVectorBase
{
public:
  ...
  virtual void DrawPressableShape(IGraphics& g, EVShape shape, const IRECT& bounds, bool pressed, bool mouseOver,
                                  bool disabled)
  {
    switch (shape)
    {
      case EVShape::Ellipse: DrawPressableEllipse(g, bounds, pressed, mouseOver, disabled); break;
      case EVShape::Rectangle: DrawPressableRectangle(g, bounds, pressed, mouseOver, disabled); break;
      case EVShape::Triangle: DrawPressableTriangle(g, bounds, pressed, mouseOver, mStyle.angle, disabled); break;
                              ...
                              bool disabled)
  {
    ...
    else
    {
      // outer shadow
      if (mStyle.drawShadows)
        g.FillTriangle(GetColor(kSH), x1 + mStyle.shadowOffset, y1 + mStyle.shadowOffset, x2 + mStyle.shadowOffset,
                       y2 + mStyle.shadowOffset, x3 + mStyle.shadowOffset, y3 + mStyle.shadowOffset, &blend);
...
public:
  ...
  virtual void AddTouch(ITouchID touchID, float x, float y, float radius)
  {
    ...
    if (NTrackedTouches() < MAX_TOUCHES)
      mTrackedTouches.insert(
        std::make_pair(touchID, TrackedTouch(touchIndex, x, y, radius, std::chrono::high_resolution_clock::now())));
  }
  ...
  TrackedTouch* GetTouch(int index)
  {
    auto itr = std::find_if(mTrackedTouches.begin(), mTrackedTouches.end(),
                            [index](auto element) { return (element.second.index == index); });
...
{
public:
  IKnobControlBase(const IRECT& bounds, int paramIdx = kNoParameter, EDirection direction = EDirection::Vertical,
...
protected:
  /** Get the area for which mouse deltas will be used to calculate the amount dragging changes the control value. This
   * is usually the area that contains the knob handle, can override if your control contains extra elements such as
   * labels
   * @return IRECT The bounds over which mouse deltas will be used to calculate the amount dragging changes the control
   * value */
...
{
public:
  ISliderControlBase(const IRECT& bounds, int paramIdx = kNoParameter, EDirection dir = EDirection::Vertical,
                     double gearing = DEFAULT_GEARING, float handleSize = 0.f);
  ISliderControlBase(const IRECT& bounds, IActionFunction aF = nullptr, EDirection dir = EDirection::Vertical,
                     double gearing = DEFAULT_GEARING, float handleSize = 0.f);
...
{
public:
  IVTrackControlBase(const IRECT& bounds, const char* label, const IVStyle& style, int maxNTracks = 1, int nSteps = 0,
                     EDirection dir = EDirection::Horizontal, std::initializer_list<const char*> trackNames = {})
  : IControl(bounds)
  , IVectorBase(style)
  , mDirection(dir)
  , mNSteps(nSteps)
  ...
  IVTrackControlBase(const IRECT& bounds, const char* label, const IVStyle& style, int lowParamidx, int maxNTracks = 1,
                     int nSteps = 0, EDirection dir = EDirection::Horizontal,
  ...
  IVTrackControlBase(const IRECT& bounds, const char* label, const IVStyle& style,
                     const std::initializer_list<int>& params, int nSteps = 0, EDirection dir = EDirection::Horizontal,
                     std::initializer_list<const char*> trackNames = {})
  : IControl(bounds)
  , IVectorBase(style)
  , mDirection(dir)
   ...
   * You probably want to call IEditorDelegate::SendCurrentParamValuesFromDelegate() after this, to update the control
   * values */
...
protected:
  ...
  virtual void DrawTrack(IGraphics& g, const IRECT& r, int chIdx)
  {
    ...
    if (bv > 0.f)
    {
      ...
      {
        fillRect = IRECT(r.L, trackPos < bv ? r.B - r.H() * bv : r.B - r.H() * trackPos, r.R,
                         trackPos < bv ? r.B - r.H() * trackPos : r.B - r.H() * bv);
      }
      else
      {
        fillRect = IRECT(trackPos < bv ? r.L + r.W() * trackPos : r.L + r.W() * bv, r.T,
                         trackPos < bv ? r.L + r.W() * bv : r.L + r.W() * trackPos, r.B);
      }
    }
    else
    {
    ...
    else
    {
      ...
      if (mDirection == EDirection::Vertical)
      {
        peakRect = IRECT(fillRect.L, trackPos < mBaseValue ? fillRect.B : fillRect.T, fillRect.R,
                         trackPos < mBaseValue ? fillRect.B - mPeakSize : fillRect.T + mPeakSize);
      }
      else
      {
        peakRect = IRECT(trackPos < mBaseValue ? fillRect.L + mPeakSize : fillRect.R - mPeakSize, fillRect.T,
                         trackPos < mBaseValue ? fillRect.L : fillRect.R, fillRect.B);
      }
  ...
  virtual void MakeTrackRects(const IRECT& bounds)
  {
    ...
    {
      mTrackBounds.Get()[ch] =
        bounds.SubRect(EDirection(!dir), nVals, ch)
 ...
 * The default action function triggers the default click function, which returns mValue to 0. after
...
/** An abstract IControl base class that you can inherit from in order to make a control that pops up a menu to browse
 * files */
...
public:
  ...
  virtual void OnEndAnimation()
    override // if you override this you must call the base implementation, to free mAnimationFunc
  {
...
public:
  ...
  IBitmapControl(const IRECT& bounds, const IBitmap& bitmap, int paramIdx = kNoParameter,
...
{
public:
  ITextControl(const IRECT& bounds, const char* str = "", const IText& text = DEFAULT_TEXT,
               const IColor& BGColor = DEFAULT_BGCOLOR, bool setBoundsBasedOnStr = false);
...
{
public:
  IURLControl(const IRECT& bounds, const char* str, const char* url, const IText& text = DEFAULT_TEXT,
              const IColor& BGColor = DEFAULT_BGCOLOR, const IColor& MOColor = COLOR_WHITE,
...
{
public:
  ITextToggleControl(const IRECT& bounds, int paramIdx = kNoParameter, const char* offText = "OFF",
                     const char* onText = "ON", const IText& text = DEFAULT_TEXT,
                     const IColor& BGColor = DEFAULT_BGCOLOR);

  ITextToggleControl(const IRECT& bounds, IActionFunction aF = nullptr, const char* offText = "OFF",
                     const char* onText = "ON", const IText& text = DEFAULT_TEXT,
                     const IColor& BGColor = DEFAULT_BGCOLOR);
...
public:
  ...
  ICaptionControl(const IRECT& bounds, int paramIdx, const IText& text = DEFAULT_TEXT,
                  const IColor& BGColor = DEFAULT_BGCOLOR, bool showParamLabel = true);
  void Draw(IGraphics& g) override;
  void OnMouseDown(float x, float y, const IMouseMod& mod) override;
  void OnResize() override;
...
public:
  ...
  {
    GetUI()->CreateTextEntry(*this, mText, mRECT, mStr.Get());
  }
