...
class IGraphicsNanoVG : public IGraphicsPathBase
{
 ...
 public:
	...
	void DrawDottedLine(const IColor& color,
						float x1,
						float y1,
						float x2,
						float y2,
	...
	void DrawDottedRect(
		const IColor& color, const IRECT& bounds, const IBlend* pBlend, float thickness, float dashLen) override;
	...
	void DrawFastDropShadow(const IRECT& innerBounds,
							...
							float roundness = 0.f,
							float blur      = 10.f,
							IBlend* pBlend  = nullptr) override;
