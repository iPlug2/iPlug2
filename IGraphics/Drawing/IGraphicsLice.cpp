...
{
  DrawText(DEFAULT_TEXT, "UNSUPPORTED", const_cast<IRECT&>(bounds), nullptr, false, false);
}
...
void IGraphicsLice::DrawRotatedSVG(ISVG& svg, float destCtrX, float destCtrY, float width, float height, double angle, const IBlend* pBlend)
{
  ...
  DrawText(DEFAULT_TEXT, "UNSUPPORTED", r, nullptr, false, false);
}
...
bool IGraphicsLice::DrawText(const IText& text, const char* str, IRECT& bounds, const IBlend* pBlend, bool measure, bool textEntry)
{
  const float ds = GetDisplayScale();
  if (!str || str[0] == '\0')
  {
