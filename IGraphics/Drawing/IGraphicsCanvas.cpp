...
void IGraphicsCanvas::DrawBitmap(const IBitmap& bitmap, const IRECT& bounds, int srcX, int srcY, const IBlend* pBlend)
{
  ...
  context.call<void>(
    "drawImage", img, srcX * bs, srcY * bs, sr.W(), sr.H(), bounds.L, bounds.T, bounds.W(), bounds.H());
...
void IGraphicsCanvas::PathStroke(const IPattern& pattern, float thickness, const IStrokeOptions& options,
                                 const IBlend* pBlend)
{
  val context = GetContext();
...
void IGraphicsCanvas::PrepareAndMeasureText(const IText& text, const char* str, IRECT& r, double& x, double& y) const
{
  ...
  std::string fontString =
    GetFontString(descriptor->first.Get(), descriptor->second.Get(), text.mSize * pFont->mEMRatio);
...
{
  return !CompareFontMetrics(style, font, "monospace") || !CompareFontMetrics(style, font, "sans-serif")
...
bool IGraphicsCanvas::LoadAPIFont(const char* fontID, const PlatformFontPtr& font)
{
  ...
  if (!font->IsSystem())
  {
    ...
    if (data->IsValid())
    {
      ...
      const double ascenderRatio =
        data->GetAscender() / static_cast<double>(data->GetAscender() - data->GetDescender());
