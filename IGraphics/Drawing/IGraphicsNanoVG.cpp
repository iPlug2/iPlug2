...
IGraphicsNanoVG::Bitmap::Bitmap(IGraphicsNanoVG* pGraphics, NVGcontext* pContext, int width, int height, float scale,
                                float drawScale)
{
  mGraphics = pGraphics;
  mVG = pContext;
...
IGraphicsNanoVG::Bitmap::Bitmap(NVGcontext* pContext, int width, int height, const uint8_t* pData, float scale,
                                float drawScale)
{
...
void IGraphicsNanoVG::DrawResize()
{
  ...
  {
    mMainFrameBuffer =
      nvgCreateFramebuffer(mVG, WindowWidth() * GetScreenScale(), WindowHeight() * GetScreenScale(), 0);
...
void IGraphicsNanoVG::PathStroke(const IPattern& pattern, float thickness, const IStrokeOptions& options,
                                 const IBlend* pBlend)
{
  // First set options
...
void IGraphicsNanoVG::PathFill(const IPattern& pattern, const IFillOptions& options, const IBlend* pBlend)
{
  switch (options.mFillRule)
  {
    ...
    case EFillRule::Winding: nvgPathWinding(mVG, NVG_CCW); break;
    case EFillRule::EvenOdd: nvgPathWinding(mVG, NVG_CW); break;
...
void IGraphicsNanoVG::UpdateLayer()
{
  ...
  else
  {
    ...
    nvgBeginFrame(mVG, mLayers.top()->Bounds().W() * GetDrawScale(), mLayers.top()->Bounds().H() * GetDrawScale(),
                  GetScreenScale());
  }
}
...
void IGraphicsNanoVG::DrawDottedLine(const IColor& color, float x1, float y1, float x2, float y2, const IBlend* pBlend,
...
void IGraphicsNanoVG::DrawDottedRect(const IColor& color, const IRECT& bounds, const IBlend* pBlend, float thickness,
                                     float dashLen)
{
...
void IGraphicsNanoVG::DrawFastDropShadow(const IRECT& innerBounds, const IRECT& outerBounds, float xyDrop,
                                         float roundness, float blur, IBlend* pBlend)
{
  NVGpaint shadowPaint =
    nvgBoxGradient(mVG, innerBounds.L + xyDrop, innerBounds.T + xyDrop, innerBounds.W(), innerBounds.H(), roundness,
                   blur, NanoVGColor(COLOR_BLACK_DROP_SHADOW, pBlend), NanoVGColor(COLOR_TRANSPARENT, nullptr));
  nvgBeginPath(mVG);
