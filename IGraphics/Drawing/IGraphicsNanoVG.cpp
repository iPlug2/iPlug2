...
BEGIN_INCLUDE_DEPENDENCIES
		...
		//  #define NANOVG_GLES2_IMPLEMENTATION
		//#else
		//  #error Define either IGRAPHICS_GLES2 or IGRAPHICS_GLES3 when using IGRAPHICS_GL and IGRAPHICS_NANOVG with
		//  OS_IOS
		//#endif
...
IGraphicsNanoVG::Bitmap::Bitmap(NVGcontext* pContext, const char* path, double sourceScale, int nvgImageID, bool shared)
{
	...
	SetBitmap(nvgImageID,
			  w,
			  h,
			  static_cast<int>(sourceScale),
			  1.f);  // TODO: sourceScale is a double, but SetBitmap takes an int. Is that intended?
}

IGraphicsNanoVG::Bitmap::Bitmap(
	IGraphicsNanoVG* pGraphics, NVGcontext* pContext, int width, int height, int scale, float drawScale)
{
	mGraphics = pGraphics;
	mVG       = pContext;
...
IGraphicsNanoVG::Bitmap::Bitmap(
	NVGcontext* pContext, int width, int height, const uint8_t* pData, int scale, float drawScale)
{
...
NVGpaint NanoVGPaint(NVGcontext* pContext, const IPattern& pattern, const IBlend* pBlend)
{
	...
	if (pattern.mType == EPatternType::Radial)
	{
		return nvgRadialGradient(
			pContext, s[0], s[1], inverse.mXX * pattern.GetStop(0).mOffset, inverse.mXX, icol, ocol);
	}
	else
	{
...
bool IGraphicsNanoVG::BitmapExtSupported(const char* ext)
{
	...
	return (strstr(extLower, "png") != nullptr) || (strstr(extLower, "jpg") != nullptr) ||
...
APIBitmap* IGraphicsNanoVG::CreateAPIBitmap(int width, int height, int scale, float drawScale, bool cacheable)
{
	...
	if (mInDraw)
	{
		...
		nvgBeginFrame(mVG, GetWindowWidth(), GetWindowHeight(), GetScreenScale());
	}
...
{
#if defined IGRAPHICS_METAL
	mVG = nvgCreateContext(pContext,
						   NVG_ANTIALIAS | NVG_TRIPLE_BUFFER);  // TODO: NVG_STENCIL_STROKES currently has issues
#else
...
void IGraphicsNanoVG::DrawResize()
{
	...
	{
		mMainFrameBuffer =
			nvgCreateFramebuffer(mVG, GetWindowWidth() * GetScreenScale(), GetWindowHeight() * GetScreenScale(), 0);
...
#ifdef IGRAPHICS_GL
	glViewport(0, 0, GetWindowWidth() * GetScreenScale(), GetWindowHeight() * GetScreenScale());
...
void IGraphicsNanoVG::EndFrame()
{
	...
	NVGpaint img = nvgImagePattern(mVG, 0, 0, GetWindowWidth(), GetWindowHeight(), 0, mMainFrameBuffer->image, 1.0f);

	nvgSave(mVG);
	nvgResetTransform(mVG);
	nvgTranslate(mVG, mXTranslation, mYTranslation);
	nvgBeginPath(mVG);
	nvgRect(mVG, 0, 0, GetWindowWidth(), GetWindowHeight());
...
void IGraphicsNanoVG::UpdateLayer()
{
	...
	else
	{
		...
		nvgBeginFrame(mVG,
					  mLayers.top()->Bounds().W() * GetDrawScale(),
					  mLayers.top()->Bounds().H() * GetDrawScale(),
					  GetScreenScale());
	}
}
...
void IGraphicsNanoVG::DrawDottedLine(
	const IColor& color, float x1, float y1, float x2, float y2, const IBlend* pBlend, float thickness, float dashLen)
{
...
void IGraphicsNanoVG::DrawDottedRect(
	const IColor& color, const IRECT& bounds, const IBlend* pBlend, float thickness, float dashLen)
{
	...
	for (int j = 0; j < 2; j++)
	{
		...
		{
			x2 = math::Clamp(x1 + dashLen, bounds.L, bounds.R);
			PathLineTo(x2, y2);
			x1 = math::Clamp(x2 + dashLen, bounds.L, bounds.R);
			PathMoveTo(x1, y1);
		}
		...
		{
			y2 = math::Clamp(y1 + dashLen, bounds.T, bounds.B);
			PathLineTo(x2, y2);
			y1 = math::Clamp(y2 + dashLen, bounds.T, bounds.B);
			PathMoveTo(x1, y1);
		}
...
void IGraphicsNanoVG::DrawFastDropShadow(
	const IRECT& innerBounds, const IRECT& outerBounds, float xyDrop, float roundness, float blur, IBlend* pBlend)
{
	NVGpaint shadowPaint = nvgBoxGradient(mVG,
										  ...
										  roundness,
										  blur,
										  NanoVGColor(COLOR_BLACK_DROP_SHADOW, pBlend),
										  NanoVGColor(COLOR_TRANSPARENT, nullptr));
	nvgBeginPath(mVG);
