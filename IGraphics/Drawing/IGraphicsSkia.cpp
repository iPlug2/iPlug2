...
{
	Font(IFontDataPtr&& data, sk_sp<SkTypeface> typeFace) : mData(std::move(data)), mTypeface(typeFace) {}
		...
		switch (pattern.mType)
		{
			case EPatternType::Linear:
				paint.setShader(SkGradientShader::MakeLinear(
					points, colors, positions, numStops, SkiaTileMode(pattern), 0, nullptr));
				break;
			...
			case EPatternType::Radial:
			{
				...
				paint.setShader(SkGradientShader::MakeRadial(
					points[0], radius, colors, positions, numStops, SkiaTileMode(pattern), 0, nullptr));
				break;
			}
			...
			case EPatternType::Sweep:
			{
				...
				paint.setShader(SkGradientShader::MakeSweep(x1,
															y1,
															colors,
															nullptr,
															numStops,
...
//
//    sk_sp<SkImage> image = SkImage::MakeFromTexture(mGrContext.get(), grBackendTexture, kTopLeft_GrSurfaceOrigin,
//    kBGRA_8888_SkColorType, kOpaque_SkAlphaType, nullptr); return new Bitmap(image, scale);
//  }
//  else
//#endif
#if PLATFORM_WINDOWS
...
void IGraphicsSkia::DrawResize()
{
	...
	const size_t bmpSize = sizeof(BITMAPINFOHEADER) +
						   (WindowWidth() * GetScreenScale()) * (WindowHeight() * GetScreenScale()) * sizeof(uint32_t);
...
void IGraphicsSkia::BeginFrame()
{
	...
	if (mGrContext.get())
	{
		...
		mScreenSurface = SkSurface::MakeFromBackendRenderTarget(
			mGrContext.get(), backendRT, kBottomLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, nullptr, nullptr);
	...
	if (mGrContext.get())
	{
		...
		mScreenSurface = SkSurface::MakeFromBackendRenderTarget(
			mGrContext.get(), backendRT, kTopLeft_GrSurfaceOrigin, kBGRA_8888_SkColorType, nullptr, nullptr);
...
void IGraphicsSkia::PrepareAndMeasureText(
	const IText& text, const char* str, IRECT& r, double& x, double& y, SkFont& font) const
{
	...
	switch (text.mVAlign)
	{
		case EVAlign::Top:
			...
			break;
		case EVAlign::Middle:
			y = r.MH() - descender + (textHeight / 2.0);
			break;
		case EVAlign::Bottom:
			y = r.B - descender;
			break;
...
void IGraphicsSkia::DrawRoundRect(
	const IColor& color, const IRECT& bounds, float cornerRadius, const IBlend* pBlend, float thickness)
{
	auto paint = SkiaPaint(color, pBlend);
	paint.setStyle(SkPaint::Style::kStroke_Style);
	paint.setStrokeWidth(thickness);
...
void IGraphicsSkia::DrawArc(
	const IColor& color, float cx, float cy, float r, float a1, float a2, const IBlend* pBlend, float thickness)
{
	auto paint = SkiaPaint(color, pBlend);
	paint.setStyle(SkPaint::Style::kStroke_Style);
	paint.setStrokeWidth(thickness);
...
{
	mCanvas =
		mLayers.empty() ? mSurface->getCanvas() : mLayers.top()->GetAPIBitmap()->GetBitmap()->mSurface->getCanvas();
}
...
void IGraphicsSkia::DrawFastDropShadow(
