...
public:
  ...
  void OnInit() override { GetUI()->EnableMouseOver(true); }
  ...
  void OnMouseDrag(float x, float y, float dX, float dY, const IMouseMod& mod) override
  {
    ...
    if (mClickedOnControl > 0)
    {
      ...
      if (mMouseClickedOnResizeHandle)
      {
        ...
        if (r.R < mMouseDownRECT.L + mGridSize)
          r.R = mMouseDownRECT.L + mGridSize;
        if (r.B < mMouseDownRECT.T + mGridSize)
          r.B = mMouseDownRECT.T + mGridSize;
