...
#ifndef NO_IGRAPHICS
	#if defined(IGRAPHICS_AGG) + defined(IGRAPHICS_CAIRO) + defined(IGRAPHICS_NANOVG) + defined(IGRAPHICS_LICE) + \
			defined(IGRAPHICS_CANVAS) + defined(IGRAPHICS_SKIA) !=                                                \
		1
...
#pragma mark - Drawing API implementation
	...
	/** Called by some drawing API classes to finally blit the draw bitmap onto the screen or perform other cleanup
	 * after drawing */
	 ...
	 * @param destCentreX The X coordinate in the graphics context of the centre point at which to rotate the image
	 * around. \todo check this
	 * @param destCentreY The Y coordinate in the graphics context of the centre point at which to rotate the image
	 * around. \todo check this
	 * @param width \todo
	 * @param height \todo
	 * @param angle The angle to rotate the bitmap mask at in degrees clockwise
	 * @param pBlend Optional blend method */
	virtual void DrawRotatedSVG(const ISVG& svg,
								float destCentreX,
								float destCentreY,
								float width,
								float height,
	...
	virtual void DrawBitmap(
		const IBitmap& bitmap, const IRECT& bounds, int srcX, int srcY, const IBlend* pBlend = 0) = 0;
	 ...
	 * @param pBlend Optional blend method */
	virtual void DrawRotatedBitmap(const IBitmap& bitmap,
								   float destCentreX,
								   float destCentreY,
								   double angle,
								   int yOffsetZeroDeg   = 0,
								   const IBlend* pBlend = 0) = 0;
	...
	virtual void DrawLine(const IColor& color,
						  float x1,
						  float y1,
						  float x2,
						  float y2,
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawDottedLine(const IColor& color,
								float x1,
								float y1,
								float x2,
								float y2,
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawTriangle(const IColor& color,
							  float x1,
							  float y1,
							  float x2,
							  float y2,
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawRect(const IColor& color,
						  const IRECT& bounds,
						  const IBlend* pBlend = 0,
						  float thickness      = 1.f) = 0;
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawRoundRect(const IColor& color,
							   const IRECT& bounds,
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawArc(const IColor& color,
						 float cx,
						 float cy,
						 float r,
						 float a1,
	...
	virtual void DrawCircle(
		const IColor& color, float cx, float cy, float r, const IBlend* pBlend = 0, float thickness = 1.f) = 0;
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawEllipse(const IColor& color,
							 const IRECT& bounds,
							 const IBlend* pBlend = 0,
							 float thickness      = 1.f) = 0;
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawConvexPolygon(
		const IColor& color, float* x, float* y, int nPoints, const IBlend* pBlend = 0, float thickness = 1.f) = 0;
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawDottedRect(const IColor& color,
								const IRECT& bounds,
								const IBlend* pBlend = 0,
								float thickness      = 1.f,
								float dashLen        = 2.f) = 0;
	...
	virtual void FillTriangle(
		const IColor& color, float x1, float y1, float x2, float y2, float x3, float y3, const IBlend* pBlend = 0) = 0;
	 ...
	 * @param cornerRadius The corner radius in pixels
	 * @param pBlend Optional blend method */
	virtual void FillRoundRect(const IColor& color,
							   const IRECT& bounds,
							   float cornerRadius   = 5.f,
							   const IBlend* pBlend = 0) = 0;
	...
	virtual void FillEllipse(
		const IColor& color, float x, float y, float r1, float r2, float angle = 0.0, const IBlend* pBlend = 0) = 0;
	...
	virtual void FillArc(
		const IColor& color, float cx, float cy, float r, float a1, float a2, const IBlend* pBlend = 0) = 0;
	 ...
	 * @param text An IText struct containing font and text properties and layout info
	 * @param str The text string to draw in the graphics context
	 * @param bounds after calling the method this IRECT will be updated with the rectangular region the text will
	 * occupy */
...
#pragma mark - Base implementation - drawing helpers
	...
	void DrawVerticalLine(
		const IColor& color, const IRECT& bounds, float x, const IBlend* pBlend = 0, float thickness = 1.f);
	...
	void DrawHorizontalLine(
		const IColor& color, const IRECT& bounds, float y, const IBlend* pBlend = 0, float thickness = 1.f);
	 ...
	 * @param thickness Optional line thickness */
	void DrawVerticalLine(
		const IColor& color, float xi, float yLo, float yHi, const IBlend* pBlend = 0, float thickness = 1.f);
	 ...
	 * @param thickness Optional line thickness */
	void DrawHorizontalLine(
		const IColor& color, float yi, float xLo, float xHi, const IBlend* pBlend = 0, float thickness = 1.f);
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	void DrawRadialLine(const IColor& color,
						float cx,
						float cy,
						float angle,
						float rMin,
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawGrid(const IColor& color,
						  const IRECT& bounds,
						  float gridSizeH,
						  float gridSizeV,
						  const IBlend* pBlend = 0,
	 ...
	 * @param pBlend Optional blend method
	 * @param thickness Optional line thickness */
	virtual void DrawData(const IColor& color,
						  const IRECT& bounds,
						  float* normYPoints,
						  int nPoints,
...
#pragma mark - Drawing API path support
 public:
	...
	virtual bool HasPathSupport() const
	{
		return false;
	}
	 ...
	 * @param pBlend Optional blend method */
	virtual void PathStroke(const IPattern& pattern,
							float thickness,
...
#pragma mark - Platform implementation
	 ...
	 * @return \todo check */
	virtual EMsgBoxResult ShowMessageBox(const char* str,
										 const char* caption,
										 EMsgBoxType type,
										 IMsgBoxCompletionHanderFunc completionHandler = nullptr) = 0;

	/** Create a platform file prompt dialog to choose a file/directory path for opening/saving a file/directory. NOTE:
	 ...
	 * save dialogs, to provide a default file name. For load dialogs, on successful selection of a file this will get
	 ...
	 * @param extensions A comma separated CString list of file extensions to filter in the dialog (e.g. “.wav, .aif”
	 * \todo check */
	virtual void PromptForFile(WDL_String& fileName,
							   WDL_String& path,
							   EFileAction action     = EFileAction::Open,
							   const char* extensions = 0) = 0;

	/** Create a platform file prompt dialog to choose a directory path for opening/saving a directory. NOTE: this
	 * method will block the main thread
	 * @param dir Non const WDL_String reference specifying the directory path. Set this prior to calling the method for
	 * save dialogs, to provide a default path. For load dialogs, on successful selection of a directory this will get
	...
	/** @return a void pointer that can be cast back to HINSTANCE to get the module handle on windows, returns nullptr
	 * on other platforms */
	...
	virtual void SetPlatformContext(void* pContext)
	{
 ...
 protected:
	 ...
	 * @param bounds /todo
	 * @param str /todo */
	virtual void CreatePlatformTextEntry(
		int paramIdx, const IText& text, const IRECT& bounds, int length, const char* str) = 0;
...
#pragma mark - Base implementation
 public:
	 ...
	 * @param control A reference to the IControl creating this pop-up menu. If it exists
	 * IControl::OnPopupMenuSelection() will be called on successful selection
	 ...
	 * @param control The control that the text entry belongs to. If this control is linked to a parameter, the text
	 * entry will be configured with initial text matching the parameter value
	...
	void CreateTextEntry(
		IControl& control, const IText& text, const IRECT& bounds, const char* str = "", int valIdx = 0);
	...
	inline constexpr int GetWindowWidth() const
	{
		return static_cast<int>(static_cast<float>(mWidth) * mDrawScale);
	}
	...
	inline constexpr int GetWindowHeight() const
	{
		return static_cast<int>(static_cast<float>(mHeight) * mDrawScale);
	}
	...
	inline constexpr int GetFPS() const
	{
		return mFPS;
	}
	...
	{
		return static_cast<float>(mDrawScale * static_cast<float>(mScreenScale));
	}
	...
	{
		return r.GetPixelSnapped(GetBackingPixelScale());
	}
	...
	inline bool TooltipsEnabled() const
	{
		return mEnableTooltips;
	}

	/** @return An EUIResizerMode Representing whether the graphics context should scale or be resized, e.g. when
	 * dragging a corner resizer */
	...
	{
		return IRECT(0.f, 0.f, (float) GetWidth(), (float) GetHeight());
	}
	...
	virtual int GetPlatformWindowScale() const
	{
		return 1;
	}
...
#pragma mark - Control management
 public:
	...
	/** Attach an IBitmapControl as the lowest IControl in the control stack to be the background for the graphics
	 * context
	...
	void AttachCornerResizer(EUIResizerMode sizeMode      = EUIResizerMode::Scale,
	 ...
	 * @param sizeMode Choose whether to scale or size the UI */
	void AttachCornerResizer(ICornerResizerControl* pControl,
	...
	void ShowBubbleControl(IControl* pCaller,
						   float x,
						   float y,
						   const char* str,
	...
	{
		return mPerfDisplay != nullptr;
	}

	/** Attach an IControl to the graphics context and add it to the top of the control stack. The control is owned by
	 * the graphics context and will be deleted when the context is deleted.
	...
	{
		return std::find_if(std::begin(mCapturedMap), std::end(mCapturedMap), [pControl](auto&& press) {
				   return press.second == pControl;
			   }) != mCapturedMap.end();
	}
...
#pragma mark - Event handling
 public:
	...
	/** @param enable Set \c true if you want to handle mouse over messages. Note: this may increase the amount CPU
	 * usage if you redraw on mouse overs etc */
	void EnableMouseOver(bool enable)
	{
		mEnableMouseOver = enable;
	}
	...
	{
		return mEnableMouseOver;
	}

	/** @return An integer representing the control index in IGraphics::mControls which the mouse is over, or -1 if it
	 * is not */
	...
	/** Get the x, y position in the graphics context of the last mouse down message. Does not get cleared on mouse up
	 * etc.
	 * @param x Where the X position will be stored
	 * @param y Where the Y position will be stored */
...
#pragma mark - Plug-in API Specific
	...
	/** [AAX only] This can be called by the ProTools API class (e.g. IPlugAAX) in order to ascertain the parameter
	 * linked to the control under the mouse. The purpose is to facillitate ProTool's special contextual menus (for
	 * configuring parameter automation)
...
#pragma mark - Resource/File Loading
	...
	{
		return mSharedResourcesSubPath.Get();
	}
	...
	virtual IBitmap LoadBitmap(const char* fileNameOrResID,
							   int nStates              = 1,
	 ...
	 * @param targetScale Set \c to a number > 0 to explicity load e.g. an @2x.png
	 * @return An IBitmap representing the image */
	virtual IBitmap LoadBitmap(const char* name,
							   const void* pData,
							   int dataSize,
							   int nStates              = 1,
							   bool framesAreHorizontal = false,
 ...
 protected:
	...
	virtual APIBitmap* LoadAPIBitmap(const char* fileNameOrResID,
									 int scale,
	...
	EResourceLocation SearchImageResource(
	 ...
	 * @param bounds /todo
	 * @param pBlend /todo */
	virtual void DoDrawText(const IText& text,
							const char* str,
							const IRECT& bounds,
							const IBlend* pBlend = nullptr) = 0;
...
#pragma mark -
 ...
 private:
	...
	float mDrawScale = 1.f;  // scale deviation from  default width and height i.e stretching the UI by dragging bottom
							 // right hand corner
	...
	std::unordered_map<ITouchID, IControl*> mCapturedMap;  // associative array of touch ids to control pointers, the
														   // same control can be touched multiple times
