...
public:
  ...
  void Init(const IRECT& r, YGFlexDirection direction = YGFlexDirectionRow, YGJustify justify = YGJustifyFlexStart,
            YGWrap wrap = YGWrapNoWrap, float padding = 0.f, float margin = 0.f);

  /** Add a flex item, with some parameters
   * @param width Postitive value sets width in pixels, negative value sets percentage or YGUndefined for "auto"
   * https://yogalayout.com/docs/width-height
   * @param height Postitive value sets height In pixels, negative value sets percentage or YGUndefined for "auto"
   * https://yogalayout.com/docs/width-height
  ...
  YGNodeRef AddItem(float width, float height, YGAlign alignSelf = YGAlignAuto, float grow = 0.f, float shrink = 1.f,
                    float margin = 0.f);
