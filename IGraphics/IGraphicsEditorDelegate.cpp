...
IGEditorDelegate::IGEditorDelegate(int nParams) : IEditorDelegate(nParams) {}
...
void IGEditorDelegate::CloseWindow()
{
	if (!mClosing)
	{
		...
		if (mGraphics)
		{
			mLastWidth  = mGraphics->GetWidth();
			mLastHeight = mGraphics->GetHeight();
...
bool IGEditorDelegate::SerializeEditorSize(IByteChunk& data) const
{
	...
	int width   = mGraphics ? mGraphics->GetWidth() : mLastWidth;
	int height  = mGraphics ? mGraphics->GetHeight() : mLastHeight;
