                                                  ...
                                                  const char* sharedResourceSubPath)
{
  ...
  const EResourceLocation fontLocation =
    LocateResource(fileNameOrResID, "ttf", fullPath, bundleID, nullptr, sharedResourceSubPath);
  ...
  CFLocal<CGDataProviderRef> provider(url.Get() ? CGDataProviderCreateWithURL(url.Get())
                                                : nullptr); // CGDataProviderCreateWithURL will fail in macOS sandbox!
...
PlatformFontPtr CoreTextHelpers::LoadPlatformFont(const char* fontID, const char* fontName, ETextStyle style)
{
  ...
  CFLocal<CFDictionaryRef> dictionary(CFDictionaryCreate(NULL, (const void**)&keys, (const void**)&values, 2,
...
PlatformFontPtr CoreTextHelpers::LoadPlatformFont(const char* fontID, void* pData, int dataSize)
{
  ...
  CFLocal<CGDataProviderRef> provider(
    CGDataProviderCreateWithData(nullptr, dataCopy, (size_t)dataSize, &releaseFontData));
...
void CoreTextHelpers::CachePlatformFont(const char* fontID, const PlatformFontPtr& font,
...
CoreTextFontDescriptor* CoreTextHelpers::GetCTFontDescriptor(const IText& text,
                                                             StaticStorage<CoreTextFontDescriptor>& cache)
{
  StaticStorage<CoreTextFontDescriptor>::Accessor storage(cache);
