  ...
  for (int i = 0; i < numItems; ++i)
  {
    ...
    if (mMenu->GetPrefix())
    {
      ...
      switch (mMenu->GetPrefix())
      {
        ...
        case 0:
        default: prefixString = [NSString stringWithUTF8String:""]; break;
      }
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
  ...
  if (pItem->GetChecked())
    ...
    cell.accessoryType =
      pItem->GetSubmenu() ? UITableViewCellAccessoryDisclosureIndicator : UITableViewCellAccessoryNone;
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
  ...
  if (pSubMenu)
  {
    IGRAPHICS_UITABLEVC* newViewController = [[IGRAPHICS_UITABLEVC alloc] initWithIPopupMenuAndIGraphics:
                                                                                                pSubMenu:mGraphics];
    [newViewController
      setTitle:[NSString stringWithUTF8String:CStringHasContents(pSubMenu->GetRootTitle()) ? pSubMenu->GetRootTitle()
                                                                                           : pItem->GetText()]];
...
- (void)createTextEntry:(int)paramIdx:(const IText&)text:(const char*)str:(int)length:(CGRect)areaRect
{
  ...
  mAlertController = [UIAlertController alertControllerWithTitle:@"Input a value:"
                                                         message:@""
                      ...
                      :(IMsgBoxCompletionHandlerFunc)completionHandler
{
  ...
  void (^handlerBlock)(UIAlertAction*) = ^(UIAlertAction* action) {
...
- (void)attachGestureRecognizer:(EGestureType)type
{
  ...
  switch (type)
  {
    ...
    {
      gestureRecognizer = [[UILongPressGestureRecognizer alloc] initWithTarget:self
                                                                        action:@selector(onLongPressGesture:)];
      [(UILongPressGestureRecognizer*)gestureRecognizer
        setNumberOfTouchesRequired:type == EGestureType::LongPress1 ? 1 : 2];
      break;
    }
...
- (void)traitCollectionDidChange:(UITraitCollection*)previousTraitCollection
{
  ...
  if (mGraphics)
  {
    mGraphics->OnAppearanceChanged([self.traitCollection userInterfaceStyle] == UIUserInterfaceStyleDark
                                     ? EUIAppearance::Dark
                                     : EUIAppearance::Light);
  }
}
