...
public:
  ...
  IFontDataPtr GetFontData() override { return IFontDataPtr(new IFontData(mData.Get(), mData.GetSize(), 0)); }
...
static EM_BOOL key_callback(int eventType, const EmscriptenKeyboardEvent* pEvent, void* pUserData)
{
  ...
  IKeyPress keyPress{keyUTF8.Get(), domVKToWinVK(pEvent->keyCode), static_cast<bool>(pEvent->shiftKey),
                     static_cast<bool>(pEvent->ctrlKey || pEvent->metaKey), static_cast<bool>(pEvent->altKey)};
...
static EM_BOOL outside_mouse_callback(int eventType, const EmscriptenMouseEvent* pEvent, void* pUserData)
{
  ...
  info.ms = {(pEvent->buttons & 1) != 0, (pEvent->buttons & 2) != 0, static_cast<bool>(pEvent->shiftKey),
             static_cast<bool>(pEvent->ctrlKey), static_cast<bool>(pEvent->altKey)};
...
EM_BOOL touch_callback(int eventType, const EmscriptenTouchEvent* pEvent, void* pUserData)
{
  ...
  switch (eventType)
  {
    case EMSCRIPTEN_EVENT_TOUCHSTART: pGraphics->OnMouseDown(points); return true;
    case EMSCRIPTEN_EVENT_TOUCHEND: pGraphics->OnMouseUp(points); return true;
    case EMSCRIPTEN_EVENT_TOUCHMOVE: pGraphics->OnMouseDrag(points); return true;
    case EMSCRIPTEN_EVENT_TOUCHCANCEL: pGraphics->OnTouchCancelled(points); return true;
...
EMsgBoxResult IGraphicsWeb::ShowMessageBox(const char* str, const char* caption, EMsgBoxType type,
                                           IMsgBoxCompletionHandlerFunc completionHandler)
{
  ...
  switch (type)
  {
    ...
    // case MB_CANCEL:
    //   break;
    default: return result = kNoResult;
  }
                                 ...
                                 IFileDialogCompletionHandlerFunc completionHandler)
{
  ...
  // inputEl.call<void>("addEventListener", std::string("onChange"), val::module_property("file_dialog_callback"),
  // false);
}
...
void IGraphicsWeb::CreatePlatformTextEntry(int paramIdx, const IText& text, const IRECT& bounds, int length,
                                           const char* str)
{
  ...
  if (paramIdx > kNoParameter)
  {
    ...
    switch (pParam->Type())
    {
      ...
      case IParam::kTypeBool:
        ...
        break;
      case IParam::kTypeDouble: input.set("type", val("number")); break;
      default: break;
    }
  }
  else
...
bool IGraphicsWeb::OpenURL(const char* url, const char* msgWindowTitle, const char* confirmMsg,
...
PlatformFontPtr IGraphicsWeb::LoadPlatformFont(const char* fontID, const char* fileNameOrResID)
{
  ...
  const EResourceLocation fontLocation =
    LocateResource(fileNameOrResID, "ttf", fullPath, GetBundleID(), nullptr, nullptr);
