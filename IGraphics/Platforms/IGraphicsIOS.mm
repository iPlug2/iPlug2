...
: IGRAPHICS_DRAW_CLASS(dlg, w, h, fps, scale)
{
  ...
  if (!gTextureMap.size())
  {
    ...
    if ([pTextureFiles count])
    {
      ...
      {
        gTextureMap.insert(std::make_pair([[[pTextureFiles[i] lastPathComponent] stringByDeletingPathExtension]
                                            cStringUsingEncoding:NSUTF8StringEncoding],
                                          (MTLTexturePtr)gTextures[i]));
      }
...
EMsgBoxResult IGraphicsIOS::ShowMessageBox(const char* str, const char* caption, EMsgBoxType type,
...
void IGraphicsIOS::CreatePlatformTextEntry(int paramIdx, const IText& text, const IRECT& bounds, int length,
                                           const char* str)
{
  ReleaseMouseCapture();
  CGRect areaRect = ToCGRect(this, bounds);
...
bool IGraphicsIOS::OpenURL(const char* url, const char* msgWindowTitle, const char* confirmMsg,
