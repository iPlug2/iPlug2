...
{
 public:
	InstalledFont(void* data, int resSize) : mFontHandle(nullptr)
	{
		if (data)
		{
			DWORD numFonts = 0;
...
{
 public:
	Font(HFONT font, const char* styleName, bool system) : PlatformFont(system), mFont(font), mStyleName(styleName) {}
	~Font()
	{
		DeleteObject(mFont);
	}

	FontDescriptor GetDescriptor() override
	{
		return mFont;
	}
...
EMsgBoxResult IGraphicsWin::ShowMessageBox(const char* text,
										   const char* caption,
										   EMsgBoxType type,
										   IMsgBoxCompletionHanderFunc completionHandler)
{
	ReleaseMouseCapture();
...
void* IGraphicsWin::OpenWindow(void* pParent)
{
	...
	mPlugWnd = CreateWindow(wndClassName,
							"IPlug",
	...
	if (mPlugWnd && TooltipsEnabled())
	{
		...
		{
			mTooltipWnd = CreateWindowEx(0,
										 TOOLTIPS_CLASS,
										 NULL,
										 WS_POPUP | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | TTS_NOPREFIX | TTS_ALWAYSTIP,
										 CW_USEDEFAULT,
										 CW_USEDEFAULT,
										 CW_USEDEFAULT,
										 CW_USEDEFAULT,
...
void IGraphicsWin::CreatePlatformTextEntry(
...
void IGraphicsWin::PromptForFile(WDL_String& fileName, WDL_String& path, EFileAction action, const char* extensions)
{
	...
	if (rc)
	{
		...
		if (_splitpath_s(
				tempUTF8.Get(), drive, sizeof(drive), directoryOutCStr, sizeof(directoryOutCStr), NULL, 0, NULL, 0) ==
			0)
		{
			path.Set(drive);
...
static HFONT GetHFont(const char* fontName,
					  int weight,
					  bool italic,
					  bool underline,
					  ...
					  bool enumerate = false)
{
	...
	constexpr auto enumProc = [](const LOGFONT* pLFont, const TEXTMETRIC* pTextMetric, DWORD FontType, LPARAM lParam) {
		return -1;
	};

	if ((!enumerate || EnumFontFamiliesEx(hdc, &lFont, enumProc, 0, 0) == -1))
