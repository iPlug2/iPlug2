...
PlatformFontPtr IGraphicsWin::LoadPlatformFont(const char* fontID, const char* fileNameOrResID, void* winModule)
{
  StaticStorage<InstalledFont>::Accessor fontStorage(sPlatformFontCache);
  ...
  const EResourceLocation fontLocation = LocateResource(fileNameOrResID, "ttf", fullPath, GetBundleID(), winModule, nullptr);
  ...
  switch (fontLocation)
  {
    ...
    case kWinBinary:
    {
      pFontMem = const_cast<void *>(LoadWinResource(fullPath.Get(), "ttf", resSize, winModule));
