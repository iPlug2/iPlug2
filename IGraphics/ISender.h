...
template <int MAXNC = 1, typename T = float> struct ISenderData
{
	...
	ISenderData(int ctrlTag, int nChans, int chanOffset) : ctrlTag(ctrlTag), nChans(nChans), chanOffset(chanOffset) {}
...
{
 public:
	/** Queue sample buffers into the sender, checking the data is over the required threshold. This can be called on
