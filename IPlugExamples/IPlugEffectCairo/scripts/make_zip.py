...
def  main():
  ...
  installer = "\installer\IPlugEffectCairo Installer.exe"
   
  if demo:
    installer = "\installer\IPlugEffectCairo Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugEffectCairo manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugEffectCairo-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugEffectCairo-v" + FULLVERSIONSTR + "-win-demo.zip"
