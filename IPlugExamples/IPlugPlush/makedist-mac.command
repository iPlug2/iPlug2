...
VST2=${VST2//\VST_FOLDER = }/IPlugPlush.vst

VST3=`echo | grep VST3_FOLDER ../../common.xcconfig`
VST3=${VST3//\VST3_FOLDER = }/IPlugPlush.vst3

AU=`echo | grep AU_FOLDER ../../common.xcconfig`
AU=${AU//\AU_FOLDER = }/IPlugPlush.component

APP=`echo | grep APP_FOLDER ../../common.xcconfig`
APP=${APP//\APP_FOLDER = }/IPlugPlush.app

# Dev build folder
RTAS=`echo | grep RTAS_FOLDER ../../common.xcconfig`
RTAS=${RTAS//\RTAS_FOLDER = }/IPlugPlush.dpm
RTAS_FINAL="/Library/Application Support/Digidesign/Plug-Ins/IPlugPlush.dpm"

# Dev build folder
AAX=`echo | grep AAX_FOLDER ../../common.xcconfig`
AAX=${AAX//\AAX_FOLDER = }/IPlugPlush.aaxplugin
AAX_FINAL="/Library/Application Support/Avid/Audio/Plug-Ins/IPlugPlush.aaxplugin"

PKG='installer/build-mac/IPlugPlush Installer.pkg'
PKG_US='installer/build-mac/IPlugPlush Installer.unsigned.pkg'
...
xcodebuild -project IPlugPlush.xcodeproj -xcconfig IPlugPlush.xcconfig -target "All" -configuration Release 2> ./build-mac.log
...
echo "setting icons"
echo ""
setfileicon resources/IPlugPlush.icns $AU
...
setfileicon resources/IPlugPlush.icns "${AAX}"
...
sudo ashelper -f "${AAX_FINAL}/Contents/MacOS/IPlugPlush" -l ../../../Certificates/aax.crt -k ../../../Certificates/aax.key -o "${AAX_FINAL}/Contents/MacOS/IPlugPlush"
#---------------------------------------------------------------------------------------------------------
...
# echo ""
# codesign -f -s "3rd Party Mac Developer Application: ""${CERT_ID}" $APP --entitlements resources/IPlugPlush.entitlements
#  
...
echo ""
packagesbuild installer/IPlugPlush.pkgproj
...
#set installer icon
setfileicon resources/IPlugPlush.icns "${PKG}"
...
then
  ...
  sudo rm -R -f installer/IPlugPlush.dmg
fi
